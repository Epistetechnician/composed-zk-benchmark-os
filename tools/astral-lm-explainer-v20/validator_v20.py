...
def validate_lock(root: Path) -> dict:
    ...
    if not lock["assessment_effects_absent"] or lock["assessment_family_count"] != 50:
    ...
    if any(len(rows) != 50 for rows in predictions.values()):
...
def validate(root: Path) -> dict:
    ...
    if result["classification"] not in {
        ...
        "ContinuousMarginReplicationNoCandidate",
    }:
    ...
    if result["classification"] == "ContinuousMarginReplicationCandidate":

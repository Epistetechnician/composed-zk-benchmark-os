"""Independent aggregate and raw-event validator for V3.
    ...
    *,
    ...
    try:
        ...
        expectation = protocol.RunExpectation(
            ...
            causal_events=int(counts.get("bundle_ablation", 0))
            + int(counts.get("singleton_ablation", 0))
            + int(counts.get("bundle_replacement", 0))
            + int(counts.get("shuffled_bundle", 0))
            + int(counts.get("constant_bundle", 0))
            + int(counts.get("decoy_bundle", 0))
    ...
    value = {
        ...
        "validator": "validate_v3_slice.validate_run",
    }
...
def validate_aggregate(aggregate: dict[str, Any]) -> dict[str, Any]:
    ...
    if aggregate.get("classification") not in {"HeldOutCausalFeatureBundleAssessmentV3", "NoCandidate"}:
    ...
    if aggregate.get("classification") == "HeldOutCausalFeatureBundleAssessmentV3":

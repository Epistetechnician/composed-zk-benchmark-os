"""Fail-closed contracts for the Gemma 3 causal feature-bundle slice.
...
STATE_SLICE = "astral-trace-completeness-gemma3-causal-feature-bundle-effects-v3"
PROTOCOL_ID = "astral-trace-completeness-gemma3-causal-feature-bundle-effects-v3.0"
MODEL_ID = "google/gemma-3-1b-pt"
MODEL_ROOT = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
CUSTODY_ROOT = Path(
    "/Users/shaanp/Documents/astral-custody/"
    "trace-completeness-gemma3-causal-feature-bundle-effects-v3"
)
...
HIDDEN_WIDTH = 1_152
LAYER_COUNT = 26
CORPUS_ID = "gemma3-causal-feature-bundle-effects-cross-half-v3-20260902"
CORPUS_SEED = 2026090201
FAMILY_COUNT = 144
SPLIT_SIZE = 48
FIT_HALF_SIZE = 24
FEATURE_SELECTION_COUNT = 3
FEATURE_STABILITY_TOP_K = 16
FEATURE_STABILITY_MIN_INTERSECTION = 6
BUNDLE_PAIRWISE_CORRELATION_MIN = 0.30
REPEAT_COUNT = 3
RAW_RETENTION_HOURS = 72
CONSTANT_FEATURE_VALUE = 1.0

# These digests are the exact V3 model, runtime, and asset identities. Each is
...
# These V3 identity digests are frozen only after fresh external custody and
# corpus generation. They are intentionally not imported from V1 scientific
# artifacts.
FRESH_ASSET_QC_SHA256 = "531fdc214f466232bad30115b6cdf39daeb0a70cb47c8efd7f7e974e30b59467"
CORPUS_MANIFEST_SHA256 = "31fb4e9f3554e148a4fdd12db5d7c3ad81d363392059f894995b04c61cca200e"
MODEL_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "model-binding-v3.json"
RUNTIME_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "runtime-binding-v3.json"
ASSET_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "asset-qc-v3.json"
CORPUS_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "corpus-custody-v3.json"
TRANSFER_BUCKET = "astral-trace-completeness-gemma3-causal-feature-bundle-effects-v3"
TRANSFER_PREFIXES = {
    "model": "model/",
...
QUALIFICATION_CEILING = "LocalDevelopmentGemma3JointBundleCausalQualificationV3"
ASSESSMENT_CEILING = "LocalDevelopmentGemma3HeldOutJointBundleCausalAssessmentV3"

NODE_PROVIDER = "GiveMeANode"
NODE_ALLOCATION_RECEIPT = CUSTODY_ROOT / "node" / "allocation-receipt.json"
NODE_ID: str | None = None
HARD_SPEND_CEILING_USD: float | None = None
OPERATOR_ID = "shaanp"
OPERATOR_HOST = "Shaans-MacBook-Pro"
REVIEWER_ROLE = "independent-causal-feature-bundle-effects-reviewer-v3"
...
class ProtocolError(ValueError):
    """Raised when a V3 contract or receipt is not admissible."""
...
def _load_binding(path: Path, name: str) -> dict[str, Any]:
    if not path.is_file() or path.is_symlink():
        raise ProtocolError(f"V3 {name} binding is missing")
    if stat.S_IMODE(path.stat().st_mode) != 0o600:
        raise ProtocolError(f"V3 {name} binding mode is not 0600")
    value = strict_json(path)
    if not isinstance(value, dict):
        raise ProtocolError(f"V3 {name} binding is not an object")
    ...
    if value.get("protocol") != PROTOCOL_ID or value.get("state_slice") != STATE_SLICE:
        raise ProtocolError(f"V3 {name} binding identity mismatch")
    if value.get("binding_type") != name:
        raise ProtocolError(f"V3 {name} binding type mismatch")
    if value.get("receipt_sha256") != digest_json(unsigned):
        raise ProtocolError(f"V3 {name} binding digest mismatch")
    return value
...
def validate_external_bindings(root: Path = CUSTODY_ROOT, packet_value: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not errors:
        ...
        if asset_payload.get("asset_qc_sha256") != digest_json(asset_qc):
            errors.append("V3 asset QC payload digest is malformed")
        if (
            ...
            errors.append("V3 asset binding identity mismatch")
        ...
        if (
            ...
            errors.append("V3 corpus custody binding does not match the frozen corpus")
...
class TraceEvent:
    ...
    def validate(self) -> None:
        ...
        if not all(isinstance(key, str) and _is_metadata_value(value) for key, value in self.metadata.items()):
            raise ProtocolError("event metadata must be scalar or integer-sequence only")
        ...
        if self.kind == "intervention":
            ...
            expected_operator = f"exact-{intervention_kind}-v3"
            ...
            feature_index = self.metadata.get("feature_index")
            feature_indices = self.metadata.get("feature_indices")
            if intervention_kind in feature_kinds:
                if not isinstance(feature_indices, (list, tuple)) or not feature_indices:
                    raise ProtocolError("bundle intervention feature indices are missing")
                if any(not isinstance(index, int) or not 0 <= index < FEATURE_WIDTH for index in feature_indices):
                    raise ProtocolError("bundle intervention feature index is invalid")
                if len(set(feature_indices)) != len(feature_indices):
                    raise ProtocolError("bundle intervention feature indices are duplicated")
                if intervention_kind == "singleton_ablation" and len(feature_indices) != 1:
                    raise ProtocolError("singleton intervention must name one feature")
            elif feature_index is not None:
                raise ProtocolError("non-feature intervention carries a feature index")
...
class RunExpectation:
    ...
    def counts(self) -> dict[str, int]:
        ...
        return {
            ...
            "bundle_ablation": 0,
            "singleton_ablation": 0,
            "bundle_replacement": 0,
            "shuffled_bundle": 0,
            "constant_bundle": 0,
            "decoy_bundle": 0,
            "activation_patch": 0,
...
def public_contract() -> dict[str, Any]:
    value = {
        ...
        "feature_asset": {
            ...
            "fresh_v3_slice_qc_required": True,
            "prior_scientific_artifacts_as_inputs": False,
        },
        "roles": {
            "operator": OPERATOR_ID,
            "operator_host": OPERATOR_HOST,
            "runner": "tools/astral-trace-completeness-gemma3-causal-feature-bundle-effects-v3/run_v3.py",
        ...
        "corpus": {
            ...
            "fresh_from_prior_slices": True,
            "feature_stability": {
                "estimand": "cross-half replication of activation co-occurrence among fit-selected features",
                "discovery_half_families": FIT_HALF_SIZE,
                "replication_half_families": FIT_HALF_SIZE,
                "top_k_per_half": FEATURE_STABILITY_TOP_K,
                "minimum_intersection": FEATURE_STABILITY_MIN_INTERSECTION,
                "selection": "intersect the two fit-half top-16 activation ranks, enumerate triples, and select the triple maximizing the lower-half minimum absolute pairwise correlation with deterministic pooled-score and lexicographic tie-breaks; otherwise NoCandidate",
                "bundle_size": FEATURE_SELECTION_COUNT,
                "minimum_pairwise_absolute_correlation": BUNDLE_PAIRWISE_CORRELATION_MIN,
                "v1_scientific_inputs": False,
                "v2_scientific_inputs": False,
            },
        ...
        "estimand": {
            "theory": "a jointly causal three-feature SAE bundle has a non-additive downstream effect that is reproducible across family-disjoint splits",
            "primary": "paired family-level mean change in target-minus-distractor logit margin under simultaneous bundle ablation versus natural execution",
            "interaction": "kappa_B = tau_bundle - sum(tau_singleton_i) for the locked three-feature bundle",
            "secondary": "paired output-distribution movement and exact bundle replacement, activation patch, path patch, shuffled-bundle, constant-bundle, and decoy controls",
            ...
            "positivity": "all finite sealed families execute every declared arm and all three repeats; zero missing cells, zero duplicate cells, and zero excluded selected positions are permitted",
        ...
        "interchange_operator": {
            "bundle_ablation": "h'_p(t)=h_p(t)+D(z_B=0)-D(z) at p=layer12 post-feedforward output",
            "singleton_ablation": "the same decoder-difference operator with exactly one locked bundle coordinate set to zero",
            "bundle_replacement": "h'_p(t)=h_p(t)+D(z_{B}=z_{d,B})-D(z) using the fixed donor family",
            "activation_patch": "h'_p(t)=h_p^donor(t) for an exact-shape same-position donor run",
            "path_patch": "apply the exact bundle interchange over the frozen path tuple layer12.post_feedforward -> output_distribution",
            "causal_scrubbing": "h_{f<-d}=h_f+D(z_{f,B<-d,B})-D(z_f) with a predeclared assessment donor permutation",
            "locked_abstraction": "g(z_B)=intercept + three standardized main effects + three standardized pairwise products, fit once with ridge lambda 1.0 on fit-family effects",
            "constant_feature_value": CONSTANT_FEATURE_VALUE,
            "controls": "noop, exact-copy, zero, shuffled bundle donor, constant bundle donor, activation-only donor, text-only donor, and deterministic decoy triple",
        },
        "thresholds": {
            "primary_alpha": PRIMARY_ALPHA,
            "primary_feature_effect_sign_agreement_min": PRIMARY_FEATURE_EFFECT_SIGN_MIN,
            "prediction_family_sign_agreement_min": PRIMARY_PREDICTION_SIGN_MIN,
        ...
        "statistics": {
            "uncertainty": "10000 fixed-seed bootstrap resamples over family IDs after collapsing three repeats per family; 95 percent percentile interval",
            "multiplicity": "Holm correction over the two preregistered primary quantities tau_bundle and kappa_bundle; singleton and control effects are separately labeled",
            "power": {
                "target": POWER_TARGET,
...
def require_node_admission(node_receipt: Mapping[str, Any], *, spend_ceiling_usd: float) -> None:
    ...
    if node_receipt.get("mission") != STATE_SLICE:
        raise ProtocolError("node receipt mission is not the V3 state slice")
    if not isinstance(NODE_ID, str) or not NODE_ID:
        raise ProtocolError("V3 node identity is not frozen")
    ...
    if spend_ceiling_usd != HARD_SPEND_CEILING_USD:
        raise ProtocolError("requested spend ceiling is not the frozen V3 ceiling")
    ...
    if node_ceiling != HARD_SPEND_CEILING_USD:
        raise ProtocolError("node receipt spend ceiling is not the frozen V3 ceiling")

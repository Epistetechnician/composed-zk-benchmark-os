"""Fresh deterministic prompt-family roster for the causal bundle slice.
...
PROMPT_TEMPLATES = (
    "Solve the exact arithmetic expression: {left} {operator} {right} =",
    "Give the exact digit for {left} {operator} {right} =",
    "Evaluate this expression exactly: {left} {operator} {right} =",
    "Return only the arithmetic result: {left} {operator} {right} =",
    "The answer is the exact value of {left} {operator} {right} =",
    "Compute the single-digit result of {left} {operator} {right} =",
)
...
class PromptFamily:
    ...
    def prompt(self, variant: str = "clean") -> str:
        if variant not in VARIANTS:
            raise protocol.ProtocolError(f"unknown V3 corpus variant: {variant}")
        ...
        try:
            ...
            raise protocol.ProtocolError("V3 prompt template index is outside the frozen corpus") from exc
...
def families() -> tuple[PromptFamily, ...]:
    ...
    if len(roster) < protocol.FAMILY_COUNT:
        raise protocol.ProtocolError("token-safe V3 corpus roster is smaller than the frozen family count")
    ...
    for index in range(protocol.FAMILY_COUNT):
        ...
        result.append(PromptFamily(f"v3-family-{index:03d}", split, template_index, left, right, operation))
    return tuple(result)
...
def arm_order(family_id: str) -> tuple[str, ...]:
    ...
    if not any(family.family_id == family_id for family in families()):
        raise protocol.ProtocolError("unknown V3 family ID")
...
def public_manifest() -> dict[str, Any]:
    value = {
        ...
        "feature_stability": {
            ...
            "selection": "lower-half minimum absolute pairwise coactivation over every candidate triple in the top-16 intersection, with pooled score and lexicographic tie-breaks",
            "minimum_intersection": protocol.FEATURE_STABILITY_MIN_INTERSECTION,
            "bundle_size": protocol.FEATURE_SELECTION_COUNT,
            "minimum_pairwise_absolute_correlation": protocol.BUNDLE_PAIRWISE_CORRELATION_MIN,
        },

"""Packet compiler and independent-review admission boundary for V3.
...
SOURCE_FILES = (
    "protocol_v3_slice.py",
    "corpus_v3_slice.py",
    "custody_v3_slice.py",
    "registry_v3_slice.py",
    "adapter_v3_slice.py",
    "transcoder_v3_slice.py",
    "effects_v3_slice.py",
    "review_v3_slice.py",
    "validate_v3_slice.py",
    "run_v3.py",
)
DEPENDENCY_FILES = (
...
def source_manifest(repository_root: Path) -> dict[str, Any]:
    ...
    files = {
        f"tools/astral-trace-completeness-gemma3-causal-feature-bundle-effects-v3/{name}": protocol.sha256_file(root / name)
    ...
    expected_files = {
        *(f"tools/astral-trace-completeness-gemma3-causal-feature-bundle-effects-v3/{name}" for name in SOURCE_FILES),
        *DEPENDENCY_FILES,
    }
    value = {
...
def packet(repository_root: Path, custody_root: Path = protocol.CUSTODY_ROOT) -> dict[str, Any]:
    ...
    value = {
        ...
        "v2_freeze": {
            ...
            "freeze_rule": "V2 source, corpus, activations, effects, predictions, and results are immutable and are not V3 scientific inputs",
        },
        "new_theory": {
            "name": "jointly_causal_non_additive_three_feature_bundle",
            "change_from_v2": "the treatment is simultaneous bundle interchange and the estimand includes kappa_B non-additivity; no V2 effect, feature, prediction, or result value is imported",
            "claim_status": "hypothesis_to_be_tested",
        },
        "contract": contract,
        ...
        "required_external_bindings": {
            ...
            "fresh_v3_slice_model_manifest": True,
            "fresh_v3_slice_runtime_manifest": True,
            "fresh_v3_slice_asset_qc": True,
            "fresh_v3_slice_corpus_custody": True,
...
def validate_signed_acceptance(receipt: Mapping[str, Any], packet_digest: str) -> None:
    if receipt.get("protocol") != protocol.PROTOCOL_ID or receipt.get("state_slice") != protocol.STATE_SLICE:
        raise protocol.ProtocolError("review receipt identity is not bound to V3")
...
def static_review(packet_value: Mapping[str, Any], *, node_receipt: Mapping[str, Any] | None, spend_ceiling_usd: float | None, reviewer_receipt: Mapping[str, Any] | None, custody_root: Path = protocol.CUSTODY_ROOT) -> dict[str, Any]:
    ...
    if node_receipt is None:
        findings.append({"id": "V3-NODE-001", "severity": "critical", "finding": "exact GiveMeANode allocation receipt and node_id are absent"})
    else:
        try:
            ...
            findings.append({"id": "V3-NODE-002", "severity": "critical", "finding": str(exc)})
    if spend_ceiling_usd is None or spend_ceiling_usd <= 0:
        findings.append({"id": "V3-COST-001", "severity": "critical", "finding": "positive hard USD spend ceiling is absent"})
    binding_report = protocol.validate_external_bindings(custody_root, packet_value)
    if not binding_report["valid"]:
        findings.append({"id": "V3-BINDING-001", "severity": "critical", "finding": "; ".join(binding_report["errors"])})
    if reviewer_receipt is None:
        findings.append({"id": "V3-REVIEW-001", "severity": "critical", "finding": "genuinely independent packet-bound signed ACCEPT is absent"})
    else:
        try:
            validate_signed_acceptance(reviewer_receipt, str(packet_value["packet_sha256"]))
        except protocol.ProtocolError as exc:
            findings.append({"id": "V3-REVIEW-002", "severity": "critical", "finding": str(exc)})
    value = {
        "protocol": protocol.PROTOCOL_ID,
        "state_slice": protocol.STATE_SLICE,

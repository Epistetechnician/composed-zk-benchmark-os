#!/usr/bin/env python3
"""Independently validate the external V44 sealed panel.
...
def _receipt(errors: list[str], manifest_digest: str | None) -> dict[str, Any]:
    return {
        ...
        "claim_ceiling": "LocalDevelopmentV44PanelSealed",
        "classification": "PanelSealedForMeasurementInvariance" if not errors else "PanelInvalid",
        "valid": not errors,
        "errors": errors,
...
def validate(panel_root: Path, corpus_root: Path, model_root: Path, repository_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if manifest.get("candidate_layers") != list(protocol.CANDIDATE_LAYERS) or manifest.get("position_names") != list(protocol.POSITION_NAMES) or manifest.get("position_offsets") != list(protocol.POSITION_OFFSETS) or manifest.get("position_rule") != protocol.FIXED_POSITION_RULE:
        ...
        for key in (
            "by_split", "by_document", "documents_by_split", "authors_by_split",
            "corpus_validator_receipt_sha256", "wrapper_names", "control_names",
            "candidate_layers", "position_names", "position_offsets", "position_rule",
        ):
        ...
        for family in families:
            ...
            if family.get("response_position_rule") != protocol.FIXED_POSITION_RULE:
                errors.append(f"position_rule_mismatch:{family['family_id']}")

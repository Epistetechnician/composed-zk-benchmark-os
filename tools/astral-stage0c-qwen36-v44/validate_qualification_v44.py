#!/usr/bin/env python3
"""Independently validate V44 qualification custody and gates.
...
def _receipt(errors: list[str], result_digest: str | None) -> dict[str, Any]:
    return {
        ...
        "claim_ceiling": "LocalDevelopmentV44QualificationValidated",
...
def validate(qualification_root: Path, model_root: Path, repository_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for field in ("zero_replacement_max_abs_logit_delta_by_cell", "nonzero_reach_max_abs_logit_delta_by_cell"):
            value = result.get(field)
            if not isinstance(value, dict) or set(value) != expected_cells or any(not isinstance(item, (int, float)) or isinstance(item, bool) or not math.isfinite(float(item)) or item < 0 for item in value.values()):
                errors.append(f"invalid_cell_field:{field}")
        if result.get("candidate_layers") != list(protocol.CANDIDATE_LAYERS) or result.get("position_names") != list(protocol.POSITION_NAMES) or result.get("position_offsets") != list(protocol.POSITION_OFFSETS) or result.get("position_rule") != protocol.FIXED_POSITION_RULE:

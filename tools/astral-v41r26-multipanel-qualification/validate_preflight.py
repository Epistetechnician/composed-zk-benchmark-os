...
CONTRACT_SPEC = importlib.util.spec_from_file_location("v41r26_preflight_contract", HERE / "validate.py")
...
WORKER_SPEC = importlib.util.spec_from_file_location("v41r26_preflight_base", HERE / "validate_worker.py")
...
def expected_acquisition_rows() -> list[dict[str, Any]]:
    return [{"case_id": row["case_id"], "prompt": row["composition_prompt"],
            ...
            for row in CONTRACT.expected_acquisition()["cases"]]
...
def validate(artifact: Path, rgs_root: Path) -> dict[str, Any]:
    result_path, manifest = artifact / "preflight-result.json", artifact / "MANIFEST.sha256"
    ...
    for key, value in {"version": "mesh.astral_v41r26_multipanel_qualification.v1",
                       ...
                       "claim_ceiling": "RemoteH100CampaignPreflightV41R26"}.items():
    ...
    expected_p = CONTRACT.expected_protected()["rows"]; scored_p = result.get("protected_rows")
    ...
    if manifest.read_text() != file_hash(result_path).removeprefix("sha256:") + f"  {result_path.name}\n": errors.add("manifest")
    source = result.get("source", {}); commit = source.get("rgs_commit", "")
    for key, relative in (("runner_sha256", "scripts/run_v41r26_preflight.py"),
    ...
    return {"version": "astral.v41r26_preflight_artifact_validation.v1", "valid": not errors,

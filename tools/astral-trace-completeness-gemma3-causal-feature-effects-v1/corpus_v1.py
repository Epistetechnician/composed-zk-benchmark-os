...
class PromptFamily:
    ...
    def prompt(self, variant: str = "clean") -> str:
        if variant not in VARIANTS:
            raise protocol.ProtocolError(f"unknown V1 corpus variant: {variant}")
        ...
        try:
            ...
            raise protocol.ProtocolError("V1 prompt template index is outside the frozen corpus") from exc
...
def families() -> tuple[PromptFamily, ...]:
    ...
    if len(roster) < protocol.FAMILY_COUNT:
        raise protocol.ProtocolError("token-safe V1 corpus roster is smaller than the frozen family count")
    ...
    for index in range(protocol.FAMILY_COUNT):
        ...
        result.append(PromptFamily(f"v1-family-{index:03d}", split, template_index, left, right, operation))
    return tuple(result)
...
def arm_order(family_id: str) -> tuple[str, ...]:
    ...
    if not any(family.family_id == family_id for family in families()):
        raise protocol.ProtocolError("unknown V1 family ID")

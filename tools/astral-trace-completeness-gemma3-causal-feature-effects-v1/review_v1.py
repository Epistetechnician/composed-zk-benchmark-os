"""Packet compiler and independent-review admission boundary for V1.
...
def source_manifest(repository_root: Path) -> dict[str, Any]:
    ...
    files = {
        f"tools/astral-trace-completeness-gemma3-causal-feature-effects-v1/{name}": protocol.sha256_file(root / name)
    ...
    expected_files = {
        *(f"tools/astral-trace-completeness-gemma3-causal-feature-effects-v1/{name}" for name in SOURCE_FILES),
        *DEPENDENCY_FILES,
    }
    value = {
...
def packet(repository_root: Path, custody_root: Path = protocol.CUSTODY_ROOT) -> dict[str, Any]:
    ...
    value = {
        ...
        "v4_freeze": {
            ...
            "freeze_rule": "historical V4 source, corpus, effects, prediction, and results are immutable and are not V1 scientific inputs",
        },
        "contract": contract,
        ...
        "required_external_bindings": {
            ...
            "fresh_v1_model_manifest": True,
            "fresh_v1_runtime_manifest": True,
            "fresh_v1_asset_qc": True,
            "fresh_v1_corpus_custody": True,
...
def validate_signed_acceptance(receipt: Mapping[str, Any], packet_digest: str) -> None:
    if receipt.get("protocol") != protocol.PROTOCOL_ID or receipt.get("state_slice") != protocol.STATE_SLICE:
        raise protocol.ProtocolError("review receipt identity is not bound to V1")
...
def static_review(packet_value: Mapping[str, Any], *, node_receipt: Mapping[str, Any] | None, spend_ceiling_usd: float | None, reviewer_receipt: Mapping[str, Any] | None, custody_root: Path = protocol.CUSTODY_ROOT) -> dict[str, Any]:
    ...
    if node_receipt is None:
        findings.append({"id": "V1-NODE-001", "severity": "critical", "finding": "exact GiveMeANode allocation receipt and node_id are absent"})
    else:
        try:
            ...
            findings.append({"id": "V1-NODE-002", "severity": "critical", "finding": str(exc)})
    if spend_ceiling_usd is None or spend_ceiling_usd <= 0:
        findings.append({"id": "V1-COST-001", "severity": "critical", "finding": "positive hard USD spend ceiling is absent"})
    binding_report = protocol.validate_external_bindings(custody_root, packet_value)
    if not binding_report["valid"]:
        findings.append({"id": "V1-BINDING-001", "severity": "critical", "finding": "; ".join(binding_report["errors"])})
    if reviewer_receipt is None:
        findings.append({"id": "V1-REVIEW-001", "severity": "critical", "finding": "genuinely independent packet-bound signed ACCEPT is absent"})
    else:
        try:
            validate_signed_acceptance(reviewer_receipt, str(packet_value["packet_sha256"]))
        except protocol.ProtocolError as exc:
            findings.append({"id": "V1-REVIEW-002", "severity": "critical", "finding": str(exc)})
    value = {
        "protocol": protocol.PROTOCOL_ID,
        "state_slice": protocol.STATE_SLICE,

...
STATE_SLICE = "astral-trace-completeness-gemma3-causal-feature-effects-v1"
PROTOCOL_ID = "astral-trace-completeness-gemma3-causal-feature-effects-v1.0"
MODEL_ID = "google/gemma-3-1b-pt"
MODEL_ROOT = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
CUSTODY_ROOT = Path(
    "/Users/shaanp/Documents/astral-custody/"
    "trace-completeness-gemma3-causal-feature-effects-v1"
)
...
HIDDEN_WIDTH = 1_152
LAYER_COUNT = 26
CORPUS_ID = "gemma3-causal-feature-effects-token-safe-families-v2-20260831"
CORPUS_SEED = 2026083101
FAMILY_COUNT = 96
SPLIT_SIZE = 32
...
# These digests are the exact V1 model, runtime, and asset identities. Each is
...
V4_ASSET_QC_SHA256 = "a19f47ff2d36db1591476ae1c8b3a645b282b781df2e4303fc097cf413510abb"
# These are re-custodied V1 identity bindings. They are not scientific V4
# result artifacts and are populated only after the binding payloads pass QC.
FRESH_ASSET_QC_SHA256 = "14bbb0a00bcca0863ecd4be8ce87e12b6c3ee89eb9e7bce2a92307db7dc251f8"
CORPUS_MANIFEST_SHA256 = "dc4d667067a513979a304031fb0c9a3ed386bf600f7ae99f5da22100dfa42eeb"
MODEL_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "model-binding-v1.json"
RUNTIME_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "runtime-binding-v1.json"
ASSET_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "asset-qc-v1.json"
CORPUS_BINDING_RECEIPT = CUSTODY_ROOT / "receipts" / "corpus-custody-v2.json"
TRANSFER_BUCKET = "astral-trace-completeness-gemma3-causal-feature-effects-v1"
TRANSFER_PREFIXES = {
    "model": "model/",
...
QUALIFICATION_CEILING = "LocalDevelopmentGemma3CausalFeatureEffectsQualificationV1"
ASSESSMENT_CEILING = "LocalDevelopmentGemma3HeldOutCausalFeatureEffectsAssessmentV1"

NODE_PROVIDER = "GiveMeANode"
NODE_ALLOCATION_RECEIPT = CUSTODY_ROOT / "node" / "allocation-receipt.json"
NODE_ID = "7289c582-2e04-4d6a-ac3c-6ca8d4139356"
HARD_SPEND_CEILING_USD: float | None = 50.0
OPERATOR_ID = "shaanp"
OPERATOR_HOST = "Shaans-MacBook-Pro"
REVIEWER_ROLE = "independent-causal-feature-effects-reviewer-v1"
...
class ProtocolError(ValueError):
    """Raised when a V1 contract or receipt is not admissible."""
...
def _load_binding(path: Path, name: str) -> dict[str, Any]:
    if not path.is_file() or path.is_symlink():
        raise ProtocolError(f"V1 {name} binding is missing")
    if stat.S_IMODE(path.stat().st_mode) != 0o600:
        raise ProtocolError(f"V1 {name} binding mode is not 0600")
    value = strict_json(path)
    if not isinstance(value, dict):
        raise ProtocolError(f"V1 {name} binding is not an object")
    ...
    if value.get("protocol") != PROTOCOL_ID or value.get("state_slice") != STATE_SLICE:
        raise ProtocolError(f"V1 {name} binding identity mismatch")
    if value.get("binding_type") != name:
        raise ProtocolError(f"V1 {name} binding type mismatch")
    if value.get("receipt_sha256") != digest_json(unsigned):
        raise ProtocolError(f"V1 {name} binding digest mismatch")
    return value
...
def validate_external_bindings(root: Path = CUSTODY_ROOT, packet_value: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not errors:
        ...
        if asset_payload.get("asset_qc_sha256") != digest_json(asset_qc):
            errors.append("V1 asset QC payload digest is malformed")
        if (
            ...
            errors.append("V1 asset binding identity mismatch")
        ...
        if (
            ...
            errors.append("V1 corpus custody binding does not match the frozen corpus")
...
class TraceEvent:
    ...
    def validate(self) -> None:
        ...
        if self.kind == "intervention":
            ...
            expected_operator = f"exact-{intervention_kind}-v1"
...
def public_contract() -> dict[str, Any]:
    value = {
        ...
        "feature_asset": {
            ...
            "prior_qc_sha256": V4_ASSET_QC_SHA256,
            "fresh_v1_qc_required": True,
        },
        "roles": {
            "operator": OPERATOR_ID,
            "operator_host": OPERATOR_HOST,
            "runner": "tools/astral-trace-completeness-gemma3-causal-feature-effects-v1/run_v1.py",
            "validator": "tools/astral-trace-completeness-gemma3-causal-feature-effects-v1/validate_v1.py",
...
def require_node_admission(node_receipt: Mapping[str, Any], *, spend_ceiling_usd: float) -> None:
    ...
    if node_receipt.get("mission") != STATE_SLICE:
        raise ProtocolError("node receipt mission is not the V1 state slice")
    ...
    if spend_ceiling_usd != HARD_SPEND_CEILING_USD:
        raise ProtocolError("requested spend ceiling is not the frozen V1 ceiling")
    ...
    if node_ceiling != HARD_SPEND_CEILING_USD:
        raise ProtocolError("node receipt spend ceiling is not the frozen V1 ceiling")

#!/usr/bin/env python3
"""Independently validate the aggregate-only V40 assessment bundle.

State slice: astral-stage0c-qwen36-intervention-conditioned-target-v40.

The validator checks review, lock, source, digest, aggregate-retention, and
assessment-census invariants. It never reruns model effects.
"""
...
import run_assessment_v40 as assessment
...
FORBIDDEN_KEYS = frozenset({"prompts", "tokens", "raw_activations", "activations", "raw_logits", "logits", "raw_traces", "traces", "per_family_effects", "per_family_predictions", "credentials", "pii"})
...
def validate(assessment_root: Path, preassessment_root: Path, panel_root: Path, corpus_root: Path, qualification_root: Path, review_root: Path, model_root: Path, repository_root: Path) -> dict[str, Any]:
    errors: list[str] = []
    ...
    try:
        ...
        summary = assessment._strict_json(summary_path)
        run = assessment._strict_json(run_path)
        ...
        for document in (summary, run):
            ...
            if document.get("prediction_locked_before_assessment") is not True or document.get("independent_review_accepted") is not True and document is run:
                errors.append("assessment_gate_state_invalid")
        ...
        try:
            assessment._verify_review(review_root.resolve(), preassessment_root.resolve(), panel_root.resolve(), corpus_root.resolve(), qualification_root.resolve(), model_root.resolve())
        except (OSError, json.JSONDecodeError, protocol.ProtocolError, ValueError) as exc:
            errors.append(f"review_validation_failed:{type(exc).__name__}:{exc}")
        expected_digests = {
        ...
        for key, expected in expected_digests.items():
            ...
            if key in {"protocol_source_sha256", "corpus_manifest_sha256", "panel_manifest_sha256", "preassessment_run_manifest_sha256", "preassessment_prediction_lock_sha256", "preassessment_validator_receipt_sha256", "independent_review_receipt_sha256", "qualification_result_sha256", "qualification_validator_receipt_sha256", "model_manifest_sha256"} and run.get(key) != expected:
        ...
        if target.get("count") != protocol.FAMILIES_PER_SPLIT or not all(_finite(target.get(name)) for name in ("mean", "std", "min", "max")):
            errors.append("target_aggregate_invalid")
        if matched.get("count") != protocol.FAMILIES_PER_SPLIT or not all(_finite(matched.get(name)) for name in ("mean", "std", "min", "max")):
            errors.append("matched_aggregate_invalid")
        if matched.get("sequence_length_delta_max") != 0 or matched.get("donor_violations") != 0 or not _finite(matched.get("norm_relative_error_max")) or float(matched.get("norm_relative_error_max", 1.0)) > protocol.MATCH_NORM_RELATIVE_TOLERANCE:
        ...
        actual_files = {path.relative_to(assessment_root).as_posix() for path in assessment_root.rglob("*") if path.is_file()}
        if not EXPECTED_FILES <= actual_files or not actual_files <= EXPECTED_FILES | {"validator-receipt.json"}:
...
def main(argv: list[str] | None = None) -> int:
    ...
    receipt = validate(args.assessment_root, args.preassessment_root, args.panel_root, args.corpus_root, args.qualification_root, args.review_root, args.model, args.repository_root)

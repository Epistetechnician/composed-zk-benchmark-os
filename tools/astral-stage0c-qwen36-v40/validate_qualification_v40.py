#!/usr/bin/env python3
"""Independently validate the V40 qualification bundle.

State slice: astral-stage0c-qwen36-intervention-conditioned-target-v40.

This validator recomputes model and source custody and checks every aggregate
qualification gate. It does not infer scientific target validity.
"""
...
def validate(qualification_root: Path, model_root: Path, repository_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        runner_path = Path(__file__).resolve().parent / "qualify_v40.py"
        ...
        for key in ("zero_replacement_max_abs_logit_delta_by_layer", "nonzero_reach_max_abs_logit_delta_by_layer"):
            ...
            if not isinstance(values, dict) or set(values) != {str(layer) for layer in protocol.QUALIFICATION_LAYERS}:

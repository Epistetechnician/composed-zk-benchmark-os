...
SPEC = importlib.util.spec_from_file_location("v41r27_contract", HERE / "validate.py")
assert SPEC and SPEC.loader
CONTRACT = importlib.util.module_from_spec(SPEC); SPEC.loader.exec_module(CONTRACT)
BASE_PATH = Path(__file__).parents[1] / "astral-v41r25-disjoint-replay" / "validate_artifact.py"
BASE_SPEC = importlib.util.spec_from_file_location("v41r27_worker_base", BASE_PATH)
...
def validate(artifact: Path, rgs_root: Path) -> dict[str, Any]:
    ...
    expected_constants = {"version": "mesh.astral_v41r27_agem_retention.v1",
                          ...
                          "claim_ceiling": "RemoteH100AGEMRetentionWorkerV41R27"}
    ...
    if spec is not None:
        panel = int(spec["panel_id"].rsplit("-", 1)[1]); acquisition = CONTRACT.acquisition()["cases"]
        protected = CONTRACT.protected()["rows"]; selected = acquisition[panel * 4:panel * 4 + 4]
        protected_selected = protected[panel * 16:panel * 16 + 16]
        score_rows = [{"case_id": f'{row["case_id"]}-v41r27-exact', "prompt": row["composition_prompt"],
        ...
        if candidate.get("update", {}).get("projection_steps") != sum(
                receipt.get("projection_applied") is True for receipt in receipts): errors.add("projection_census")
    ...
    for key, relative in (("runner_sha256", "scripts/run_v41r27_worker.py"),
                          ("method_sha256", "mesh_brain/meshmodel/v41r27_agem_retention.py"),
                          ("instrument_sha256", "mesh_brain/meshmodel/v41r27_agem_retention.py"),
    ...
    return {"version": "astral.v41r27_worker_artifact_validation.v1", "valid": not errors,

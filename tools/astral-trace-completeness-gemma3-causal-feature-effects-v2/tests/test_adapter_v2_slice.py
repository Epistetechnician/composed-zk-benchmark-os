...
def test_intervention_kinds_are_typed_and_validated():
    ...
    assert adapter.intervention_metadata(plan)["operator"] == "exact-feature_ablation-v2"
...
def test_repeated_treatment_helper_checks_all_repeats(monkeypatch):
    ...
    family = __import__("corpus_v2_slice").PromptFamily("v2-family-000", "fit", 0, 2, 1, "sum")
    baseline = type("Run", (), {"logits": (torch.zeros(1, 10),), "trial_id": "v2-family-000:natural"})()
    calls = []
    ...
    def fake_run(*_args, repeat_index, intervention, **_kwargs):
        ...
        return type(
            ...
            (),
            {
                ...
                "trial_id": f"v2-family-000:{intervention.kind}",
            },
        )(), None
    ...
    rows = run_v2._repeated_effect_rows(
        family,
...
def test_control_gate_uses_full_logit_delta():
    ...
    result = run_v2._control_gate(rows)

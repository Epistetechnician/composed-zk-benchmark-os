"""External custody and raw-expiry helpers for the V2 causal slice.
...
def write_raw_events(
    ...
    *,
    ...
    if not receipt["valid"]:
        raise protocol.ProtocolError("V2 custody validation failed")
...
def _write_binding(root: Path, name: str, payload: dict[str, Any]) -> Path:
    ...
    if path.exists() or path.is_symlink():
        raise protocol.ProtocolError(f"V2 {name} binding already exists")
    unsigned = {
        "protocol": protocol.PROTOCOL_ID,
        "state_slice": protocol.STATE_SLICE,
        "binding_type": name,
...
def write_identity_bindings(
    ...
    *,
    ...
    """Create fresh V2 identity receipts without copying scientific results.
    ...
    if any(path.exists() or path.is_symlink() for path in protocol.binding_paths(root).values()):
        raise protocol.ProtocolError("V2 identity bindings are immutable once written")

    model_manifest = protocol.tree_manifest(model_root)
    if model_manifest["manifest_sha256"] != protocol.MODEL_MANIFEST_SHA256:
        raise protocol.ProtocolError("model tree does not match the frozen V2 model")
    model_payload = {
    ...
    if {entry["path"] for entry in asset_manifest["files"]} != expected_names:
        raise protocol.ProtocolError("V2 asset file set is not the frozen Gemma Scope asset")
    asset_qc_core = {
        ...
        "reconstruction_target_source": "fresh_model_v2_slice_fit_rows",
        "schema": {
    ...
    if asset_qc_sha256 != protocol.FRESH_ASSET_QC_SHA256:
        raise protocol.ProtocolError("asset QC does not match the frozen V2 asset identity")
    asset_payload = {
        **asset_qc_core,
    ...
    if (
        ...
        raise protocol.ProtocolError("corpus generator does not match the frozen V2 corpus")
...
def validate_root(root: Path, repository_root: Path) -> dict[str, Any]:
    """Return the canonical custody-root validation receipt for V2.

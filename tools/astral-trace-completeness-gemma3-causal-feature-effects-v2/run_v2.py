"""Qualification, locking, and held-out causal runner for V2.
...
import adapter_v2_slice as adapter
...
import registry_v2_slice as registry
import review_v2_slice as review
import transcoder_v2_slice as transcoder
import validate_v2_slice as validator


REPOSITORY_ROOT = Path(__file__).resolve().parents[2]
CAMPAIGN_ID = "v2-givemeanode-causal-feature-effects-20260901"
...
def _artifact_names(execution_id: str) -> tuple[str, str]:
    ...
    return (
        f"v2-causal-feature-effects-aggregate-{execution_id}.json",
...
def _capture_manifest(root: Path, run: adapter.TraceRun) -> dict[str, Any]:
    ...
    if not tensors:
        raise protocol.ProtocolError("V2 capture set is empty")
...
def _run_one(
    ...
    *,
    ...
    run._v2_slice_feature_store = store  # type: ignore[attr-defined]
    return run, receipt
...
def _feature_selection(
    ...
    for family in families:
        ...
        if any(
            _max_delta(
                repeated[0]._v2_slice_feature_store["features"],  # type: ignore[attr-defined]
                run._v2_slice_feature_store["features"],  # type: ignore[attr-defined]
            )
        ...
        features = repeated[0]._v2_slice_feature_store["features"]  # type: ignore[attr-defined]
        reconstruction = repeated[0]._v2_slice_feature_store["reconstruction"]  # type: ignore[attr-defined]
    ...
    if not reconstruction_gate["pass"]:
        raise protocol.ProtocolError("fresh V2 transcoder reconstruction gate failed")
    if len(half_vectors) != protocol.SPLIT_SIZE or protocol.FIT_HALF_SIZE * 2 != len(half_vectors):
        raise protocol.ProtocolError("V2 feature-stability halves are not the frozen fit split")
    discovery_scores = torch.stack(half_vectors[: protocol.FIT_HALF_SIZE]).sum(dim=0)
    replication_scores = torch.stack(half_vectors[protocol.FIT_HALF_SIZE :]).sum(dim=0)
    discovery_top = {
        int(index)
        for index in torch.topk(discovery_scores, protocol.FEATURE_STABILITY_TOP_K).indices.tolist()
    }
    replication_top = {
        int(index)
        for index in torch.topk(replication_scores, protocol.FEATURE_STABILITY_TOP_K).indices.tolist()
    }
    intersection = discovery_top & replication_top
    stability_pass = len(intersection) >= protocol.FEATURE_SELECTION_COUNT
    pooled = discovery_scores + replication_scores
    if stability_pass:
        stable_indices = torch.tensor(sorted(intersection), device=pooled.device, dtype=torch.long)
        stable_scores = pooled.index_select(0, stable_indices)
        order = torch.argsort(stable_scores, descending=True)[: protocol.FEATURE_SELECTION_COUNT]
        selected = tuple(int(stable_indices[index].item()) for index in order)
    else:
        selected = ()
    stability_gate = {
    ...
    return selected, reconstruction_gate, stability_gate
...
def _evaluate_split(
    ...
    for family in families:
        ...
        store = baseline._v2_slice_feature_store  # type: ignore[attr-defined]
...
def _fit_prediction(rows: Sequence[Mapping[str, Any]], selected_features: Sequence[int]) -> dict[str, Any]:
    ...
    value = {
        "graph": "locked-feature-ablation-margin-mean-v2",
...
def execute(
    *,
    ...
    if not protocol.custody_receipt(custody_root, repository_root)["valid"]:
        raise protocol.ProtocolError("V2 custody root is invalid")
    ...
    if not binding_report["valid"]:
        raise protocol.ProtocolError(f"V2 external identity bindings failed: {binding_report['errors']}")
    bindings = {
    ...
    if os.environ.get("HF_HUB_OFFLINE") != "1" or os.environ.get("TRANSFORMERS_OFFLINE") != "1":
        raise protocol.ProtocolError("V2 model execution requires offline Hugging Face flags")
    ...
    selected, reconstruction_gate, feature_stability_gate = _feature_selection(
        generator, tokenizer, transcoder_model, fit, custody_root
    )
    ...
    if (
        not feature_stability_gate["pass"]
        or not power_gate["pass"]
        or not fit_summary["all_pass"]
        or not tune_gate["pass"]
        or not tune_summary["all_pass"]
        or not tune_controls["pass"]
    ):

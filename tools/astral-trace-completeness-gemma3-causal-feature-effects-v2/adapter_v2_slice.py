...
class TraceEmitter(legacy.TraceEmitter):
    """Replace the V2 event constructor with the V2 typed event schema."""
...
def as_legacy_plan(intervention: CausalIntervention) -> Any:
    """Convert a V2 plan to the V2 hook ABI without changing the recipient."""
...
def intervention_metadata(intervention: CausalIntervention) -> dict[str, Any]:
    operator = f"exact-{intervention.kind}-v2"
    return {
...
class InstrumentedGenerator(legacy.InstrumentedGenerator):
    """V2 generator with a V2 plan binding for every intervention event."""

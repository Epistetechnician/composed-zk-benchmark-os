...
def fixture() -> dict[str, Any]:
    base = V37.fixture()
    body = {**{key: value for key, value in base.items() if key not in {"version", "arms", "fixture_sha256"}}, "version": "mesh.astral_v39_quarter_allocation_fixture.v1", "arms": PROTOCOL["arms"]}
...
def validate(root: Path) -> dict[str, Any]:
    ...
    expected_name = f"astral-rgs-v39-quarter-allocation-{str(manifest.get('manifest_sha256',''))[7:19]}-r1"
    ...
    for cell in cells:
        ...
        if cell["arm_id"].startswith("joint_"):
            ...
            if sum(r["prior_examples"] for r in later) != 72 or sum(r["protected_examples"] for r in later) != 24: errors.append("alternation")
    ...
    return {"version": "astral.v39_validation_report.v1", "valid": not errors, "status": status, "errors": errors, "artifact_manifest_sha256": manifest.get("manifest_sha256"), "claim_ceiling": PROTOCOL["claim_ceiling"], "model_execution": False, "external_review": "NotRun"}

#!/usr/bin/env python3
"""Run V41 Qwen3.6 instrument qualification.

State slice: astral-stage0c-qwen36-directional-block-target-v41.

This qualification is independent of the V40 result chain. It loads only the
already-cached checkpoint, verifies the controlled MLX seam, and retains an
aggregate receipt in a new external output root.
"""
        ...
        if self.mode != "none" and self.index == self.target_layer:
            if self.mode == "zero":
                ...
                prefix = self.mx.zeros(
                    (output.shape[0], max(output.shape[1] - 1, 0), output.shape[-1]),
                    dtype=output.dtype,
                )
                suffix = self.mx.ones((output.shape[0], 1, output.shape[-1]), dtype=output.dtype)
                delta = self.mx.concatenate(
                    [prefix, suffix * protocol.REPLACEMENT_SCALE], axis=1
                )
            output = output + delta
...
def qualify(model_root: Path, output_root: Path, repository_root: Path) -> Path:
    ...
    gates = {
        ...
        "feature_map_bound": protocol.feature_map_digest() == protocol.feature_map_digest(),
    ...
    result = {
        ...
        "qualification_id": f"{protocol.PROTOCOL_ID}-qualification-v1",

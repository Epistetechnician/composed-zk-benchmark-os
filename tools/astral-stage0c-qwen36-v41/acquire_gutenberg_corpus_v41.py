#!/usr/bin/env python3
"""Acquire and atomically seal the fresh V41 Gutenberg corpus.

State slice: astral-stage0c-qwen36-directional-block-target-v41.

This is an intake-only command. Network access is limited to downloading the
fixed V41 Gutenberg text and RDF metadata into a repository-external staging
root. Model execution is not performed here and is explicitly recorded as
closed in the corpus manifest.
"""
...
END_MARKER = b"*** END OF"
USER_AGENT = "Astral-V41-corpus-custody/1.0"
...
def acquire(output_root: Path, repository_root: Path) -> Path:
    ...
    try:
        ...
        _write_json(staging / "selection-manifest.json", selection_manifest)
        ...
        manifest = {
            "protocol": protocol.PROTOCOL_ID,
            "state_slice": protocol.STATE_SLICE,
            "corpus_kind": "project-gutenberg-canonical-text-rdf-v41",
            "selection_sha256": protocol.selection_digest(),
        ...
        _write_json(staging / "corpus-manifest.json", manifest)
        if output_root.exists():

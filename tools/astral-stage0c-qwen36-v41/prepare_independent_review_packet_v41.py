#!/usr/bin/env python3
"""Prepare the V41 independent-review packet.

State slice: astral-stage0c-qwen36-directional-block-target-v41.

This command validates the preassessment chain and publishes a pending,
aggregate-only packet. It cannot accept review or run assessment.
"""
...
def prepare(
    ...
    if review_root.exists():
        raise protocol.ProtocolError(f"refusing to overwrite review root: {review_root}")
    ...
    if summary.get("tune_utility_gate_passed") is not True:
        raise protocol.ProtocolError("tune utility gate failed; review and assessment remain closed")
    packet = {
        ...
        "review_checklist": [
            {
                ...
                "status": "PENDING",
                "required_verification": "Recompute model, runtime, installed-source, protocol, runner, and validator digests.",
            },
            {
                "item": "fresh_data_identity_and_lane_separation",
                "status": "PENDING",
                "required_verification": "Verify the V41 selection manifest, freshness exclusion digest, prior-panel exclusion, and no V40 scientific artifact reuse.",
            },
            {
                "item": "document_author_concept_split_disjointness",
                "status": "PENDING",
                "required_verification": "Recompute the 18-document/144-family census, author separation, contained-work exclusion, and 12-gram split exclusion.",
            },
            {
                "item": "direct_target_and_controls",
                "status": "PENDING",
                "required_verification": "Verify directional-block, clean activation, text-only, shuffled, constant, and matched controls; matched is not a learned target.",
            },
            {
                "item": "prediction_lock_before_assessment_effects",
                "status": "PENDING",
                "required_verification": "Verify fit/tune-only estimator state, canonical assessment order, absent assessment effects, and no per-family predictions retained.",
            },
            {
                "item": "privacy_and_aggregate_only_retention",
                "status": "PENDING",
                "required_verification": "Verify output census and absence of prompts, tokens, activations, logits, traces, credentials, PII, and per-family records.",
            },
            {
                "item": "validator_behavior_and_claim_ceiling",
                "status": "PENDING",
                "required_verification": "Run the independent validators and confirm Stage 0C, Stage 1, accepted-evidence, benchmark, and production claims remain closed.",
            },
        ],
    ...
    staging = Path(
        tempfile.mkdtemp(prefix=f".{review_root.name}.staging-", dir=str(review_root.parent))
    )
    try:

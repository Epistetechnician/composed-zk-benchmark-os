"""Asset-only quality and schema gate for the V3 transcoder candidate.

State slice: astral-trace-completeness-gemma3-end-to-end-v3.
This command does not load the base model or produce activation effects.
"""
...
def _tensor_specs(path: Path, expected: dict[str, tuple[tuple[int, ...], str]], *, check_finite: bool) -> dict[str, Any]:
    ...
    with safe_open(str(path), framework="pt", device="cpu") as handle:
        ...
        for key in handle.keys():
            ...
            observed[key] = {
                ...
                "finite": finite if check_finite else None,
            }
        ...
        for key, (shape, dtype) in expected.items():
            ...
            if actual["shape"] != shape or actual["dtype"] != dtype:
                raise protocol.ProtocolError(f"tensor schema mismatch: {path.name}:{key}")
            if check_finite and actual["finite"] is False:
                raise protocol.ProtocolError(f"nonfinite tensor: {path.name}:{key}")
...
def inspect_asset(asset_root: Path, *, repository_root: Path) -> dict[str, Any]:
    ...
    if not receipt["valid"]:
        raise protocol.ProtocolError(f"invalid V3 custody root: {receipt['errors']}")
    asset_dir = asset_root / protocol.ASSET_VARIANT
    config_path = asset_dir / "config.json"
    params_path = asset_dir / "params.safetensors"
    examples_path = asset_dir / "examples.safetensors"
    ...
    params = _tensor_specs(params_path, PARAMETER_SPECS, check_finite=True)
    examples = _tensor_specs(examples_path, EXAMPLE_SPECS, check_finite=True)
    value = {

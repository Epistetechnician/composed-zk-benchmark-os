"""Fresh deterministic V3 prompt families.
...
def families() -> tuple[PromptFamily, ...]:
    ...
    for index in range(FAMILY_COUNT):
        ...
        left = rng.randrange(25, 95)
        right = rng.randrange(2, 21)
        ...
        result.append(PromptFamily(f"v3-family-{index:03d}", split, left, right, operation))
    return tuple(result)

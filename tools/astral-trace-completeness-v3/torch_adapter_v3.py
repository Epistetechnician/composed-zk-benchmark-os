"""PyTorch-native synchronized generation trace adapter for Gemma 3 V3.

State slice: astral-trace-completeness-gemma3-causal-graph-v3.
"""
                ...
                if intervention is not None and emitter.step == intervention.step and path == intervention.module_path:
                    ...
                    emitter.emit(
                        ...
                        metadata={
                            ...
                            "operator": "exact-module-output-interchange-v3",
                        },
                    )
    ...
    def run(
        ...
        *,
        ...
        if input_ids.ndim != 2 or input_ids.shape[0] != 1:
            raise protocol.ProtocolError("V3 requires one prompt per isolated run")
        if max_new_tokens <= 0:

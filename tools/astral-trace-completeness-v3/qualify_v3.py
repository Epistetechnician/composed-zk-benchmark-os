"""Run the V3 offline end-to-end trace qualification.
...
CAMPAIGN_ID = "v3-hypothesis-1-affine-pooled-20260830"
...
def _custody_run(root: Path, repository_root: Path, run: adapter.TraceRun) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if not receipt["valid"]:
        raise protocol.ProtocolError(f"independent V3 event validation failed: {receipt['errors']}")
...
def _feature_observer(transcoder_model: Any, statistics: list[dict[str, float | int]], stability: list[Any]) -> Any:
    def observe(emitter: adapter.TraceEmitter, captures: dict[str, list[Any]], step: int, logits: Any) -> None:
        ...
        emitter.emit(
            ...
            metadata={"active_feature_count": int(__import__("torch").count_nonzero(features).item())},
        )
        emitter.emit(
            "sae_reconstruction",
            layer_index=12,
...
def _check_preload_review(custody_root: Path) -> dict[str, Any]:
    review_path = custody_root / "review" / "preload-review-v3-r2.json"
    review = protocol.strict_json(review_path)
    expected = {key: value for key, value in review.items() if key != "review_sha256"}
    if review.get("review_sha256") != protocol.digest_json(expected):
        raise protocol.ProtocolError("pre-load review digest mismatch")
    if review.get("status") != "PRELOAD_ACCEPTED_STATIC_VALIDATOR" or review.get("model_execution") is not False or review.get("assessment_opened") is not False:
        raise protocol.ProtocolError("pre-load review is not sealed and accepted")
    if review.get("signed_assessment_acceptance") is not False:
        raise protocol.ProtocolError("pre-load review cannot authorize assessment")
    return review
...
def execute(repository_root: Path, custody_root: Path, model_root: Path) -> dict[str, Any]:
    ...
    if not custody_receipt["valid"]:
        raise protocol.ProtocolError("V3 custody root invalid")
    review = _check_preload_review(custody_root)
    asset_qc = protocol.strict_json(custody_root / "aggregate" / "preload-asset-qc.json")
    if asset_qc.get("valid") is not True or asset_qc.get("model_execution") is not False:
        raise protocol.ProtocolError("V3 asset QC is not sealed")
    runtime = _runtime_manifest()
    if not runtime["offline_execution"]:
        raise protocol.ProtocolError("V3 qualification requires offline execution")
    ...
    if len(fit_families) != corpus.SPLIT_SIZE:
        raise protocol.ProtocolError("V3 fit split cardinality mismatch")
    runs: list[adapter.TraceRun] = []
    ...
    for family in fit_families:
        ...
        run = generator.run(
            input_ids,
            trial_id=family.family_id,
            max_new_tokens=1,
            capture_paths=(FEATURE_INPUT_PATH, FEATURE_OUTPUT_PATH),
            feature_observer=_feature_observer(transcoder_model, statistics, stability),
        ...
        captures.append(_write_capture(custody_root, run))
    first = runs[0]
    ...
    repeat_statistics: list[dict[str, float | int]] = []
    repeat_stability: list[Any] = []
    repeat = generator.run(
        first_input_ids,
        trial_id=f"{fit_families[0].family_id}-repeat",
        max_new_tokens=1,
        capture_paths=(FEATURE_INPUT_PATH, FEATURE_OUTPUT_PATH),
        feature_observer=_feature_observer(transcoder_model, repeat_statistics, repeat_stability),
        sae_feature_events_per_step=1,
        sae_reconstruction_events_per_step=1,
    )
    noop = generator.run(
        first_input_ids,
        trial_id=f"{fit_families[0].family_id}-noop",
        max_new_tokens=1,
        intervention=adapter.InterventionPlan(FEATURE_OUTPUT_PATH, 0, "noop"),
    )
    zero = generator.run(
        first_input_ids,
        trial_id=f"{fit_families[0].family_id}-zero",
        max_new_tokens=1,

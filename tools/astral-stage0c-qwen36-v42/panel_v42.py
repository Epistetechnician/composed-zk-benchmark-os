#!/usr/bin/env python3
"""Build the fresh V42 paired-wrapper target-reliability panel.

State slice: astral-stage0c-qwen36-causal-target-reliability-v42.

The corpus and tokenizer are used only to construct a sealed panel. No model
forward pass or intervention effect is performed here.
"""
...
def _load_manifest(corpus_root: Path) -> dict[str, Any]:
    ...
    if selection.get("selection") != list(protocol.SELECTION) or selection.get("selection_sha256") != protocol.selection_digest():
        raise protocol.ProtocolError("selection manifest does not match V42 selection")
    ...
    if manifest.get("corpus_validator_receipt_required") is not True or not receipt_path.is_file():
...
def build_registry(corpus_root: Path, tokenizer: Any) -> list[dict[str, Any]]:
    ...
    for document, candidates in sorted(parsed, key=lambda item: int(item[0]["gutenberg_id"])):
        ...
        for candidate in sorted(
            candidates,
            key=lambda item: _hash(protocol.PANEL_ID, "candidate", ebook_id, item["line_number"]),
        ):
        ...
        for ordinal, (candidate, target, distractor) in enumerate(chosen):
            ...
            counterfactual = re.sub(
                rf"\b{re.escape(target)}\b", distractor, excerpt, count=1, flags=re.IGNORECASE
            )
            ...
            registry.append(
                {
                    "family_id": f"v42-{document['split']}-doc{ebook_id}-{ordinal:02d}",
...
def build_panel(corpus_root: Path, model_manifest_sha256: str, registry: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "panel_kind": "document-derived-two-wrapper-paired-replacement-v42",

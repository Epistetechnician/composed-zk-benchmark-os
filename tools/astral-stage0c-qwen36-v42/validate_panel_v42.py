#!/usr/bin/env python3
"""Independently validate the external V42 sealed panel.
...
def _receipt(errors: list[str], manifest_digest: str | None) -> dict[str, Any]:
    return {
        ...
        "claim_ceiling": "LocalDevelopmentV42PanelSealed",
        "classification": "PanelSealedForTargetReliability" if not errors else "PanelInvalid",
        "valid": not errors,
        "errors": errors,
...
def validate(panel_root: Path, corpus_root: Path, model_root: Path, repository_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for key in ("by_split", "by_document", "documents_by_split", "authors_by_split", "corpus_validator_receipt_sha256", "wrapper_names", "control_names"):

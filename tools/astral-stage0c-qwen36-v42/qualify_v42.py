#!/usr/bin/env python3
"""Run the V42 Qwen3.6 layer-capture qualification.

State slice: astral-stage0c-qwen36-causal-target-reliability-v42.

This command is qualification only. It writes aggregate custody and seam
checks to a new external root and never writes activations or logits.
"""
    ...
    def __call__(self, x: Any, mask: Any = None, cache: Any = None) -> Any:
        ...
        if self.mode != "none" and self.index == self.target_layer:
            if self.mode == "zero":
                ...
                delta = self.mx.concatenate([prefix, suffix * protocol.REPLACEMENT_SCALE], axis=1)
            else:
...
def qualify(model_root: Path, output_root: Path, repository_root: Path) -> Path:
    ...
    result = {
        ...
        "claim_ceiling": "LocalDevelopmentV42InstrumentFeasibilityOnly",

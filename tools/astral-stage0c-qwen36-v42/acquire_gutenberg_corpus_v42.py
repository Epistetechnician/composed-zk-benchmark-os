#!/usr/bin/env python3
"""Acquire and atomically seal the V42 external Gutenberg corpus.
...
END_MARKER = b"*** END OF"
USER_AGENT = "Astral-V42-corpus-custody/1.0"
...
def acquire(output_root: Path, repository_root: Path) -> Path:
    ...
    try:
        ...
        protocol.write_json(
            staging / "corpus-manifest.json",
            {
                "protocol": protocol.PROTOCOL_ID,
                "state_slice": protocol.STATE_SLICE,
                "corpus_kind": "project-gutenberg-canonical-text-rdf-v42",
                "selection_sha256": protocol.selection_digest(),
                ...
                "freshness_exclusion_sha256": protocol.freshness_exclusion_digest(
                    protocol.FRESHNESS_EXCLUSION_INVENTORY
                ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {"corpus_root": str(root), "selection_sha256": protocol.selection_digest(), "valid": True},
            indent=2,
        )
    )
    return 0

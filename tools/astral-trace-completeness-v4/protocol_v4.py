"""Frozen V4 contracts for the second model-matched transcoder hypothesis.
...
PROTOCOL_ID = "astral-trace-completeness-gemma3-v4.1"
MODEL_ID = "google/gemma-3-1b-pt"
MODEL_ROOT = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
CUSTODY_ROOT = Path("/Users/shaanp/Documents/astral-custody/trace-completeness-gemma3-end-to-end-v4")
...
CORPUS_ID = "gemma3-trace-causal-families-v4-2026-08-30"
QUALIFICATION_CEILING = "LocalDevelopmentGemma3EndToEndCausalTraceQualificationV4"
ASSESSMENT_CEILING = "LocalDevelopmentGemma3HeldOutCausalTraceAssessmentV4"
HIDDEN_WIDTH = 1152
FEATURE_WIDTH = 16384
...
def custody_receipt(root: Path = CUSTODY_ROOT, repository_root: Path | None = None) -> dict[str, Any]:
    ...
    value = {"protocol": PROTOCOL_ID, "state_slice": STATE_SLICE, "root": str(root), "owner_uid": os.getuid(), "mode": "0700", "valid": not errors, "errors": errors}
...
def public_contract() -> dict[str, Any]:
    value = {
        ...
        "assignment": "fixed official L0-big affine asset variant and every eligible row in the fresh fit split",
...
def reject_raw_fields(value: Mapping[str, Any]) -> None:
    lowered = " ".join(str(key) for key in value.keys()).lower()
...
def validate_event_stream(events: Sequence[TraceEvent], expectation: RunExpectation) -> dict[str, Any]:
    ...
    return {**result, "protocol": PROTOCOL_ID, "state_slice": STATE_SLICE, "module_registry_sha256": digest_json({"inputs": list(expectation.module_input_paths), "outputs": list(expectation.module_output_paths)})}

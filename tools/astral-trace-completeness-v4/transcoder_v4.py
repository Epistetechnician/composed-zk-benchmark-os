"""V4 fixed affine Gemma Scope 2 transcoder loader and metric.
...
def load(*, root: Path = protocol.ASSET_ROOT, device: str = "cpu", dtype: Any = None) -> Any:
    ...
    if not path.is_file() or path.is_symlink():
        raise protocol.ProtocolError("V4 transcoder parameters are missing")
    return load_gemma_scope_2_transcoder(str(path), 12, device=torch.device(device), dtype=dtype or torch.float32)
...
def pooled_sufficient_statistics(transcoder: Any, input_acts: Any, target_acts: Any) -> dict[str, float | int]:
    ...
    with torch.no_grad():
        ...
        return {"sum_squared_error": float(torch.sum(error * error).item()), "target_sum": float(torch.sum(target).item()), "target_squared_sum": float(torch.sum(target * target).item()), "coordinate_count": int(target.numel())}

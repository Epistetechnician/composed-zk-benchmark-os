"""Fresh deterministic V4 prompt families.
...
FAMILY_COUNT = 48
SPLIT_SIZE = 16
VARIANTS = ("clean", "corrupted", "exact_copy_noop", "shuffled", "constant", "activation_only", "text_only", "access_null")
...
class PromptFamily:
    ...
    def prompt(self, variant: str = "clean") -> str:
        if variant not in VARIANTS:
            raise protocol.ProtocolError(f"unknown V4 corpus variant: {variant}")
...
def families() -> tuple[PromptFamily, ...]:
    ...
    for index in range(FAMILY_COUNT):
        ...
        left, right = rng.randrange(25, 95), rng.randrange(2, 21)
        if operation == "difference" and right >= left:
            left, right = right + 25, left % 21 + 1
        split = "fit" if index < SPLIT_SIZE else "tune" if index < 2 * SPLIT_SIZE else "assessment"
        result.append(PromptFamily(f"v4-family-{index:03d}", split, left, right, operation))
    return tuple(result)

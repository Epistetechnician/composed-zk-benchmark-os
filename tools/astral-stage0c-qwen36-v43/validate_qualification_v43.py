#!/usr/bin/env python3
"""Independently validate V43 qualification custody and gates.
...
def _receipt(errors: list[str], result_digest: str | None) -> dict[str, Any]:
    return {
        ...
        "claim_ceiling": "LocalDevelopmentV43QualificationValidated",
...
def validate(qualification_root: Path, model_root: Path, repository_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if result.get("candidate_layers") != list(protocol.CANDIDATE_LAYERS) or result.get("fixed_position") != protocol.FIXED_POSITION:

#!/usr/bin/env python3
"""Run the V43 Qwen3.6 capture/replacement qualification only.

State slice: astral-stage0c-qwen36-causal-target-localization-v43.
No panel or scientific effect is opened by this command. Only aggregate seam
and custody checks are written.
"""
...
def qualify(model_root: Path, output_root: Path, repository_root: Path) -> Path:
    ...
    repeat_delta = 0.0
    noop_delta = 0.0
    zero_replacement_delta = {str(layer): 0.0 for layer in protocol.QUALIFICATION_LAYERS}
    nonzero_reach = {str(layer): 0.0 for layer in protocol.QUALIFICATION_LAYERS}
    ...
    for prompt in protocol.QUALIFICATION_PROMPTS:
        ...
        native, _ = _forward(model, base_layers, tokens, "native", protocol.CANDIDATE_LAYERS[0], mx)
        wrapped, probes = _forward(model, base_layers, tokens, "noop", protocol.CANDIDATE_LAYERS[0], mx)
        repeated, _ = _forward(model, base_layers, tokens, "noop", protocol.CANDIDATE_LAYERS[0], mx)
        native_parity = max(native_parity, _max_abs(native, wrapped, mx))
        repeat_delta = max(repeat_delta, _max_abs(wrapped, repeated, mx))
        noop_delta = max(noop_delta, _max_abs(wrapped, _forward(model, base_layers, tokens, "none", protocol.CANDIDATE_LAYERS[0], mx)[0], mx))
        ...
        for target_layer in protocol.QUALIFICATION_LAYERS:
            zero, _ = _forward(model, base_layers, tokens, "zero", target_layer, mx)
            patched, _ = _forward(model, base_layers, tokens, "nonzero", target_layer, mx)
            zero_replacement_delta[str(target_layer)] = max(zero_replacement_delta[str(target_layer)], _max_abs(wrapped, zero, mx))
            nonzero_reach[str(target_layer)] = max(nonzero_reach[str(target_layer)], _max_abs(wrapped, patched, mx))
    ...
    result = {
        ...
        "claim_ceiling": "LocalDevelopmentV43InstrumentFeasibilityOnly",
        ...
        "zero_replacement_max_abs_logit_delta_by_layer": zero_replacement_delta,

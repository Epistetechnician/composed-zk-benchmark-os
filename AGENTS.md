...
Explicit managed-signature verification docs-first boundary now allowed and completed: Markdown spec and source-index updates under `docs/`, `README.md`, and `AGENTS.md`. This docs-first slice is limited to source-cited boundary planning in `docs/66-managed-signature-verification-boundary-spec.md` and URL attribution in `docs/research/zk_external_source_index.md` for zk-Harness, gnark, zkML benchmark, formal-lane, Phala/dstack, Azure Attestation, Intel Trust Authority, and attested-TLS references. It does not permit Rust implementation code, network access, JWKS fetch code, JWT signature verification code, Intel DCAP quote verification code, PCCS or collateral handling, TLS channel implementation, external repo clones, vendored source, secrets, external rails, backend execution, benchmark outputs, claims above `Attested`, or changes to Phase 4 registry semantics.

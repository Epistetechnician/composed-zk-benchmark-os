...
- Managed-attestation operator preflight examples and capture runbooks are capture input documentation only, not attestation evidence, not proof, not benchmark evidence, and not Phase 4 authorization.
- Managed-attestation Phase 57 real-artifact acceptance is managed-verifier local regression evidence only, not local DCAP quote verification, not managed-service signature verification, not benchmark evidence, not global software-agent uniqueness, and not Phase 4 authorization.

...
Explicit Phase M recursion-envelope stress docs-first opening now allowed: Markdown spec and navigation updates under `docs/`, `README.md`, and `AGENTS.md`. This phase is limited to the docs-first boundary contract in `docs/63-phase-m-recursion-envelope-stress-spec.md`: recursion-envelope input contracts, candidate metric labels, validation rules, negative tests, and claim-boundary restrictions. It does not permit Rust implementation code, live gnark execution, Go code, external repo clones, vendored source, external result import, benchmark outputs, official benchmark evidence, ZK backend performance claims, Level2+ evidence creation, dashboards, or claiming that recursion proof is semantic proof.

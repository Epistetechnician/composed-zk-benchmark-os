...
def validate_launch_binding(path: Path, repo_root: Path, result: dict[str, Any]) -> None:
    ...
    if ceiling != HARD_USD_CEILING or min(ceiling, estimate, rate, minutes) <= 0 or (rate * minutes).quantize(Decimal('0.01')) != estimate.quantize(Decimal('0.01')) or estimate > ceiling:

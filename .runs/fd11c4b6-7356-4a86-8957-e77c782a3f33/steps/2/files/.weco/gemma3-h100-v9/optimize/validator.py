...
EXCLUSION_RANGES = (
    ...
    ("prior-h100-v2", 18_432, 34_816),
    ("prior-h100-v1", 34_816, 51_200),
    ("prior-h100-v3", 51_200, 67_584),
    ("prior-h100-v4", 67_584, 83_968),
    ("prior-h100-v5", 83_968, 88_064),
    ("prior-h100-v6", 88_064, 92_160),
    ("prior-h100-v7", 92_160, 96_256),
    ("prior-h100-v8", 96_256, FRESH_START),
)
...
def external_runtime_path(path: Path, repo_root: Path, label: str) -> Path:
    ...
    while True:
        if curr.is_symlink():
            raise ValueError(f"{label} has a symlinked component: {curr}")
        if not curr.exists():
            raise ValueError(f"{label} component missing: {curr}")
        
        # Ancestor permission check: directories must be owner-writable but not group/other-writable.
        if curr.is_dir():
            if curr.stat().st_mode & 0o022:
                raise ValueError(f"{label} ancestor is mutable by others: {curr}")
        else:
            if curr.stat().st_mode & 0o222:
                raise ValueError(f"{label} ancestor is mutable: {curr}")
...
def require_immutable_external_file(path: Path, repo_root: Path, label: str) -> Path:
    return external_runtime_path(path, repo_root, label)
...
def source_bundle(raw_root: Path, source_root: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if len(excluded_ids) != 100352 or len(set(excluded_ids)) != len(excluded_ids) or manifest["excluded_id_sha256"] != hashlib.sha256(canonical(excluded_ids)).hexdigest():
...
def validate(result_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or any(not isinstance(runtime[key], str) for key in (*lock_fields, "cuda_driver_version", "gpu_name", "dtype", "network"))

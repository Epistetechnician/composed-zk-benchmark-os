...
PROTOCOL_PATH = Path(
    "docs/research/"
    "continual-learning/320-gemma3-fineweb-edu-replication-h100-v9-protocol.md"
)
PACKET_PATH = Path(
    "docs/research/"
    "continual-learning/321-gemma3-fineweb-edu-replication-h100-v9-review-packet.md"
)
...
def _immutable_existing_path(
    ...
    while True:
        if curr.is_symlink():
            raise ValueError(f"{label} has a symlinked path component: {curr}")
        if not curr.exists():
            raise ValueError(f"{label} component does not exist: {curr}")
        
        # Ancestor permission check: directories must be owner-writable but not group/other-writable.
        # This allows 0700 (bundle root) while rejecting insecure path components.
        # Files must be non-writable by anyone (checked later for descendants).
        if curr.is_dir():
            if curr.stat().st_mode & 0o022:
                raise ValueError(f"{label} directory component is group/other-writable: {curr}")
        else:
            if curr.stat().st_mode & 0o222:
                raise ValueError(f"{label} path component is mutable: {curr}")
...
def validate_implementation_manifest(
    ...
    if (
        manifest["schema"]
        != "gemma3-fineweb-edu-replication-h100-v9-implementation"
...
def validate_review_receipt(
    ...
    if receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v9-independent-review" or receipt["state_slice"] != STATE_SLICE or receipt["review_decision"] != "ACCEPT" or receipt["effects_run"] is not False:
...
def validate_launch_manifest(
    ...
    if manifest["runner_entrypoint"] != "run_h100_v9.sh":
        raise ValueError("runner entrypoint is not the reviewed provider entrypoint")
    if manifest["launch_command"] != "./run_h100_v9.sh":
    ...
    if manifest["network_lock"] != "network-none-v9":

...
def require_network_none() -> None:
    ...
    if not proc_net_dev.is_file():
        raise RuntimeError("network device proof is unavailable: /proc/net/dev")
    names = {
...
def run(model_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, result_root: Path, launch_manifest: Path, repo_root: Path | None = None, tokenizer: Any | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "hard_usd_ceiling": format(launch["hard_usd_ceiling"], ".2f"),
            "estimated_max_total_usd": format(launch["estimated_max_total_usd"], ".2f"),
            ...
            "provider_charged_usd": format(provider_receipt["charged_usd"], ".2f"),

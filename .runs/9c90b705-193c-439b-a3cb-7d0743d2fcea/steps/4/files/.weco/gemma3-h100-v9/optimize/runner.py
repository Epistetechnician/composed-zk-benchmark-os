...
def aggregate_metrics(metrics: dict[str, Any], windows: Sequence[Window]) -> dict[str, Any]:
    ...
    rows = metrics["rows"]
    if [row["document_id"] for row in rows] != expected_ids:
    ...
    total_nll = sum(Decimal(str(row["nll"])) for row in rows)
    mean_nll = (total_nll / Decimal(target_tokens)).quantize(Decimal("1e-9"))
    perplexity = mean_nll.exp().quantize(Decimal("1e-9"))
...
def decision(bootstrap: dict[str, Any]) -> str:
    mean, upper = bootstrap["mean_delta"], bootstrap["upper"]
...
def run(model_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, result_root: Path, launch_manifest: Path, repo_root: Path | None = None, tokenizer: Any | None = None) -> dict[str, Any]:
    ...
    with network_block():
        ...
        fit_candidates = [{"config": item["config"], "metrics": aggregate_metrics(item["metrics"], fit)} for item in candidates_raw]
        
        selected_index = min(
            (item["metrics"]["mean_nll"], index)
            for index, item in enumerate(fit_candidates)
        )[1]
        selected_fit = fit_candidates[selected_index]["config"]
        ...
        for source_layer, destination_layer in CANDIDATE_PAIRS:
            ...
            max_abs_nll_delta = max(abs(metrics["rows"][i]["nll"] - assessment_baseline_raw["rows"][i]["nll"]) for i in range(EXPECTED_WINDOWS))
            zero_raw[pair] = {"metrics": aggregate_metrics(metrics, assessment), "max_abs_nll_delta": max_abs_nll_delta}
    ...
    for i in range(EXPECTED_WINDOWS):
        b_nll = Decimal(str(assessment_baseline_raw["rows"][i]["nll"]))
        s_nll = Decimal(str(assessment_selected_raw["rows"][i]["nll"]))
        ledger_rows.append({
            "document_id": assessment[i].document_id,
            "baseline_nll": b_nll,
            "selected_nll": s_nll,
            "delta_selected_minus_baseline": s_nll / Decimal(WINDOW_TOKENS - 1) - b_nll / Decimal(WINDOW_TOKENS - 1),
        })

    reach: list[dict[str, Any]] = []
    for pair_idx, pair in enumerate(CANDIDATE_PAIRS):
        cand_rows = candidates_raw[pair_idx]["metrics"]["rows"]
        base_rows = fit_baseline_raw["rows"]
        maximum = max(abs(cand_rows[i]["nll"] - base_rows[i]["nll"]) for i in range(EXPECTED_WINDOWS))
    ...
    if max(item["max_abs_nll_delta"] for item in zero_raw.values()) > PARITY_TOLERANCE:
        raise RuntimeError("zero-alpha identity failed")
    if repeat != assessment_selected:
    ...
    try:
        ...
        with destination.open("x", encoding="utf-8") as handle:
            json.dump(result, handle, indent=2, sort_keys=True, default=lambda o: float(o) if isinstance(o, Decimal) else str(o))
            handle.write("\n")

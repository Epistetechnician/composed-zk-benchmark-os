...
def validate_provider_receipt(path: Path, launch: dict[str, Any], launch_sha256: str) -> dict[str, Any]:
    ...
    if (stop - start).total_seconds() > float(exact_decimal(launch["max_runtime_minutes"], "launch maximum runtime")) * 60:
    ...
    if (quote * exact_decimal(launch["max_runtime_minutes"], "launch maximum runtime")).quantize(Decimal('0.01')) != estimate.quantize(Decimal('0.01')) or estimate > ceiling or charged > estimate or charged > ceiling:
...
def source_bundle(raw_root: Path, source_root: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if set(manifest) != required or manifest["schema"] != "gemma3-fineweb-edu-replication-h100-v9-source" or manifest["state_slice"] != STATE_SLICE or manifest["dataset"] != {"repo": DATASET_REPO, "source": DATASET_SOURCE, "revision": DATASET_REVISION, "config": DATASET_CONFIG, "split": DATASET_SPLIT} or manifest["raw_root"] != "raw" or manifest["fresh_row_range"] != {"start": FRESH_START, "end_exclusive": FRESH_END, "count_per_shard": FRESH_END - FRESH_START} or manifest["excluded_ranges"] != [{"name": n, "start": s, "end_exclusive": e, "count_per_shard": e - s} for n, s, e in EXCLUSION_RANGES] or any(manifest[name] is not False for name in ("network_access", "training", "scientific_execution", "evidence_ledger_mutation")) or manifest["manifest_sha256"] != digest(manifest, "manifest_sha256"):
...
def validate_tree_manifest(root: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if config.get("model_type") != "gemma3" or MODEL_ARCHITECTURE not in config.get("architectures", []):
...
def validate_launch_binding(path: Path, repo_root: Path, result: dict[str, Any]) -> None:
    ...
    if ceiling != HARD_USD_CEILING or min(ceiling, estimate, rate, minutes) <= 0 or (rate * minutes).quantize(Decimal('0.01')) != estimate or estimate > ceiling:
...
def metric(value: Any, windows: dict[str, dict[str, Any]], expected_temperature: Decimal, expected_config: dict[str, Any] | None, label: str) -> None:
    ...
    if value["mean_nll"] != expected_mean or value["perplexity"] != Decimal(str(round(math.exp(float(expected_mean)), 9))):
...
def validate(result_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if set(receipt) != {"schema", "state_slice", "result_sha256", "result_file_sha256", "receipt_sha256"} or receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v9-result-receipt" or receipt["state_slice"] != STATE_SLICE or receipt["result_sha256"] != result["results_sha256"] or receipt["result_file_sha256"] != sha256_file(result_root / "result.json") or receipt["receipt_sha256"] != digest(receipt, "receipt_sha256"):
    ...
    try:
        ...
        if result["bootstrap"] != {**expected_bootstrap, "mean_delta": float(expected_bootstrap["mean_delta"]), "lower": float(expected_bootstrap["lower"]), "upper": float(expected_bootstrap["upper"])} or result["decision"] != ("ReplicationCandidate" if expected_bootstrap["mean_delta"] < 0 and expected_bootstrap["upper"] < 0 else "NoCandidate"):

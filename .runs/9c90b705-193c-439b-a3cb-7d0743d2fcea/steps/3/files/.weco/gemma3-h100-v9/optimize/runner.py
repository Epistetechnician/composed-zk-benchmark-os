...
def load_corpus(
    ...
    fit = [_window(row, f"fit window {index}", source_rows) for index, row in enumerate(
        _jsonl(root / f"fit/windows.jsonl", "fit windows")
    )]
    assessment = [_window(row, f"assessment window {index}", source_rows) for index, row in enumerate(
        _jsonl(root / f"assessment/windows.jsonl", "assessment windows")
    )]
...
def require_network_none() -> None:
    ...
    if not proc_net_dev.is_file():
        raise RuntimeError("network device proof is unavailable")
    names = {
...
def validate_runtime_lock(repo_root: Path, launch: dict[str, Any], torch: Any) -> dict[str, Any]:
    ...
    if not isinstance(lock, dict) or set(lock) != expected_keys or lock["schema"] != "gemma3-fineweb-edu-replication-h100-v9-runtime-lock" or lock["state_slice"] != STATE_SLICE:
    ...
    if lock["dtype"] != "bfloat16" or lock["gpu_required"] != "NVIDIA H100" or lock["network_policy"] != "network-none-v9" or lock["package_install_at_runtime"] is not False:
...
class RecirculationConfig:
    ...
    def validate(self, layer_count: int) -> None:
        ...
        if (
            ...
            or abs(self.beta - (1 - self.alpha)) > 1e-6
            or self.epsilon <= 0
        ):
...
def evaluate_windows(model: Any, tokenizer: Any, windows: Sequence[Window], config: RecirculationConfig | None,
    ...
    return {
        ...
        "mean_nll": mean_nll,
        "perplexity": Decimal(str(round(math.exp(float(mean_nll)), 9))),
...
def bootstrap_mean_ci(deltas: Sequence[Decimal]) -> dict[str, Any]:
    ...
    values = [Decimal(str(value)) for value in deltas]
    n = len(values)
...
def decision(bootstrap: dict[str, Any]) -> str:
    mean, upper = Decimal(str(bootstrap["mean_delta"])), Decimal(str(bootstrap["upper"]))
    return "ReplicationCandidate" if mean < 0 and upper < 0 else "NoCandidate"


def format_usd(val: Any) -> str:
    return format(Decimal(str(val)).quantize(Decimal("0.01")), ".2f")
...
def run(model_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, result_root: Path, launch_manifest: Path, repo_root: Path | None = None, tokenizer: Any | None = None) -> dict[str, Any]:
    ...
    for field, actual in expected_custody_paths.items():
        if actual.expanduser().resolve() != Path(launch[field]).expanduser().resolve():
            raise RuntimeError(f"{field} is not the exact launch-manifest custody path")
    ...
    if provider_receipt_path.is_symlink() or provider_receipt_path.stat().st_mode & 0o222:
        raise RuntimeError("provider receipt must be a regular file")
    from experiments.continual_learning import validate_gemma3_fineweb_edu_replication_h100_v9 as validator
    ...
    with network_block():
        ...
        for source_layer, destination_layer in CANDIDATE_PAIRS:
            ...
            zero_raw[pair] = {"metrics": metrics, "max_abs_nll_delta": max(abs(Decimal(str(metrics["rows"][i]["nll"])) - Decimal(str(assessment_baseline_raw["rows"][i]["nll"]))) for i in range(EXPECTED_WINDOWS))}
    ...
    def _to_float_metrics(m: dict[str, Any]) -> dict[str, Any]:
        return {**m, "mean_nll": float(m["mean_nll"]), "perplexity": float(m["perplexity"])}

    fit_baseline = aggregate_metrics(_to_float_metrics(fit_baseline_raw), fit)
    candidates = [{"config": item["config"], "metrics": aggregate_metrics(_to_float_metrics(item["metrics"]), fit)} for item in candidates_raw]
    assessment_baseline = aggregate_metrics(_to_float_metrics(assessment_baseline_raw), assessment)
    assessment_selected = aggregate_metrics(_to_float_metrics(assessment_selected_raw), assessment)
    temperature_baseline = aggregate_metrics(_to_float_metrics(temperature_baseline_raw), assessment)
    temperature_selected = aggregate_metrics(_to_float_metrics(temperature_selected_raw), assessment)
    repeat = aggregate_metrics(_to_float_metrics(repeat_raw), assessment)
    zero = {
        pair: {"metrics": aggregate_metrics(_to_float_metrics(item["metrics"]), assessment), "max_abs_nll_delta": float(item["max_abs_nll_delta"])}
    ...
    for window in assessment:
        ...
        ledger_rows.append({
            "document_id": window.document_id,
            "baseline_nll": after["nll"], # Logic Error in line: actually should match assessment_selected
            "selected_nll": after["nll"],
            "baseline_nll": before["nll"],
            "delta_selected_minus_baseline": (after["nll"] / (WINDOW_TOKENS - 1)) - (before["nll"] / (WINDOW_TOKENS - 1)),
        })
    ...
    for pair, candidate in zip(CANDIDATE_PAIRS, candidates_raw, strict=True):
        ...
        reach.append({"source_layer": pair[0], "destination_layer": pair[1], "max_abs_fit_nll_delta": float(maximum), "reached": maximum != Decimal("0")})
    if not all(item["reached"] for item in reach):
        raise RuntimeError("nonzero intervention reach failed for a candidate")
    parity = max(Decimal(str(item["max_abs_nll_delta"])) for item in zero.values())
    if parity > Decimal(str(PARITY_TOLERANCE)):
        raise RuntimeError("zero-alpha identity failed")
    if _to_float_metrics(repeat_raw) != _to_float_metrics(assessment_selected_raw):
    ...
    try:
        ...
        bootstrap_raw = bootstrap_mean_ci([row["delta_selected_minus_baseline"] for row in ledger_rows])
        bootstrap = {**bootstrap_raw, "mean_delta": float(bootstrap_raw["mean_delta"]), "lower": float(bootstrap_raw["lower"]), "upper": float(bootstrap_raw["upper"])}
        elapsed = time.monotonic() - started
        result = {
            "schema": SCHEMA,
            "state_slice": STATE_SLICE,
            "claim_ceiling": "LocalDevelopmentGemma3FineWebEduReplicationH100V9",
            "launch_manifest_sha256": launch_manifest_sha256,
            "hard_usd_ceiling": format_usd(launch["hard_usd_ceiling"]),
            "estimated_max_total_usd": format_usd(launch["estimated_max_total_usd"]),
            ...
            "provider_charged_usd": format_usd(provider_receipt["charged_usd"]),
            ...
            "runtime": {
                **runtime,
                ...
                "dtype": "bfloat16",
                "network": "offline-process-block-v9",
            },
        ...
        try:
            ...
            with destination.open("x", encoding="utf-8") as handle:
                ...
                handle.write("\n")
            receipt = {"schema": "gemma3-fineweb-edu-replication-h100-v9-result-receipt", "state_slice": STATE_SLICE, "result_sha256": result["results_sha256"], "result_file_sha256": sha256_file(destination)}

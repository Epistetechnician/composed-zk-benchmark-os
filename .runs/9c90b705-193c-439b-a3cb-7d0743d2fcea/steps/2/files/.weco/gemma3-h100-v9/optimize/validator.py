...
def config(value: Any, expected_alpha: float, expected_beta: float, label: str) -> dict[str, Any]:
    ...
    if float(exact_decimal(value["alpha"], f"{label} alpha")) != expected_alpha or float(exact_decimal(value["beta"], f"{label} beta")) != expected_beta or float(exact_decimal(value["epsilon"], f"{label} epsilon")) != 1e-6:
...
def metric(value: Any, windows: dict[str, dict[str, Any]], expected_temperature: float, expected_config: dict[str, Any] | None, label: str) -> None:
    ...
    if float(exact_decimal(value["temperature"], f"{label} temperature")) != expected_temperature or value["evaluation_config"] != expected_config:
...
def validate_assessment_ledger(path: Path, result: dict[str, Any], assessment: dict[str, dict[str, Any]]) -> list[Decimal]:
    ...
    baseline_mean = (baseline_total / (WINDOW_COUNT * (WINDOW_TOKENS - 1))).quantize(Decimal("1e-9"))
    selected_mean = (selected_total / (WINDOW_COUNT * (WINDOW_TOKENS - 1))).quantize(Decimal("1e-9"))
...
def validate(result_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if result["candidate_pairs"] != [list(pair) for pair in CANDIDATE_PAIRS] or float(exact_decimal(result["fit_alpha"], "fit alpha")) != FIT_ALPHA or float(exact_decimal(result["fit_beta"], "fit beta")) != FIT_BETA or float(exact_decimal(result["evaluation_alpha"], "eval alpha")) != EVALUATION_ALPHA or float(exact_decimal(result["evaluation_beta"], "eval beta")) != EVALUATION_BETA or float(exact_decimal(result["temperature_control"], "temp control")) != TEMPERATURE_CONTROL or result["normalization"] != "source_l2_norm_to_destination_l2_norm":
    ...
    for source_layer, destination_layer in CANDIDATE_PAIRS:
        ...
        if exact_decimal(entry["max_abs_nll_delta"], "zero-alpha delta") > Decimal(str(PARITY_TOLERANCE)):
    ...
    try:
        ...
        if result["bootstrap"] != expected_bootstrap or result["decision"] != ("ReplicationCandidate" if exact_decimal(expected_bootstrap["mean_delta"], "boot mean") < 0 and exact_decimal(expected_bootstrap["upper"], "boot upper") < 0 else "NoCandidate"):
            raise ValueError("bootstrap or decision mismatch")
    finally:
        if ledger_path.exists():
            ledger_path.unlink()
    reach = result["qualification"]
    if not isinstance(reach, dict) or set(reach) != {"nonzero_intervention_reach", "reach_evidence", "zero_alpha_identity_passed", "parity_tolerance"} or reach["nonzero_intervention_reach"] is not True or reach["zero_alpha_identity_passed"] is not True or float(exact_decimal(reach["parity_tolerance"], "parity tolerance")) != PARITY_TOLERANCE:

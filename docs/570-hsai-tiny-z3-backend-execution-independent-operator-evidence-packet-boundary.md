...
State slice: `Phase 570 HSAI tiny Z3 backend execution independent operator evidence packet boundary`.

Phase 570 defines the docs-first packet boundary for the independent-operator
evidence materials required by Phase 569. It does not create a packet, import
external results, accept independent external reproduction, or advance the
accepted-evidence path.

## Current Input

The only allowed source is one exact Phase 569 independent-reproduction
...
That record must bind the exact Phase 567 policy-resolution metadata record,
Phase 565 accepted-result eligibility metadata, Phase 563 review, Phase 561
quarantined candidate, Phase 559 capture, Phase 557 handoff packet, Phase 555
manual handoff, and inherited Phase
...
2. `operator_statement`: a signed or digest-bound declaration naming the exact
   Phase 557 handoff packet, command/procedure, timestamp, and nonclaim set.
...
5. `redaction_report`: proof that no secrets, credentials, raw provider
   bodies, undeclared raw logs, or environment secrets are retained.
6. `replay_correspondence`: binding from the reproduced run to the same source
   obligation, command descriptor, packet digest, and expected solver result.
...
- Phase 569 source requirement metadata;
- Phase 567/565/563/561/559/557/555 source bindings;
...
- `IndependentOperatorEvidencePacketRejected`;
- `IndependentOperatorEvidencePacketQuarantined`;
- `IndependentOperatorEvidencePacketReadyForImportCandidate`.
...
- the Phase 569 source record is not exact;
- the Phase 569 classification is not
  `IndependentReproductionEvidenceBlocked`;
- any Phase 567/565/563/561/559/557/555 or inherited digest binding drifts;
...
- the operator statement does not name the exact Phase 557 handoff packet;
...
- captured output contains raw stdout, raw stderr, credentials, secrets, raw
  provider bodies, or undeclared logs;
- the redaction report is missing or conflicts with the captured-output
  summary;
- replay/correspondence targets a different obligation, command, source
  digest, or expected result;
...
A future Phase 571 may implement local packet metadata only if it:

- accepts exactly one Phase 569 requirement metadata record;
- validates all Phase 569, Phase 567, Phase 565, Phase 563, Phase 561, Phase
  559, Phase 557, Phase 555, and inherited digest bindings;
- records `IndependentOperatorEvidencePacketMissing` under the current
  evidence state;
...
- adds focused tests for successful missing-packet metadata, Phase 569 drift
  rejection, packet-role drift rejection, and promotion rejection.

## Meaning

Phase 570 moves the path forward by naming the independent-operator evidence
packet shape that would be needed before the external operator path can
advance. It still does not make independent external reproduction true.

The correct statement is:

```text
HSAI has local independent-reproduction requirement metadata and a documented
independent-operator evidence packet boundary.
```

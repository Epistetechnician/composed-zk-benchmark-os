# Gemma3 FineWeb-Edu replication V28 protocol

State slice: `continual-learning-gemma3-fineweb-edu-replication-v28`.

V28 is a fresh, offline local replication attempt for the one-additional-
...
`LocalDevelopmentGemma3FineWebEduReplicationV28`.
...
V28 is a new identity: it pins every extant historical artifact and
...
- Prior pilot rows are `[0,2048)` per shard; discarded V14 rows are
  `[2048,18432)`; V28 uses only `[18432,34816)` per shard, yielding 16,384
  normalized rows per source split. Validators rederive every row and reject
  duplicate or prior document IDs.
- V28 source, corpus, and result roots, raw root, and prior root are exact
...
- The canonical cached checkpoint is never loaded by pathname after custody
  inspection. After review, the runner materializes a byte-identical V28 model
  snapshot at `/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v28-model-snapshot`
  ...
  validates the review receipt before it constructs a tokenizer. Any failed
  snapshot publication first restores deletion permissions and then removes
  both read-only files and directories; cleanup failure aborts the phase.

## Gates

1. An independent reviewer reads exactly the seven files in the V28 packet,
...
publication failure discards staging and closes V28 without tuning.

# Gemma3 FineWeb-Edu replication V25 protocol

State slice: `continual-learning-gemma3-fineweb-edu-replication-v25`.

V25 is a fresh, offline local replication attempt for the one-additional-
...
`LocalDevelopmentGemma3FineWebEduReplicationV25`.
...
validator did not reject a true evidence-ledger mutation flag. V25 is a new identity: it pins every extant historical artifact and
...
- Prior pilot rows are `[0,2048)` per shard; discarded V14 rows are
  `[2048,18432)`; V25 uses only `[18432,34816)` per shard, yielding 16,384
  normalized rows per source split. Validators rederive every row and reject
  duplicate or prior document IDs.
- V25 source, corpus, and result roots, raw root, and prior root are exact
...
- The canonical cached checkpoint is never loaded by pathname after custody
  inspection. After review, the runner materializes a byte-identical V25 model
  snapshot at `/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v25-model-snapshot`
...
1. An independent reviewer reads exactly the seven files in the V25 packet,
...
2. The contract pins every extant V1–V24 protocol, packet, acceptance,
...
publication failure discards staging and closes V25 without tuning.

# Gemma3 FineWeb-Edu replication on H100 — V7 protocol
...
rejected. V6 was independently rejected for budget-precision and custody-
binding defects and is terminally closed. Their scientific data, model
executions, effects, results, and receipts are not V7 inputs. V7 has a fresh
...
manifest digest are frozen in an external `gemma3-model-manifest-v7` bundle.
...
CUDA 12.4, bfloat16, one NVIDIA H100, and `network-none-v7`.
...
`LocalDevelopmentGemma3FineWebEduReplicationH100V7`. A candidate is not a
...
failure closes V7 as `NoCandidate` without adaptive retry.

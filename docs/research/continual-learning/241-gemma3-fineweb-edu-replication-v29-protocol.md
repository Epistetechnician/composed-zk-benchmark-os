# Gemma3 FineWeb-Edu replication V29 protocol

State slice: `continual-learning-gemma3-fineweb-edu-replication-v29`.

V29 is a fresh, offline local replication attempt for the one-additional-
...
`LocalDevelopmentGemma3FineWebEduReplicationV29`.
...
stable-read bound. V24 was independently rejected because hashing and
Parquet/model path loads were not descriptor-bound and the result validator
did not reject a true evidence-ledger mutation flag. V25 was independently
rejected because the validator corpus CLI could load before review validation
and an existing model snapshot could be writable. V27 was independently
rejected because snapshot rollback could leave read-only failed trees.
V28 was independently rejected because its review packet named stale V27
paths as the V28 protocol and packet and omitted V27 from required history
verification. V29 is a new identity that corrects those packet-bound scope
errors and pins the V28 rejection as historical evidence. No prior scientific
...
- Prior pilot rows are `[0,2048)` per shard; discarded V14 rows are
  `[2048,18432)`; V29 uses only `[18432,34816)` per shard, yielding 16,384
  normalized rows per source split. Validators rederive every row and reject
  duplicate or prior document IDs.
- V29 source, corpus, and result roots, raw root, and prior root are exact
...
- The canonical cached checkpoint is never loaded by pathname after custody
  inspection. After review, the runner materializes a byte-identical V29 model
  snapshot at `/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v29-model-snapshot`
  ...
  or any writable file/directory mode aborts the phase. Any failed snapshot
  publication first restores deletion permissions and then removes both
  read-only files and directories; cleanup failure aborts the phase.

## Gates

1. An independent reviewer reads exactly the seven files in the V29 packet,
...
2. The contract pins every extant V1-V28 protocol, packet, acceptance,
...
publication failure discards staging and closes V29 without tuning.

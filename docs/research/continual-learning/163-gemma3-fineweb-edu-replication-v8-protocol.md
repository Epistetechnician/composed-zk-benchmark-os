# Gemma3 FineWeb-Edu replication V8 protocol

State slice: `continual-learning-gemma3-fineweb-edu-replication-v8`.

V8 is a fresh reviewed protocol after V7's first execution gate exposed a
manifest-pin interpretation defect. V7's protocol, packet, implementation,
and `ACCEPT` receipt remain preserved as historical records; V7 produced no
source, corpus, result, or scientific effect. V8 must receive a new clean
...
`LocalDevelopmentGemma3FineWebEduReplicationV8`.
...
- V1 through V6 rejection history and the complete V7 reviewed protocol,
  packet, and acceptance receipt are pinned as historical records. No prior
  result is a V8 scientific input.
...
2. The packer revalidates the receipt, history, raw Parquet, prior pilot, and
   ...
   independently validating that field's self-digest; it is not confused with
   the JSON file's outer filesystem digest.
...
provider calls, production traffic, or claim above the V8 local ceiling.

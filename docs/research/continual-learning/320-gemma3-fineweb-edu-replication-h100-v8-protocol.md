# Gemma3 FineWeb-Edu replication on H100 — V8 protocol
...
rejected. V6 and V7 were independently rejected for implementation and
custody-binding defects and are terminally closed. Their scientific data, model
executions, effects, results, and receipts are not V8 inputs. V8 has a fresh
source interval, corpus identity,
...
6. Create one launch manifest binding code, runtime, model, data, image,
 ...
 `USD 69.00`. Decimal arithmetic must satisfy
   ...
   The external code bundle is a regular immutable file, and both preflight
   and independent validation recompute `code_bundle_sha256` from its bytes.
...
manifest digest are frozen in an external `gemma3-model-manifest-v8` bundle.
...
CUDA 12.4, bfloat16, one NVIDIA H100, and `network-none-v8`.
...
`LocalDevelopmentGemma3FineWebEduReplicationH100V8`. A candidate is not a
...
failure closes V8 as `NoCandidate` without adaptive retry.

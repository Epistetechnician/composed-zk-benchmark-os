# Gemma3 FineWeb-Edu replication V31 protocol

State slice: `continual-learning-gemma3-fineweb-edu-replication-v31`.

V31 is a fresh, offline local replication attempt for the one-additional-
...
`LocalDevelopmentGemma3FineWebEduReplicationV31`.
...
because the independent response was malformed and its seven-file scope could
not establish historical byte custody. V30 was invalidated because a reviewed
byte changed during its review. V31 is a new identity. It pins every prior
protocol, packet, acceptance, rejection, failure, and invalidation record in a
finite review allowlist. Rejected and invalidated artifacts are historical
only; none is approval or scientific evidence.
...
- Prior pilot rows are `[0,2048)` per shard; discarded V14 rows are
  `[2048,18432)`; V31 uses only `[18432,34816)` per shard, yielding 16,384
  normalized rows per source split. Validators rederive every row and reject
  duplicate or prior document IDs.
- V31 source, corpus, and result roots, raw root, and prior root are exact
...
- The canonical cached checkpoint is never loaded by pathname after custody
  inspection. After review, the runner materializes a byte-identical V31 model
  snapshot at `/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v31-model-snapshot`
...
1. An independent reviewer reads exactly the seven current V31 files and
   exactly the finite historical verification files listed in the V31 packet,
   accesses no model or external data, recomputes current packet-bound digests,
...
2. The contract pins every extant V1-V30 protocol, packet, acceptance,
   rejection, failure, and invalidation record. V2-V6 have no canonical
   rejection receipt; their absence is explicitly checked and no receipt is
   invented. The reviewer directly verifies each listed historical file's
   bytes against the contract hash.
...
publication failure discards staging and closes V31 without tuning.

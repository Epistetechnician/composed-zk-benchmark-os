# Gemma3 FineWeb-Edu replication V22 protocol

State slice: `continual-learning-gemma3-fineweb-edu-replication-v22`.

V22 is a fresh, offline local replication attempt for the one-additional-
...
`LocalDevelopmentGemma3FineWebEduReplicationV22`.
...
history. V22 is a new identity: it pins every extant historical artifact and
...
- Prior pilot rows are `[0,2048)` per shard; discarded V14 rows are
  `[2048,18432)`; V22 uses only `[18432,34816)` per shard, yielding 16,384
  normalized rows per source split. Validators rederive every row and reject
  duplicate or prior document IDs.
- V22 source, corpus, and result roots, raw root, and prior root are exact
  external PrimaryED paths. Symlinks, alternate roots, and alternate model
  paths are rejected.

## Gates

1. An independent reviewer reads exactly the seven files in the V22 packet,
...
publication failure discards staging and closes V22 without tuning.

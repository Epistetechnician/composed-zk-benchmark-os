# Gemma3 FineWeb-Edu replication V13 protocol

State slice: `continual-learning-gemma3-fineweb-edu-replication-v13`.

V13 is a fresh protocol after V12 was rejected because its historical custody
set omitted the V1 protocol and packet and did not pin the absent early-review
status as an explicit boundary. V11 reached its approved pre-effect execution
gate but stopped in the in-memory parameter-custody digest: NumPy cannot bridge
the MLX BF16 buffer exposed by `lm_head/weight`. V7 remains independently
accepted but exposed a prior-manifest pin bug at execution. V8 corrected that
bug but was rejected for incomplete process-launch denial. V9 closed that
denial but was rejected because enriched review metadata was passed to an exact
snapshot comparator. V10 corrected the comparator but was rejected because
corpus windows were not independently rederived as source-row prefixes and the
assessment bootstrap accepted arbitrary row order. V11 closed those gates but
had no scientific effect or result because the digest representation failed
before effects. V12's rejection is now preserved as a canonical record. V13
pins every extant V1-V12 protocol, packet, acceptance, rejection, and
pre-effect failure record. No undocumented V2-V6 rejection receipt is
invented; their extant protocol/packet records are the complete custody
records available. None is a V13 scientific input. V13 requires a new clean
independent `ACCEPT` before execution.
...
`LocalDevelopmentGemma3FineWebEduReplicationV13`.
...
- Every extant V1 through V12 protocol, packet, acceptance, rejection, and
  pre-effect failure record is pinned by exact SHA-256 in the V13 contract.
  V2-V6 have no canonical rejection receipt files in the worktree; V13 does
  not synthesize them or treat absent records as approvals. All pinned records
  are custody history only; no prior result is a V13 scientific input.
...
4. The native macOS sandbox is mandatory. The Python guard denies DNS, socket
   ...
   runtime checks.
   The parameter digest canonicalizes MLX BF16 leaves through an exact
   float32 representation while retaining the original dtype tag, so custody
   checks remain executable on this runtime without changing model weights.
...
provider calls, production traffic, or claim above the V13 local ceiling.

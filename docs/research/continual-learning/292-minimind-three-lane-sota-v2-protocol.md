# MiniMind three-lane continual-learning comparison V2
...
The published-benchmark lane is not executed by V2. The fixture corpus is
...
records. Consequently V2 cannot claim benchmark accuracy, domain-CPT BPB,
...
macro held-out accuracy in a future full campaign. The V2 fixture exercises
...
a fresh real local-domain corpus in the future published campaign. V2 uses
...
from the held-out evaluation. V2 exercises the state/accounting schema with
...
MiniMind implementation is frozen in V2. OLieRA, ASO-LoRA, DAS,
...
6. The model contract must pass independent validation. Any failed gate closes
   V2; no adaptive retuning, method shopping, or assessment reinterpretation
   is allowed.

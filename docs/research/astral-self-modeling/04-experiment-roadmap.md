...
| V28R5 | `LastTokenProjectionPreflightDocsFirstOnly` | A candidate-neutral optimized scorer is preregistered to project vocabulary logits only from the last hidden token and compare batch 8/64 against the sealed V28R4 reference; implementation and model access remain unauthorized |

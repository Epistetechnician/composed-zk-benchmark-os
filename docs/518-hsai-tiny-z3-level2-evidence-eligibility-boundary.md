...
State slice: `Phase 518 HSAI tiny Z3 Level2 evidence eligibility boundary`.

Phase 518 defines the docs-first boundary for future Level2 eligibility
metadata over the Phase 517 score-axis eligibility record:
...
Phase 517 metadata records that every score axis remains unpopulated and that
the current package is `score_axes_blocked_local_only`. A future Level2
eligibility metadata record may explain the Level2 blocker state, but it must
not convert eligibility into accepted evidence or score-axis authority.
...
A future HSAI tiny-Z3 Level2 eligibility record may classify one Phase 517
record as:
...
evidence, or external audit evidence.
...
- one Phase 517 eligibility input digest;
- the Phase 517 digest-binding map digest;
- the Phase 517 id-binding map digest;
- the Phase 517 label-binding map digest;
...
- the Phase 515 package input digest;
...
- the Phase 517 record is not exact;
- the Phase 517 classification is not `score_axes_blocked_local_only`;
...
- external artifact capture, replay manifest review, or provenance is asserted
  without explicit digest bindings;
...
This boundary is not a new backend run. Phase 404 already performed a
quarantined fixed local Z3 execution for one tiny digest-binding property, and
Phase 405 already materialized/read back that output bundle. Those artifacts
remain local and nonpromoted.

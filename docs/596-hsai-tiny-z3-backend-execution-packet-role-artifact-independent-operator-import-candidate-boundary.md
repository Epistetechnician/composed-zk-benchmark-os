...
State slice: `Phase 596 HSAI tiny Z3 backend execution packet role artifact independent-operator import-candidate boundary`.

Phase 596 defines the docs-first boundary for future import-candidate metadata
over a Phase 595 quarantined local packet-role artifact independent-operator
bundle. It does not implement import-candidate code, import external results,
mutate the accepted Evidence Ledger, accept independent external reproduction,
or advance to Level2+ evidence.

## Current Input

The only allowed source is one exact Phase 595 packet-role artifact
independent-operator output plumbing readback whose manifest classification is:
...
- the exact Phase 591 packet-role artifact independent-operator materialization metadata;
...
- Phase 583 accepted-result eligibility metadata;
- Phase 581 import-review metadata;
- Phase 579 quarantined packet-role artifact import-candidate metadata;
- Phase 577 packet-role artifact output plumbing readback;
- Phase 575 packet-role artifact output metadata;
- Phase 573 materialization metadata;
...
- a deterministic candidate digest over the Phase 595 readback manifest;
- declared artifact references for the local independent-operator packet-role
  bundle manifest;
- quarantine metadata;
...
must not treat the local bundle as accepted external result evidence.

## Required Future Validation

A future implementation must fail closed unless:

- the Phase 595 manifest schema and state slice are exact;
- the Phase 595 classification is
  `PacketRoleArtifactIndependentOperatorOutputQuarantinedLocalBundle`;
- every declared JSON file and `.sha256` sidecar was read back and validated;
- the Phase 595 readback digest recomputes from exact manifest fields;
- no undeclared files, symlinks, stale sidecars, secrets, raw logs, raw provider
  ...
  artifacts, score-axis artifacts, benchmark artifacts, or production artifacts
  are present;
- the Phase 593 source metadata remains exact and unpromoted;
- all inherited Phase 591/589/587/585/583/581/579/577/575/573/571/569/567
  /565/563/561/559/557/555 and backend-execution digest bindings are nonzero
  and unchanged;
...
- `PacketRoleArtifactIndependentOperatorImportCandidateRejected`;
- `PacketRoleArtifactIndependentOperatorImportCandidateQuarantinedLocalBundle`;
- `PacketRoleArtifactIndependentOperatorImportCandidateReadyForReviewBoundary`.
...
`PacketRoleArtifactIndependentOperatorImportCandidateReadyForReviewBoundary`
requires a later review boundary. Accepted evidence requires a later reviewed
acceptance policy and an accepted-ledger append path.

## Forbidden In This Phase

Phase 596 does not permit:
...
## Future Phase 597 Exit Criteria

A future Phase 597 may implement import-candidate metadata only if it:

- accepts exactly one Phase 595 readback;
- validates all Phase 595, Phase 593, Phase 591, Phase 589, Phase 587, Phase
  585, Phase 583, Phase 581, Phase 579, Phase 577, Phase 575, Phase 573, Phase
...
- rejects any malformed or promoted Phase 595 manifest;
...
- does not run Lean, COBALT, Rust-to-Lean, another SMT/Z3 backend, or a
  benchmark;
- adds focused tests for successful quarantined candidate metadata, Phase 595
...
Phase 596 moves the path forward by defining how a local packet-role artifact
independent-operator bundle can later become a quarantined import candidate.

The correct statement is:

```text
HSAI has a local packet-role artifact independent-operator bundle and a
documented boundary for future quarantined import-candidate metadata.
```

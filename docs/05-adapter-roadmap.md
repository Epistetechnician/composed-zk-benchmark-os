...
| Future Phase J | Reviewed proposal acceptance policy | Implemented in `zkbench-core` as acceptance policy, escalation guard, review decisions, and evidence-record candidates without ledger append. | Proposal policy tests and claim-boundary tests. |
| Phase K | gnark recursion-envelope adapter preparation | Implemented in `zkbench-core` as inert manifest, envelope plan, and evidence mapping scoped to `recursive_loop_envelope`. | Manifest/envelope round-trip, inertness, and claim-boundary tests. |
| Phase L | Narrow zkML adapter preparation | Implemented in `zkbench-core` as inert manifest, workload plan, and evidence mapping scoped to `zkml_control_flow_mixed`. | Manifest/workload round-trip, inertness, and claim-boundary tests. |
...
- Do not enable live zk-Harness execution until reviewed proposal acceptance policy, supersession, and future append eligibility exist and are tested.

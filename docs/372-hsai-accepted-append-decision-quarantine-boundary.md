...
State slice: `Phase 372 HSAI accepted-append decision quarantine boundary`.

Phase 372 defines a docs-first boundary for future local quarantine metadata
over one Phase 371 accepted-append decision blocker review. This boundary does
not implement quarantine metadata, make an accepted append decision, mutate the
...
The future quarantine may record that one Phase 371 blocker review remains
...
A future accepted-append decision quarantine input must bind:
...
- Phase 371 blocker review state is promoted or drifted;
...
HSAI can locally quarantine one accepted-append decision blocker review away
from the accepted append path.

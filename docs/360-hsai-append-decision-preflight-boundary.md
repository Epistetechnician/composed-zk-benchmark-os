...
State slice: `Phase 360 HSAI append-decision preflight boundary`.

Phase 360 defines a docs-first boundary for future local append-decision
preflight metadata over one Phase 359 proposal candidate review. This boundary
does not implement preflight metadata, mutate the accepted Evidence Ledger,
...
The future append-decision preflight may classify one Phase 359 review for
local readiness to remain blocked before any accepted append decision. It may
only review digest consistency, review-label consistency, append-blocker
...
A future append-decision preflight input must bind:
...
- Phase 359 review state is promoted or drifted;
...
## Phase 361 Implementation Exit Criteria

Phase 361 may implement local append-decision preflight metadata only if it:

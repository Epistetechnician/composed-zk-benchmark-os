...
State slice: `Phase 522 HSAI tiny Z3 external result import candidate boundary`.

Phase 522 defines the docs-first boundary for future external-result import
candidate metadata over the Phase 521 external-reproduction provenance record:

```text
Phase 521 external-reproduction provenance metadata
...
create accepted evidence, create Level2+ evidence, or populate score axes.
...
solver APIs, proof-assistant APIs, benchmark runners, score-axis writers,
accepted-ledger writers, or filesystem output writers.

## Future Metadata Meaning

A future HSAI tiny-Z3 external-result import candidate record may classify one
Phase 521 record as:
...
`zkbench-core`, but it must preserve the Phase 521 fact that independent
...
- one Phase 521 external-reproduction input digest;
- the Phase 521 digest-binding map digest;
- the Phase 521 id-binding map digest;
- the Phase 521 label-binding map digest;
- the Phase 521 classification `external_reproduction_blocked_no_independent_run`;
- the Phase 521 reproduction blocker digest;
- the Phase 521 reproduction nonpromotion digest;
- the Phase 521 external owner and contract digests;
...
- any Phase 521 external-reproduction input is marked present;
- any Phase 519/517/515 inherited nonpromotion invariant is violated;
...
candidate for the tiny-Z3 package while preserving that no independent external
...
## Phase 523 Implementation Exit Criteria

A future Phase 523 may implement local external-result import candidate
metadata only if it:
...
- validates one exact Phase 521 external-reproduction metadata record;
...
- rejects independent external reproduction, accepted formal evidence, Level2+
  evidence, populated score axes, proof/checker/solver authority, backend

# Phase 373 HSAI Accepted-Append Decision Quarantine Metadata Notes

State slice: `Phase 373 HSAI accepted-append decision quarantine metadata
implementation`.

Phase 373 implements deterministic pure-data quarantine metadata over one Phase
371 accepted-append decision blocker review. The quarantine stores only digests,
quarantine metadata, proposal metadata, the candidate disposition, the
candidate-review label, the blocker label, the blocker-review label, a bounded
quarantine label, explicit nonclaims, and nonpromotion flags. It does not make
an accepted append decision, write filesystem artifacts, mutate the accepted
...
- `GatewayFormalRealCommandLaneAcceptedAppendDecisionQuarantineInput`;
...
- `gateway_formal_real_command_lane_accepted_append_decision_quarantine_claim_boundary`;
- `gateway_formal_real_command_lane_accepted_append_decision_quarantine_required_nonclaims`;
- `build_gateway_formal_real_command_lane_accepted_append_decision_quarantine`;
- `validate_gateway_formal_real_command_lane_accepted_append_decision_quarantine_input`.

## Quarantine Labels

The implemented quarantine labels are:

- `accepted_append_decision_quarantined`;
- `accepted_ledger_mutation_quarantined`;
- `accepted_formal_evidence_quarantined`;
- `level2_evidence_quarantined`;
- `score_axis_population_quarantined`.
...
- Phase 371 blocker review digest or input digest drift;
- Phase 369, Phase 367, Phase 365, Phase 363, Phase 361, Phase 359, Phase 357,
  Phase 355, Phase 353, Phase 351, Phase 349, Phase 347, Phase 345, or Phase
  343 digest drift through the quarantine chain;
- promoted or drifted Phase 371 blocker review state;
...
The maximum claim after Phase 373 is:

HSAI can locally quarantine one accepted-append decision blocker review away from
the accepted append path.
...
Phase 373 adds tests that:

- build deterministic accepted-append decision quarantine metadata;
- verify the quarantine binds the Phase 371 blocker review digest;
- verify the quarantine binds the Phase 371 blocker review input digest;
- verify the quarantine binds the Phase 369 blocker digest;
- verify the quarantine binds the Phase 367 review digest;
- verify quarantine labels remain non-promotional;
- reject Phase 371 blocker review digest drift;

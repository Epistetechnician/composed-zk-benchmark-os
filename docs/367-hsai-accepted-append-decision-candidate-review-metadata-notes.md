# Phase 367 HSAI Accepted-Append Decision Candidate Review Metadata Notes

State slice: `Phase 367 HSAI accepted-append decision candidate review metadata
implementation`.

Phase 367 implements deterministic pure-data review metadata over one Phase 365
accepted-append decision candidate. The review stores only digests, reviewer
metadata, proposal metadata, the candidate disposition, a bounded review label,
...
- `GatewayFormalRealCommandLaneAcceptedAppendDecisionCandidateReviewInput`;
...
- `gateway_formal_real_command_lane_accepted_append_decision_candidate_review_claim_boundary`;
- `gateway_formal_real_command_lane_accepted_append_decision_candidate_review_required_nonclaims`;
- `build_gateway_formal_real_command_lane_accepted_append_decision_candidate_review`;
- `validate_gateway_formal_real_command_lane_accepted_append_decision_candidate_review_input`.

## Review Labels

The implemented review labels are:

- `decision_candidate_review_scope_acceptable`;
- `decision_candidate_review_rejected`;
...
- Phase 365 candidate digest or input digest drift;
- Phase 363, Phase 361, Phase 359, Phase 357, Phase 355, Phase 353, Phase 351,
  Phase 349, Phase 347, Phase 345, or Phase 343 digest drift through the
  candidate chain;
- promoted or drifted Phase 365 candidate state;
...
The maximum claim after Phase 367 is:

HSAI can locally review one accepted-append decision candidate while preserving
...
Phase 367 adds tests that:

- build deterministic accepted-append decision candidate review metadata;
- verify the review binds the Phase 365 candidate digest;
- verify the review binds the Phase 365 candidate input digest;
- verify the review binds the Phase 363 review digest;
- verify the review binds the Phase 361 preflight digest;
- verify blocking review labels remain non-promotional;
- reject Phase 365 candidate digest drift;
...
Phase 368 should define a docs-first boundary for accepted-append decision
blocker metadata before any accepted append decision is allowed. That boundary
must keep decision-blocker metadata separate from accepted formal evidence,

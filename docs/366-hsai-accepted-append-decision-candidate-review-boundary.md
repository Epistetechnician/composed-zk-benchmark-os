...
State slice: `Phase 366 HSAI accepted-append decision candidate review boundary`.

Phase 366 defines a docs-first boundary for future local review metadata over
one Phase 365 accepted-append decision candidate. This boundary does not
implement decision-candidate review metadata, mutate the accepted Evidence
...
The future accepted-append decision candidate review may classify one Phase 365
decision candidate for local review readiness while the real accepted append
path remains blocked. It may only review candidate digest consistency,
disposition consistency, append-blocker consistency, nonclaim completeness, and
promotion safety.
...
A future accepted-append decision candidate review input must bind:
...
- Phase 365 candidate state is promoted or drifted;
- candidate disposition or review label is outside its bounded label set;

...
PROTOCOL_PATH = Path(
    "docs/research/"
    "continual-learning/264-gemma3-fineweb-edu-replication-h100-v4-protocol.md"
)
PACKET_PATH = Path(
    "docs/research/"
    "continual-learning/265-gemma3-fineweb-edu-replication-h100-v4-review-packet.md"
)
SCHEMA = "gemma3-fineweb-edu-replication-h100-v4-launch-manifest"
EXPECTED_KEYS = {
    "schema",
    "state_slice",
    "provider",
    ...
    "provider_attestation_public_key",
    "container_image",
    "container_digest",
    "cuda_driver_version",
...
def validate_implementation_manifest(
    ...
    if (
        manifest["schema"]
        != "gemma3-fineweb-edu-replication-h100-v4-implementation"
...
def validate_review_receipt(
    ...
    if receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v4-independent-review" or receipt["state_slice"] != STATE_SLICE or receipt["review_decision"] != "ACCEPT" or receipt["effects_run"] is not False:
...
def validate_launch_manifest(
    ...
    if not math.isclose(rate * minutes, estimate, rel_tol=0.0, abs_tol=1e-9) or estimate > ceiling:
    ...
    if re.fullmatch(r"[A-Za-z0-9._:-]+", manifest["provider_attestation_key_id"]) is None:
        raise ValueError("provider_attestation_key_id is invalid")
    if re.fullmatch(r"[A-Za-z0-9+/]{43}=", manifest["provider_attestation_public_key"]) is None:
        raise ValueError("provider_attestation_public_key must be base64 Ed25519 bytes")
    if manifest["assessment_enabled"] is not True:
        raise ValueError("H100 replication must include locked assessment")
    if manifest["runner_entrypoint"] != "run_h100_v4.sh":
        raise ValueError("runner entrypoint is not the reviewed provider entrypoint")
    if manifest["launch_command"] != "./run_h100_v4.sh":
    ...
    if manifest["network_lock"] != "network-none-v4":
    ...
    for field in ("code_bundle_path", "model_bundle_path", "data_bundle_path", "source_bundle_path", "raw_bundle_path"):

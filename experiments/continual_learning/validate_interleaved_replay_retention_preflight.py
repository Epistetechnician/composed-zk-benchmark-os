#!/usr/bin/env python3
"""Independent validator for the V15 interleaved-replay preflight."""
...
from experiments.continual_learning.validate_residue_only_codebook_benchmark import LABELS, expected_prompt, model_accuracy  # noqa: E402
...
def token(row: dict[str, str]) -> str:
    return row["prompt"].split("Task token: ", 1)[1].split(".", 1)[0]


def is_round_robin(rows: list[dict[str, str]]) -> bool:
    grouped: dict[str, list[dict[str, str]]] = {}
    for row in rows:
        grouped.setdefault(token(row), []).append(row)
    expected_tokens = []
    remaining = {key: len(value) for key, value in grouped.items()}
    keys = sorted(grouped)
    while any(remaining.values()):
        for key in keys:
            if remaining[key]:
                ...
                remaining[key] -= 1
    return [token(row) for row in rows] == expected_tokens
...
def validate(root: Path) -> dict:
    ...
    if result["retention_comparison_run"] is not True or result["tasks"] != tasks:
        raise ValueError("retention/task manifest drift")
    fixed = {
    ...
    audits = {strategy: json.loads((root / "audit" / f"{strategy}.json").read_text()) for strategy in ("naive_sequential_lora", "replay_lora", "task_adapter_bank")}
    ...
    paths = sorted((root / "data").glob("*/step-*/train.jsonl")) + sorted((root / "data" / "task_adapter_bank").glob("task-*/train.jsonl"))
    if len(paths) != 12:
    ...
    for path in paths:
        ...
        for row in rows:
            ...
            if "Compose " in row["prompt"]:
                raise ValueError(f"raw-pair prompt leakage: {path}")
        if path.parts[-2] == "replay_lora" and not is_round_robin(rows):
            raise ValueError(f"interleaved replay schedule drift: {path}")
    ...
    if result["candidate_gates"] != candidate_gates or result["candidate_eligible"] != (gates["solvability_floor"] and gates["replay_retention_above_naive"]):
        raise ValueError("candidate gate drift")
    return {"valid": True, "claim_ceiling": result["claim_ceiling"], "manifest_sha256": result["manifest_sha256"], "candidate_gates": gates, "candidate_eligible": result["candidate_eligible"]}

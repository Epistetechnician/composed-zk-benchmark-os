#!/usr/bin/env python3
"""Review-gated V20 corpus stage and offline Gemma3 runner.

State slice: continual-learning-gemma3-fineweb-edu-replication-v20.
No model or scientific effect is reachable until the V20 receipt validates.
"""
...
from experiments.continual_learning import (
    validate_gemma3_fineweb_edu_replication_v20 as v,
)

VALIDATOR = "experiments.continual_learning.validate_gemma3_fineweb_edu_replication_v20"
...
def _sandbox() -> None:
    if sys.platform != "darwin" or c.native_network_denied():
        return
    if os.environ.get("V20_RUN_REEXEC") == "1":
        raise RuntimeError("V20 native network denial could not be established")
    executable = shutil.which("sandbox-exec")
    if executable is None:
        raise RuntimeError("sandbox-exec is unavailable")
    env = os.environ.copy()
    env["V20_RUN_REEXEC"] = "1"
    os.execvpe(
        executable,
        [
            executable,
...
def parity(model: Any, windows: Sequence[v.Window]) -> dict[str, Any]:
    ...
    if len(checks) != 128 or not all(item["passed"] is True for item in checks):
        raise RuntimeError("V20 zero-alpha parity failed")
    return {
...
def _run_effects(
    ...
    if (
        ...
        raise RuntimeError("V20 input changed before effects")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V20 architecture mismatch")
        ...
        if (
            ...
            raise RuntimeError("V20 input changed during effects")
        ...
        if before_parameters != after_parameters or before_model != after_model:
            raise RuntimeError("V20 model custody changed")
    ...
    if not any(item["reached"] for item in reach):
        raise RuntimeError("V20 nonzero intervention reach failed")
    common = {
    ...
    config = {
        **common,
        ...
        "selection_policy": "first-64-eligible-1024-token-windows-per-disjoint-v20-source-split",
    }
...
def stage_corpus(
    ...
    source, final, model = (
        c.exact_path(source_root, c.SOURCE_ROOT, "V20 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V20 corpus"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V20 corpus exists: {final}")
    ...
    if _reviewed_input(source, None, raw_root, prior_root, model) != input_snapshot:
        raise RuntimeError("V20 inputs changed before tokenizer")
    ...
    try:
        ...
        for split, count in (("fit", 64), ("assessment", 64)):
            selected = []
            for row in v.jsonl(source / f"{split}/fineweb_edu.jsonl", f"V20 {split}"):
                ...
                if (
                    ...
                    raise ValueError("V20 tokenizer round trip failed")
            ...
            if len(selected) != count:
                raise ValueError(f"V20 {split} has {len(selected)} eligible windows")
        ...
        _assert_review(review_snapshot)
        c.exact_file_set(staging, _expected_corpus_files(), "V20 corpus staging")
        with c.network_block():
            v.audit_corpus(
                staging,
                source,
        ...
        output = c.snapshot_files(
            staging, _expected_corpus_files(), "V20 corpus staging"
        )
        c.publish_no_replace(
            ...
            lambda: (
                ...
                if _reviewed_input(source, None, raw_root, prior_root, model) == input_snapshot
                    ...
                    RuntimeError("V20 corpus inputs changed after publication")
                ),
            ),
        )
        return {
            "manifest": _json(final / "manifest.json", "V20 corpus manifest"),
            "snapshot": output,
        }
    except Exception:
        if staging.exists():
...
def run_campaign(
    ...
    final, source, corpus, raw, prior, model = (
        c.exact_path(result_root, c.RESULT_ROOT, "V20 result"),
        c.exact_path(source_root, c.SOURCE_ROOT, "V20 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V20 corpus"),
        c.exact_path(raw_root, c.RAW_ROOT, "V20 raw"),
        c.exact_path(prior_root, c.PRIOR_ROOT, "V20 prior"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V20 result exists: {final}")
    review_snapshot = _review(receipt)
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V20 inputs changed during tokenizer load")
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V20 inputs changed before effects")
    staging = Path(tempfile.mkdtemp(prefix=f".{final.name}.staging-", dir=final.parent))
    try:
        ...
        c.exact_file_set(staging, expected, "V20 result staging")
        _assert_review(review_snapshot)
        current = _reviewed_input(source, corpus, raw, prior, model)
        if current != input_snapshot:
            raise RuntimeError("V20 inputs changed immediately before validator")
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"V20 independent validator failed:\n{completed.stdout}\n{completed.stderr}"
            )
        ...
        if (
            ...
            raise RuntimeError("V20 post-validator custody check failed")
        ...
        c.publish_no_replace(
            ...
            lambda: (
                ...
                if _reviewed_input(source, corpus, raw, prior, model) == input_snapshot
                    ...
                    RuntimeError("V20 result inputs changed after publication")
                ),
            ),
        )

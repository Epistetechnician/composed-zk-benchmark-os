...
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-h100-v4-result"
...
LAUNCH_KEYS = {
    ...
    "provider_attestation_key_id", "provider_attestation_public_key",
...
REVIEWED_FILES = (
    "docs/research/continual-learning/264-gemma3-fineweb-edu-replication-h100-v4-protocol.md",
    "docs/research/continual-learning/265-gemma3-fineweb-edu-replication-h100-v4-review-packet.md",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v4_preflight.py",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v4.py",
    "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_h100_v4.py",
    "experiments/continual_learning/pack_gemma3_fineweb_edu_replication_h100_v4.py",
    "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_h100_v4_preflight.py",
    "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_h100_v4.py",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v4_provider/Dockerfile",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v4_provider/requirements.lock",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v4_provider/runtime-lock.json",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v4_provider/run_h100_v4.sh",
    "AGENTS.md",
    "docs/research/continual-learning/266-gemma3-fineweb-edu-replication-h100-v4-implementation-manifest.json",
)
IMPLEMENTATION_FILES = tuple(
    path for path in REVIEWED_FILES
    if path != "docs/research/continual-learning/266-gemma3-fineweb-edu-replication-h100-v4-implementation-manifest.json"
)
REVIEW_RECEIPT_KEYS = {
...
PROVIDER_RECEIPT_SCHEMA = "gemma3-fineweb-edu-replication-h100-v4-provider-receipt"
...
EXCLUSION_RANGES = (("prior-pilot", 0, 2_048), ("prior-v31", 2_048, 18_432), ("discarded", 18_432, 34_816), ("prior-h100-v1", 34_816, 51_200), ("prior-h100-v3", 51_200, FRESH_START))
...
def verify_provider_attestation(receipt: dict[str, Any], launch: dict[str, Any]) -> None:
    ...
    if not isinstance(attestation["signature"], str):
        raise ValueError("provider attestation signature is missing")
    try:
        public_key = base64.b64decode(launch["provider_attestation_public_key"], validate=True)
...
def source_bundle(raw_root: Path, source_root: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if set(manifest) != required or manifest["schema"] != "gemma3-fineweb-edu-replication-h100-v4-source" or manifest["state_slice"] != STATE_SLICE or manifest["dataset"] != {"repo": DATASET_REPO, "source": DATASET_SOURCE, "revision": DATASET_REVISION, "config": DATASET_CONFIG, "split": DATASET_SPLIT} or manifest["raw_root"] != "raw" or manifest["fresh_row_range"] != {"start": FRESH_START, "end_exclusive": FRESH_END, "count_per_shard": FRESH_END - FRESH_START} or manifest["excluded_ranges"] != [{"name": n, "start": s, "end_exclusive": e, "count_per_shard": e - s} for n, s, e in EXCLUSION_RANGES] or any(manifest[name] is not False for name in ("network_access", "training", "scientific_execution", "evidence_ledger_mutation")) or manifest["manifest_sha256"] != digest(manifest, "manifest_sha256"):
...
def validate_launch_binding(path: Path, repo_root: Path, result: dict[str, Any]) -> None:
    ...
    if set(launch) != LAUNCH_KEYS or launch.get("schema") != "gemma3-fineweb-edu-replication-h100-v4-launch-manifest" or launch.get("state_slice") != STATE_SLICE:
    ...
    try:
        if len(base64.b64decode(launch["provider_attestation_public_key"], validate=True)) != 32:
            raise ValueError("launch provider attestation public key length is invalid")
    except (ValueError, TypeError) as error:
        raise ValueError("launch provider attestation public key is invalid") from error
    ...
    if launch.get("runner_entrypoint") != "run_h100_v4.sh" or launch.get("launch_command") != "./run_h100_v4.sh" or launch.get("network_lock") != "network-none-v4" or launch.get("launch_command_sha256") != hashlib.sha256(b"./run_h100_v4.sh").hexdigest():
    ...
    protocol = repo_root / "docs/research/continual-learning/264-gemma3-fineweb-edu-replication-h100-v4-protocol.md"
    packet = repo_root / "docs/research/continual-learning/265-gemma3-fineweb-edu-replication-h100-v4-review-packet.md"
    ...
    if set(review) != REVIEW_RECEIPT_KEYS or review.get("schema") != "gemma3-fineweb-edu-replication-h100-v4-independent-review":
    ...
    if implementation.get("schema") != "gemma3-fineweb-edu-replication-h100-v4-implementation" or implementation.get("state_slice") != STATE_SLICE or [item.get("path") for item in implementation.get("files", [])] != list(IMPLEMENTATION_FILES):
...
def validate_pre_effect_bundle(raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    from experiments.continual_learning import gemma3_fineweb_edu_replication_h100_v4_preflight as preflight
...
def validate(result_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if set(receipt) != {"schema", "state_slice", "result_sha256", "result_file_sha256", "receipt_sha256"} or receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v4-result-receipt" or receipt["state_slice"] != STATE_SLICE or receipt["result_sha256"] != result["results_sha256"] or receipt["result_file_sha256"] != sha256_file(result_root / "result.json") or receipt["receipt_sha256"] != digest(receipt, "receipt_sha256"):
    ...
    expected_paths = {
        "launch_manifest_path": str(path.resolve()),
    ...
    if (
        ...
        or runtime["network"] != "offline-process-block-v4"

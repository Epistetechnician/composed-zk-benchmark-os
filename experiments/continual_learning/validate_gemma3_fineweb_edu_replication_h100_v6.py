...
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-h100-v6-result"
...
PROVIDER_RECEIPT_SCHEMA = "gemma3-fineweb-edu-replication-h100-v6-provider-receipt"
...
def validate_provider_receipt(path: Path, launch: dict[str, Any], launch_sha256: str) -> dict[str, Any]:
    ...
    estimate = finite(receipt["estimated_max_total_usd"], "provider estimated USD")
    ...
    if estimate != finite(launch["estimated_max_total_usd"], "launch estimated USD"):
        raise ValueError("provider estimate does not match launch manifest")
    if exact_decimal(receipt["quoted_gpu_usd_per_minute"], "provider quote") * exact_decimal(launch["max_runtime_minutes"], "launch maximum runtime") != exact_decimal(receipt["estimated_max_total_usd"], "provider estimated USD") or estimate > ceiling or charged > estimate or charged > ceiling:
...
def source_bundle(raw_root: Path, source_root: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if set(manifest) != required or manifest["schema"] != "gemma3-fineweb-edu-replication-h100-v6-source" or manifest["state_slice"] != STATE_SLICE or manifest["dataset"] != {"repo": DATASET_REPO, "source": DATASET_SOURCE, "revision": DATASET_REVISION, "config": DATASET_CONFIG, "split": DATASET_SPLIT} or manifest["raw_root"] != "raw" or manifest["fresh_row_range"] != {"start": FRESH_START, "end_exclusive": FRESH_END, "count_per_shard": FRESH_END - FRESH_START} or manifest["excluded_ranges"] != [{"name": n, "start": s, "end_exclusive": e, "count_per_shard": e - s} for n, s, e in EXCLUSION_RANGES] or any(manifest[name] is not False for name in ("network_access", "training", "scientific_execution", "evidence_ledger_mutation")) or manifest["manifest_sha256"] != digest(manifest, "manifest_sha256"):
...
def validate_tree_manifest(root: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if set(manifest) != {"schema", "model_id", "model_revision", "architecture", "files", "manifest_sha256"} or manifest["schema"] != "gemma3-model-manifest-v6":
...
def validate_launch_binding(path: Path, repo_root: Path, result: dict[str, Any]) -> None:
    ...
    if set(launch) != LAUNCH_KEYS or launch.get("schema") != "gemma3-fineweb-edu-replication-h100-v6-launch-manifest" or launch.get("state_slice") != STATE_SLICE:
    ...
    registry_path = Path(launch["provider_trust_root_registry_path"]).expanduser()
    if not registry_path.is_absolute() or registry_path.is_symlink() or not registry_path.is_file() or registry_path.stat().st_mode & 0o222:
        raise ValueError("launch provider trust-root registry is not an immutable absolute file")
    if repo_root.resolve() in registry_path.resolve().parents:
        raise ValueError("launch provider trust-root registry must be outside the repository")
    ...
    if ceiling != HARD_USD_CEILING or min(ceiling, estimate, rate, minutes) <= 0 or exact_decimal(launch["quoted_gpu_usd_per_minute"], "launch quote") * exact_decimal(launch["max_runtime_minutes"], "launch maximum runtime") != exact_decimal(launch["estimated_max_total_usd"], "launch estimated USD") or estimate > ceiling:
        raise ValueError("launch budget arithmetic invalid")
    if launch.get("runner_entrypoint") != "run_h100_v6.sh" or launch.get("launch_command") != "./run_h100_v6.sh" or launch.get("network_lock") != "network-none-v6" or launch.get("launch_command_sha256") != hashlib.sha256(b"./run_h100_v6.sh").hexdigest():
    ...
    protocol = repo_root / "docs/research/continual-learning/270-gemma3-fineweb-edu-replication-h100-v6-protocol.md"
    packet = repo_root / "docs/research/continual-learning/271-gemma3-fineweb-edu-replication-h100-v6-review-packet.md"
    ...
    if set(review) != REVIEW_RECEIPT_KEYS or review.get("schema") != "gemma3-fineweb-edu-replication-h100-v6-independent-review":
    ...
    if implementation.get("schema") != "gemma3-fineweb-edu-replication-h100-v6-implementation" or implementation.get("state_slice") != STATE_SLICE or [item.get("path") for item in implementation.get("files", [])] != list(IMPLEMENTATION_FILES):
...
def validate_pre_effect_bundle(raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    from experiments.continual_learning import gemma3_fineweb_edu_replication_h100_v6_preflight as preflight
...
def validate(result_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if any(result[field] != value for field, value in provider_fields.items()):
    ...
    if set(receipt) != {"schema", "state_slice", "result_sha256", "result_file_sha256", "receipt_sha256"} or receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v6-result-receipt" or receipt["state_slice"] != STATE_SLICE or receipt["result_sha256"] != result["results_sha256"] or receipt["result_file_sha256"] != sha256_file(result_root / "result.json") or receipt["receipt_sha256"] != digest(receipt, "receipt_sha256"):
    ...
    if finite(result["hard_usd_ceiling"], "hard USD ceiling") != HARD_USD_CEILING or finite(result["estimated_max_total_usd"], "estimated total USD") <= 0:
    ...
    if (
        ...
        or runtime["network"] != "offline-process-block-v6"

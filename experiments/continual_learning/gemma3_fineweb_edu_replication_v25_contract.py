#!/usr/bin/env python3
"""V25 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v25"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV25"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v25-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v25-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v25-result"
)
MODEL_PATH = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
MODEL_SNAPSHOT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v25-model-snapshot"
)
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/225-gemma3-fineweb-edu-replication-v25-protocol.md"
)
PACKET_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/226-gemma3-fineweb-edu-replication-v25-review-packet.md"
)
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/227-gemma3-fineweb-edu-replication-v25-independent-review-2026-08-31.json"
)
PROTOCOL_SHA256 = "47fa38a2002ba9635de5c9f7a9849e273d1fa5bd8e77721770275a1761046f54"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v25-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v25-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v25-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v25-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v25_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v25.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v25.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v25.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v25.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V25 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def model_manifest_at(
    root: Path, label: str, model_name: str | None = None
) -> dict[str, Any]:
    root = root.absolute()
    if not root.is_dir():
...
def materialize_model_snapshot(model_path: Path) -> Path:
    ...
    if snapshot.exists() or snapshot.is_symlink():
        if (
            snapshot.is_symlink()
            or model_manifest_at(snapshot, "V25 model snapshot", MODEL_PATH.name)
            != source_manifest
        ):
            raise ValueError("V25 model snapshot already exists with a different identity")
        return snapshot
    reject_symlink_components(snapshot, "V25 model snapshot")
    staging = snapshot.parent / f".{snapshot.name}.staging-{os.getpid()}"
    if staging.exists() or staging.is_symlink():
        raise FileExistsError(f"V25 model snapshot staging exists: {staging}")
    staging.mkdir()
    published = False
    try:
        ...
        for relative in expected:
            ...
            with stable_binary_file(source, "V25 model source") as source_handle:
        ...
        staged_manifest = model_manifest_at(
            staging, "V25 model snapshot", MODEL_PATH.name
        )
        if staged_manifest != source_manifest:
            raise RuntimeError("V25 model snapshot digest mismatch")
        os.chmod(staging, 0o555)
        os.rename(staging, snapshot)
        published = True
        if (
            model_manifest_at(snapshot, "V25 model snapshot", MODEL_PATH.name)
            != source_manifest
        ):
            raise RuntimeError("V25 published model snapshot mismatch")
        return snapshot
    except Exception:
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V25 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V25 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V25 implementation file")
        item = regular(path, "V25 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V25 review receipt")
    value = json.loads(
        stable_read_bytes(path, "V25 review receipt").decode("utf-8")
    )
    if not isinstance(value, dict):
        raise ValueError("V25 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V25 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V25 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V25 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V25 review receipt binding mismatch")
    expected_keys = {
        "schema",
        "state_slice",
        "reviewer",
    ...
    if set(value) != expected_keys:
        raise ValueError("V25 review receipt schema is not closed")
    if type(value.get("effects_run")) is not bool or value["effects_run"] is not False:
        raise ValueError("V25 review effects_run must be a boolean false")
    ...
    if (
        ...
        raise ValueError("V25 receipt requires seven explicit true findings")
    if value.get("material_findings") != []:
        raise ValueError("V25 ACCEPT receipt must have an empty material_findings list")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": stable_read_bytes(PROTOCOL_PATH, "V25 protocol"),
        "packet": stable_read_bytes(PACKET_PATH, "V25 packet"),
        "receipt": stable_read_bytes(RECEIPT_PATH, "V25 receipt"),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V25 reviewed bytes or history changed")

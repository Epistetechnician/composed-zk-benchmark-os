#!/usr/bin/env python3
"""Review-gated V13 corpus stage and offline Gemma3 runner.

State slice: continual-learning-gemma3-fineweb-edu-replication-v13.
No model or scientific effect is reachable until the V13 receipt validates.
"""
...
from experiments.continual_learning import (
    validate_gemma3_fineweb_edu_replication_v13 as v,
)

VALIDATOR = "experiments.continual_learning.validate_gemma3_fineweb_edu_replication_v13"
...
def _sandbox() -> None:
    if sys.platform != "darwin" or c.native_network_denied():
        return
    if os.environ.get("V13_RUN_REEXEC") == "1":
        raise RuntimeError("V13 native network denial could not be established")
    executable = shutil.which("sandbox-exec")
    if executable is None:
        raise RuntimeError("sandbox-exec is unavailable")
    env = os.environ.copy()
    env["V13_RUN_REEXEC"] = "1"
    os.execvpe(
        executable,
        [
            executable,
...
class RecirculationConfig:
    ...
    def validate(self, layer_count: int) -> None:
        if (
            ...
            raise ValueError("V13 layer pair invalid")
        if (
            any(
            ...
            raise ValueError("V13 recirculation parameters invalid")
...
def load_runtime(model_path: Path) -> tuple[Any, Any, dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("V13 model custody/runtime mismatch")
    c.require_native_network_denial()
        ...
        if config is None:
            ...
            for index, (layer, layer_cache) in enumerate(
                ...
                if index == config.source_layer:
                    current = hidden
            if current is None:
                raise RuntimeError("V13 source activation not captured")
...
def evaluate_windows(
    ...
    if (
        ...
        raise ValueError("V13 temperature invalid")
    rows = []
    for window in windows:
        ...
        if tuple(ids) != window.token_ids or len(ids) != c.WINDOW_TOKENS:
            raise ValueError("V13 tokenizer changed")
        ...
        if not math.isfinite(total):
            raise ValueError("V13 NLL invalid")
        rows.append(
            {
...
def parity(model: Any, windows: Sequence[v.Window]) -> dict[str, Any]:
    ...
    if len(checks) != 128 or not all(item["passed"] is True for item in checks):
        raise RuntimeError("V13 zero-alpha parity failed")
    return {
...
def _run_effects(
    ...
    if (
        ...
        raise RuntimeError("V13 input changed before effects")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V13 architecture mismatch")
        ...
        if (
            ...
            raise RuntimeError("V13 input changed during effects")
        ...
        if before_parameters != after_parameters or before_model != after_model:
            raise RuntimeError("V13 model custody changed")
    ...
    config = {
        **common,
        ...
        "selection_policy": "first-64-eligible-1024-token-windows-per-disjoint-v13-source-split",
    }
...
def stage_corpus(
    ...
    source, final, model = (
        c.exact_path(source_root, c.SOURCE_ROOT, "V13 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V13 corpus"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V13 corpus exists: {final}")
    ...
    if _reviewed_input(source, None, raw_root, prior_root, model) != input_snapshot:
        raise RuntimeError("V13 inputs changed before tokenizer")
    ...
    try:
        ...
        for split, count in (("fit", 64), ("assessment", 64)):
            selected = []
            for row in v.jsonl(source / f"{split}/fineweb_edu.jsonl", f"V13 {split}"):
                ...
                if (
                    ...
                    raise ValueError("V13 tokenizer round trip failed")
            ...
            if len(selected) != count:
                raise ValueError(f"V13 {split} has {len(selected)} eligible windows")
        ...
        _assert_review(review_snapshot)
        c.exact_file_set(staging, _expected_corpus_files(), "V13 corpus staging")
        with c.network_block():
            v.audit_corpus(
                staging,
                source,
        ...
        output = c.snapshot_files(
            staging, _expected_corpus_files(), "V13 corpus staging"
        )
        c.publish_no_replace(staging, final, _expected_corpus_files(), "V13 corpus")
        return {
            "manifest": _json(final / "manifest.json", "V13 corpus manifest"),
            "snapshot": output,
        }
    except Exception:
        if staging.exists():
...
def _validator_receipt(staging: Path, result_sha: str, review_sha: str) -> None:
    body = {
        "schema": "gemma3-fineweb-edu-replication-v13-validator-receipt",
        "state_slice": c.STATE_SLICE,
...
def run_campaign(
    ...
    final, source, corpus, raw, prior, model = (
        c.exact_path(result_root, c.RESULT_ROOT, "V13 result"),
        c.exact_path(source_root, c.SOURCE_ROOT, "V13 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V13 corpus"),
        c.exact_path(raw_root, c.RAW_ROOT, "V13 raw"),
        c.exact_path(prior_root, c.PRIOR_ROOT, "V13 prior"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V13 result exists: {final}")
    review_snapshot = _review(receipt)
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V13 inputs changed during tokenizer load")
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V13 inputs changed before effects")
    staging = Path(tempfile.mkdtemp(prefix=f".{final.name}.staging-", dir=final.parent))
    try:
        ...
        c.exact_file_set(staging, expected, "V13 result staging")
        _assert_review(review_snapshot)
        current = _reviewed_input(source, corpus, raw, prior, model)
        if current != input_snapshot:
            raise RuntimeError("V13 inputs changed immediately before validator")
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"V13 independent validator failed:\n{completed.stdout}\n{completed.stderr}"
            )
        ...
        if (
            ...
            raise RuntimeError("V13 post-validator custody check failed")
        c.publish_no_replace(staging, final, expected, "V13 result")

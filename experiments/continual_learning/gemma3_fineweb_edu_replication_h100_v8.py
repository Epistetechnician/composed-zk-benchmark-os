...
SCHEMA = "gemma3-fineweb-edu-replication-h100-v8-result"
...
def validate_tree_manifest(root: Path, label: str) -> dict[str, Any]:
    ...
    if manifest["schema"] != "gemma3-model-manifest-v8" or manifest["model_id"] != MODEL_ID or manifest["architecture"] != MODEL_ARCHITECTURE or re.fullmatch(r"[0-9a-f]{40}", manifest["model_revision"]) is None:
...
def require_network_none() -> None:
    ...
    for route_path in (Path("/proc/net/route"), Path("/proc/net/ipv8_route")):
...
def validate_runtime_lock(repo_root: Path, launch: dict[str, Any], torch: Any) -> dict[str, Any]:
    ...
    if not isinstance(lock, dict) or set(lock) != expected_keys or lock["schema"] != "gemma3-fineweb-edu-replication-h100-v8-runtime-lock" or lock["state_slice"] != STATE_SLICE:
    ...
    if lock["dtype"] != "bfloat16" or lock["gpu_required"] != "NVIDIA H100" or lock["network_policy"] != "network-none-v8" or lock["package_install_at_runtime"] is not False:
...
def run(model_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, result_root: Path, launch_manifest: Path, repo_root: Path | None = None, tokenizer: Any | None = None) -> dict[str, Any]:
    ...
    from experiments.continual_learning import (
        gemma3_fineweb_edu_replication_h100_v8_preflight as preflight,
    )
    ...
    from experiments.continual_learning import validate_gemma3_fineweb_edu_replication_h100_v8 as validator
    ...
    from experiments.continual_learning import (
        pack_gemma3_fineweb_edu_replication_h100_v8 as packer,
    )
    ...
    result = {
        ...
        "claim_ceiling": "LocalDevelopmentGemma3FineWebEduReplicationH100V8",
        ...
        "runtime": {
            **runtime,
            ...
            "dtype": "bfloat16",
            "network": "offline-process-block-v8",
        },
    ...
    try:
        ...
        with destination.open("x", encoding="utf-8") as handle:
            ...
            handle.write("\n")
        receipt = {"schema": "gemma3-fineweb-edu-replication-h100-v8-result-receipt", "state_slice": STATE_SLICE, "result_sha256": result["results_sha256"], "result_file_sha256": sha256_file(destination)}

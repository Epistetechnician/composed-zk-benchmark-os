#!/usr/bin/env python3
"""Review-gated V25 corpus stage and offline Gemma3 runner.

State slice: continual-learning-gemma3-fineweb-edu-replication-v25.
No model or scientific effect is reachable until the V25 receipt validates.
"""
...
from experiments.continual_learning import (
    validate_gemma3_fineweb_edu_replication_v25 as v,
)

VALIDATOR = "experiments.continual_learning.validate_gemma3_fineweb_edu_replication_v25"
...
def _sandbox() -> None:
    if sys.platform != "darwin" or c.native_network_denied():
        return
    if os.environ.get("V25_RUN_REEXEC") == "1":
        raise RuntimeError("V25 native network denial could not be established")
    executable = shutil.which("sandbox-exec")
    if executable is None:
        raise RuntimeError("sandbox-exec is unavailable")
    env = os.environ.copy()
    env["V25_RUN_REEXEC"] = "1"
    os.execvpe(
        executable,
        [
            executable,
...
def parity(model: Any, windows: Sequence[v.Window]) -> dict[str, Any]:
    ...
    if len(checks) != 128 or not all(item["passed"] is True for item in checks):
        raise RuntimeError("V25 zero-alpha parity failed")
    return {
...
def _run_effects(
    ...
    if (
        ...
        raise RuntimeError("V25 input changed before effects")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V25 architecture mismatch")
        ...
        if (
            ...
            raise RuntimeError("V25 input changed during effects")
        ...
        if before_parameters != after_parameters or before_model != after_model:
            raise RuntimeError("V25 model custody changed")
    ...
    if not any(item["reached"] for item in reach):
        raise RuntimeError("V25 nonzero intervention reach failed")
    common = {
...
def stage_corpus(
    ...
    source, final, model = (
        c.exact_path(source_root, c.SOURCE_ROOT, "V25 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V25 corpus"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V25 corpus exists: {final}")
    ...
    if _reviewed_input(source, None, raw_root, prior_root, model) != input_snapshot:
        raise RuntimeError("V25 inputs changed before tokenizer")
    ...
    try:
        ...
        for split, count in (("fit", 64), ("assessment", 64)):
            selected = []
            for row in v.jsonl(source / f"{split}/fineweb_edu.jsonl", f"V25 {split}"):
                ...
                if (
                    ...
                    raise ValueError("V25 tokenizer round trip failed")
            ...
            if len(selected) != count:
                raise ValueError(f"V25 {split} has {len(selected)} eligible windows")
        ...
        _assert_review(review_snapshot)
        c.exact_file_set(staging, _expected_corpus_files(), "V25 corpus staging")
        with c.network_block():
            v.audit_corpus(
                staging,
                source,
        ...
        output = c.snapshot_files(
            staging, _expected_corpus_files(), "V25 corpus staging"
        )
        c.publish_no_replace(
            ...
            lambda: (
                ...
                if _reviewed_input(source, None, raw_root, prior_root, model) == input_snapshot
                    ...
                    RuntimeError("V25 corpus inputs changed after publication")
                ),
            ),
        )
        return {
            "manifest": _json(final / "manifest.json", "V25 corpus manifest"),
            "snapshot": output,
        }
    except Exception:
        if staging.exists():
...
def run_campaign(
    ...
    final, source, corpus, raw, prior, model = (
        c.exact_path(result_root, c.RESULT_ROOT, "V25 result"),
        c.exact_path(source_root, c.SOURCE_ROOT, "V25 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V25 corpus"),
        c.exact_path(raw_root, c.RAW_ROOT, "V25 raw"),
        c.exact_path(prior_root, c.PRIOR_ROOT, "V25 prior"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V25 result exists: {final}")
    review_snapshot = _review(receipt)
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V25 inputs changed during tokenizer load")
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V25 inputs changed before effects")
    staging = Path(tempfile.mkdtemp(prefix=f".{final.name}.staging-", dir=final.parent))
    try:
        ...
        c.exact_file_set(staging, expected, "V25 result staging")
        _assert_review(review_snapshot)
        current = _reviewed_input(source, corpus, raw, prior, model)
        if current != input_snapshot:
            raise RuntimeError("V25 inputs changed immediately before validator")
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"V25 independent validator failed:\n{completed.stdout}\n{completed.stderr}"
            )
        ...
        if (
            ...
            raise RuntimeError("V25 post-validator custody check failed")
        ...
        if c.validation_binding(final_validation) != c.validation_binding(validation):
            raise RuntimeError("V25 final validator result mismatch")
        expected_with_validator = set(expected) | {"validator-receipt.json"}
        c.exact_file_set(staging, expected_with_validator, "V25 result staging")
        result_snapshot = final_validation.get("result_snapshot")
        if (
            not isinstance(result_snapshot, list)
            or c.snapshot_files(staging, expected_with_validator, "V25 result staging")
            != result_snapshot
        ):
            raise RuntimeError("V25 final validator snapshot mismatch")
        c.publish_no_replace(
            staging,
            final,
            ...
            lambda: (
                ...
                if _reviewed_input(source, corpus, raw, prior, model) == input_snapshot
                    ...
                    RuntimeError("V25 result inputs changed after publication")
                ),
            ),

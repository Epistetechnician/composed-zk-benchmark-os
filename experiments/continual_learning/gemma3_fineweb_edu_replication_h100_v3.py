...
SCHEMA = "gemma3-fineweb-edu-replication-h100-v3-result"
...
def validate_tree_manifest(root: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(manifest, dict) or set(manifest) != {
        "schema", "files", "manifest_sha256"
    }:
        raise ValueError(f"{label} manifest schema is not closed")
    if manifest["schema"] != "sealed-file-tree-v1":
...
def require_network_none() -> None:
    ...
    for route_path in (Path("/proc/net/route"), Path("/proc/net/ipv6_route")):
        if route_path.is_file():
            ...
            if len(lines) > 1:
                raise RuntimeError(f"network route proof failed: {route_path}")
...
def validate_runtime_lock(repo_root: Path, launch: dict[str, Any], torch: Any) -> dict[str, Any]:
    ...
    expected_keys = {
        "schema", "state_slice", "python", "accelerate", "pytorch", "transformers", "safetensors",
        "tokenizers", "pyarrow", "cuda_runtime", "dtype", "gpu_required",
        "network_policy", "package_install_at_runtime",
    }
    if not isinstance(lock, dict) or set(lock) != expected_keys or lock["schema"] != "gemma3-fineweb-edu-replication-h100-v3-runtime-lock" or lock["state_slice"] != STATE_SLICE:
    ...
    if lock["dtype"] != "bfloat16" or lock["gpu_required"] != "NVIDIA H100" or lock["network_policy"] != "network-none-v3" or lock["package_install_at_runtime"] is not False:
...
def run(model_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, result_root: Path, launch_manifest: Path, repo_root: Path | None = None, tokenizer: Any | None = None) -> dict[str, Any]:
    ...
    if result_root.exists():
        raise FileExistsError(f"result root already exists: {result_root}")
    model_manifest = validate_tree_manifest(model_root, "model bundle")
    if model_manifest["manifest_sha256"] != launch["model_manifest_sha256"]:
    ...
    from experiments.continual_learning import (
        pack_gemma3_fineweb_edu_replication_h100_v3 as packer,
    )
    ...
    result = {
        ...
        "claim_ceiling": "LocalDevelopmentGemma3FineWebEduReplicationH100V3",
        ...
        "runtime": {
            **runtime,
            ...
            "dtype": "bfloat16",
            "network": "offline-process-block-v3",
        },
    ...
    try:
        ...
        with destination.open("x", encoding="utf-8") as handle:
            ...
            handle.write("\n")
        receipt = {"schema": "gemma3-fineweb-edu-replication-h100-v3-result-receipt", "state_slice": STATE_SLICE, "result_sha256": result["results_sha256"], "result_file_sha256": sha256_file(destination)}

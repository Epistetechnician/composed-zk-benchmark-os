...
PROTOCOL_PATH = Path(
    "docs/research/"
    "continual-learning/320-gemma3-fineweb-edu-replication-h100-v8-protocol.md"
)
PACKET_PATH = Path(
    "docs/research/"
    "continual-learning/321-gemma3-fineweb-edu-replication-h100-v8-review-packet.md"
)
SCHEMA = "gemma3-fineweb-edu-replication-h100-v8-launch-manifest"
EXPECTED_KEYS = {
    "schema",
    "state_slice",
    "provider",
...
def validate_code_bundle_digest(path: Path, expected: Any) -> None:
    sha256_hex(expected, "code_bundle_sha256")
    if path.is_symlink() or not path.is_file():
        raise ValueError("code_bundle_path must be an immutable regular file")
    if sha256_file(path) != expected:
        raise ValueError("code_bundle_sha256 does not match code_bundle_path")
...
def validate_implementation_manifest(
    ...
    if (
        manifest["schema"]
        != "gemma3-fineweb-edu-replication-h100-v8-implementation"
...
def validate_review_receipt(
    ...
    if receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v8-independent-review" or receipt["state_slice"] != STATE_SLICE or receipt["review_decision"] != "ACCEPT" or receipt["effects_run"] is not False:
...
def validate_launch_manifest(
    ...
    if exact_decimal(manifest["hard_usd_ceiling"], "hard_usd_ceiling") != HARD_USD_CEILING:
        raise ValueError("hard_usd_ceiling must be exactly USD 69.00")
    if rate * minutes != estimate or estimate > ceiling:
    ...
    if manifest["runner_entrypoint"] != "run_h100_v8.sh":
        raise ValueError("runner entrypoint is not the reviewed provider entrypoint")
    if manifest["launch_command"] != "./run_h100_v8.sh":
    ...
    if manifest["network_lock"] != "network-none-v8":
    ...
    code_bundle_path = Path(manifest["code_bundle_path"]).expanduser().resolve()
    validate_code_bundle_digest(code_bundle_path, manifest["code_bundle_sha256"])

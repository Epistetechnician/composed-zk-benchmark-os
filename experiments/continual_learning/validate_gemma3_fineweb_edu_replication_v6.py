#!/usr/bin/env python3
"""Independent fail-closed V6 validator.
...
from experiments.continual_learning import (
    gemma3_fineweb_edu_replication_v6_contract as c,
)
...
def audit_raw(root: Path) -> list[dict[str, Any]]:
    root = c.exact_path(root, c.RAW_ROOT, "V6 raw")
    c.exact_file_set(
        root, {f"dataset/{item['path']}" for item in c.DATASET_FILES}, "V6 raw"
    )
    artifacts = []
    import pyarrow.parquet as pq

    for item in c.DATASET_FILES:
        path = c.regular(root / "dataset" / item["path"], "V6 Parquet")
        if (
            path.stat().st_size != item["byte_len"]
            or c.sha256_file(path) != item["sha256"]
        ):
            raise ValueError("V6 raw pin mismatch")
        count = pq.ParquetFile(path).metadata.num_rows
        if count < c.FRESH_ROW_END:
            raise ValueError("V6 raw shard is too short")
        artifacts.append(
            {
...
def audit_prior(root: Path, raw_root: Path) -> set[str]:
    root = c.exact_path(root, c.PRIOR_ROOT, "V6 prior")
    c.exact_file_set(
        root,
        {
    ...
    if c.sha256_file(manifest_path) != c.PRIOR_MANIFEST_SHA256:
        raise ValueError("prior manifest pin mismatch")
    manifest = obj(manifest_path, "prior manifest")
    self_digest(manifest, "manifest_sha256", "prior manifest")
    if (
    ...
    raw = c.exact_path(raw_root, c.RAW_ROOT, "V6 raw")
    for split, item in (
        ...
        for ordinal, (observed, recomputed) in enumerate(
            ...
            if (
                ...
                or observed.get("source_row_index") != ordinal
            ):
...
def audit_source(
    ...
    root = c.exact_or_staging(root, c.SOURCE_ROOT, "V6 source")
    c.exact_file_set(
        root,
        {
            "acquisition-manifest.json",
    ...
    manifest = obj(root / "acquisition-manifest.json", "V6 source manifest")
    required = {
        ...
        "selection_policy": "rows-2048-through-18431-two-pinned-crawls-v6",
    ...
    if any(manifest.get(key) != value for key, value in required.items()) or any(
        ...
        raise ValueError("V6 source contract mismatch")
    self_digest(manifest, "manifest_sha256", "V6 source manifest")
    raw = c.exact_path(raw_root, c.RAW_ROOT, "V6 raw")
    ...
    if (
        ...
        raise ValueError("V6 source custody mismatch")
    ids, split_rows = set(), {}
    datasets = manifest.get("datasets")
    if not isinstance(datasets, dict):
        raise ValueError("V6 source datasets missing")
    for split, item in (
        ("fit", c.DATASET_FILES[0]),
        ("assessment", c.DATASET_FILES[1]),
    ):
        ...
        if not isinstance(metadata, dict) or any(
            ...
            raise ValueError("V6 source dataset metadata mismatch")
        path = c.safe_relative(root, expected["normalized_path"], "V6 source JSONL")
        if c.sha256_file(path) != metadata.get(
            "normalized_sha256"
        ) or path.stat().st_size != metadata.get("normalized_byte_len"):
            raise ValueError("V6 source JSONL digest mismatch")
        rows = jsonl(path, "V6 source JSONL")
        if len(rows) != c.FRESH_ROW_COUNT:
            raise ValueError("V6 source row count mismatch")
        ...
        for ordinal, (row, recomputed) in enumerate(
            ...
            if (
                ...
                or row.get("source_row_index") != c.FRESH_ROW_START + ordinal
                or row.get("source_crawl") != item["crawl"]
                or row.get("source_path") != item["path"]
            ):
                raise ValueError("V6 source identity mismatch")
...
def parse_window(
    ...
    if (
        ...
        or entry.get("window_ordinal") != 0
        or entry.get("token_count") != c.WINDOW_TOKENS
    ):
        raise ValueError("V6 window metadata mismatch")
    path = c.safe_relative(root, entry.get("path"), "V6 window")
    raw = path.read_bytes()
    text = raw.decode("utf-8")
    if len(raw) != entry.get("byte_len") or hashlib.sha256(
        raw
    ).hexdigest() != entry.get("text_sha256"):
        raise ValueError("V6 window bytes mismatch")
    token_ids = tuple(
    ...
    if len(token_ids) != c.WINDOW_TOKENS or tokenizer.decode(list(token_ids)) != text:
        raise ValueError("V6 tokenizer window mismatch")
    return Window(
        "fineweb_edu",
        entry["document_id"],
        entry["path"],
...
def audit_corpus(
    ...
    root = c.exact_or_staging(root, c.CORPUS_ROOT, "V6 corpus")
    expected = {
        "manifest.json",
        *{
    ...
    c.exact_file_set(root, expected, "V6 corpus")
    source = audit_source(source_root, raw_root, prior_root, review_sha)
    manifest = obj(root / "manifest.json", "V6 corpus manifest")
    required = {
        ...
        "model_manifest_sha256": c.EXPECTED_MODEL_MANIFEST_SHA256,
        "model_cache_manifest_sha256": c.EXPECTED_MODEL_CACHE_MANIFEST_SHA256,
    ...
    if any(manifest.get(key) != value for key, value in required.items()) or any(
        ...
        raise ValueError("V6 corpus contract mismatch")
    self_digest(manifest, "manifest_sha256", "V6 corpus manifest")
    model = c.model_manifest(model_path)
    if (
        model["manifest_sha256"] != c.EXPECTED_MODEL_MANIFEST_SHA256
        or model["cache_manifest_sha256"] != c.EXPECTED_MODEL_CACHE_MANIFEST_SHA256
    ):
        raise ValueError("V6 model pin mismatch")
    by_id = {
    ...
    for split in ("fit", "assessment"):
        ...
        if not isinstance(entries, list) or len(entries) != 64:
            raise ValueError("V6 corpus window count mismatch")
        parsed = []
        for entry in entries:
            ...
            if (
                ...
                or entry.get("source_row_index") != source_row["source_row_index"]
            ):
                raise ValueError("V6 corpus source binding mismatch")
    ...
    if set(w.document_id for w in windows["fit"]) & set(
        ...
        raise ValueError("V6 fit/assessment overlap")
    return {
        "manifest": manifest,
        "manifest_sha256": manifest["manifest_sha256"],
        "windows": windows,
...
def metrics(
    ...
    if (
        ...
        or value.get("temperature") != float(temperature)
...
def parity(value: Any, windows: list[Window]) -> dict[str, Any]:
    ...
    for check in checks:
        if (
            ...
            or check.get("token_count") != c.WINDOW_TOKENS
            or check.get("tolerance") != c.PARITY_TOLERANCE
        ):
            raise ValueError("V6 parity identity mismatch")
        finite(check.get("max_abs_logit_delta"), "V6 parity delta")
        seen.add(check["document_id"])
    if seen != set(by_id) or value["max_abs_logit_delta"] != max(
        check["max_abs_logit_delta"] for check in checks
    ):
        raise ValueError("V6 parity aggregate mismatch")
    return value
...
def input_snapshot(
    ...
    result = {
        "source": c.snapshot_files(
            c.exact_path(source, c.SOURCE_ROOT, "V6 source"),
            {
                "acquisition-manifest.json",
                "fit/fineweb_edu.jsonl",
                "assessment/fineweb_edu.jsonl",
        ...
        "raw": c.snapshot_files(
            c.exact_path(raw, c.RAW_ROOT, "V6 raw"),
            {f"dataset/{item['path']}" for item in c.DATASET_FILES},
            "V6 raw",
        ),
        "prior": c.snapshot_files(
            c.exact_path(prior, c.PRIOR_ROOT, "V6 prior"),
            {
                "acquisition-manifest.json",
                "fit/fineweb_edu.jsonl",
                "assessment/fineweb_edu.jsonl",
    ...
    if corpus is not None:
        result["corpus"] = c.snapshot_files(
            c.exact_path(corpus, c.CORPUS_ROOT, "V6 corpus"),
            {
                "manifest.json",
                *{
                    f"{split}/window-{i:06d}.txt"
...
def validate_source(
    ...
    return audit_source(
        ...
        raw_root,
        prior_root,
        c.sha256_file(c.exact_path(receipt, c.RECEIPT_PATH, "V6 receipt")),
    ) | {
        "review_receipt_sha256": c.sha256_file(
            c.exact_path(receipt, c.RECEIPT_PATH, "V6 receipt")
        ),
...
def validate_result(
    ...
    root = c.exact_or_staging(result_root, c.RESULT_ROOT, "V6 result")
    expected_files = {
        "config.json",
        "results.json",
        "receipt.json",
    ...
    c.exact_file_set(root, expected_files, "V6 result")
    review = c.validate_review_receipt(receipt)
    review_path = c.exact_path(receipt, c.RECEIPT_PATH, "V6 receipt")
    ...
    if c.digest(current_input) != invocation_snapshot_sha:
        raise ValueError("V6 invocation input snapshot mismatch")
    staged = c.regular(root / "review-receipt.json", "V6 staged receipt")
    if staged.read_bytes() != review_path.read_bytes():
        raise ValueError("V6 staged review receipt mismatch")
    source = audit_source(source_root, raw_root, prior_root, review_sha)
    corpus = audit_corpus(
        corpus_root,
        source_root,
    ...
    if (
        ...
        raise ValueError("V6 corpus result binding mismatch")
    model = obj(root / "model-manifest.json", "V6 result model manifest")
    current_model = c.model_manifest(model_path)
    if (
        model != current_model
        or model["manifest_sha256"] != c.EXPECTED_MODEL_MANIFEST_SHA256
        or model["cache_manifest_sha256"] != c.EXPECTED_MODEL_CACHE_MANIFEST_SHA256
    ):
        raise ValueError("V6 model result binding mismatch")
    config, results, result_receipt, validator_receipt = (
        obj(root / name, f"V6 {name}")
        for name in (
            "config.json",
            "results.json",
            "receipt.json",
    ...
    for value, field, label in (
        (config, "config_sha256", "V6 config"),
        (results, "results_sha256", "V6 results"),
        (result_receipt, "receipt_sha256", "V6 result receipt"),
        (validator_receipt, "receipt_sha256", "V6 validator receipt"),
    ):
        self_digest(value, field, label)
    for value, label in (
        (config, "V6 config"),
        (results, "V6 results"),
        (result_receipt, "V6 result receipt"),
    ):
        common(value, label, review_sha, source["manifest_sha256"], corpus_sha)
    if (
        validator_receipt.get("schema")
        != "gemma3-fineweb-edu-replication-v6-validator-receipt"
        ...
        raise ValueError("V6 validator receipt mismatch")
    if (
        ...
        raise ValueError("V6 result receipt cross-binding mismatch")
    if any(
        ...
        raise ValueError("V6 result flags mismatch")
    locked_constants = {
    ...
    if any(
        ...
        raise ValueError("V6 locked configuration mismatch")
    windows = corpus["windows"]
    fit_base = metrics(
        results.get("fit_baseline"), windows["fit"], "V6 fit baseline", 1.0, None
    )
    candidates = results.get("fit_candidates")
    if not isinstance(candidates, list) or len(candidates) != len(c.CANDIDATE_PAIRS):
        raise ValueError("V6 candidate set mismatch")
    candidate_means = []
    for candidate, pair in zip(candidates, c.CANDIDATE_PAIRS, strict=True):
        ...
        if not isinstance(candidate, dict) or candidate.get("config") != cfg:
            raise ValueError("V6 candidate config mismatch")
        candidate_means.append(
            (
                metrics(
                    candidate.get("metrics"), windows["fit"], "V6 candidate", 1.0, cfg
                )["mean_nll"],
                pair[0],
                pair[1],
            )
    ...
    if (
        ...
        raise ValueError("V6 selection mismatch")
    assessment_base = metrics(
        results.get("assessment_baseline"),
        windows["assessment"],
        "V6 assessment baseline",
        1.0,
        None,
    )
    selected = metrics(
    ...
    if repeat != selected or results.get("deterministic_repeat_passed") is not True:
        raise ValueError("V6 repeat mismatch")
    parity_value = parity(
    ...
    if not isinstance(per_document, list) or len(per_document) != len(
        ...
        raise ValueError("V6 per-document count mismatch")
    deltas, seen = [], set()
    ...
    for row in per_document:
        if (
            ...
            or row.get("window_ordinal") != 0
            or row.get("token_count") != c.WINDOW_TOKENS
            or strict_int(row.get("target_count"), "V6 retained targets")
            != c.WINDOW_TOKENS - 1
        ):
            raise ValueError("V6 per-document identity mismatch")
        ...
        if (
            ...
            raise ValueError("V6 per-document metric mismatch")
        finite(row["delta_selected_minus_baseline"], "V6 delta")
        deltas.append(row["delta_selected_minus_baseline"])
        seen.add(document_id)
    if seen != set(by_id):
        raise ValueError("V6 per-document set mismatch")
    ...
    if (
        ...
        raise ValueError("V6 uncertainty mismatch")
    reach_evidence = []
    ...
    if (
        ...
        raise ValueError("V6 reach was not independently derived")
    if results.get("controls") != {
        ...
        "frozen_model_manifest": {
            "before": c.EXPECTED_MODEL_MANIFEST_SHA256,
            "after": c.EXPECTED_MODEL_MANIFEST_SHA256,
        },
        ...
        raise ValueError("V6 controls mismatch")
    for field in ("model_parameter_digest_before", "model_parameter_digest_after"):
        if (
            ...
            raise ValueError("V6 parameter binding mismatch")
    if (
        results["model_parameter_digest_before"]
        != results["model_parameter_digest_after"]
    ):
        raise ValueError("V6 model parameters changed")
    c.require_native_network_denial()
    from experiments.continual_learning import (
        stage_and_run_gemma3_fineweb_edu_replication_v6 as runner,
    )

    if c.model_manifest(model_path) != current_model:
        raise ValueError("V6 model changed before independent recomputation")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V6 independent model shape/runtime mismatch")
        if (
            c.model_parameter_digest(model_value)
            != results["model_parameter_digest_before"]
        ):
            raise ValueError("V6 independent parameter-before mismatch")
        ...
        if (
            ...
            raise ValueError("V6 independent parity mismatch")
        if (
            runner.evaluate_windows(model_value, tokenizer_value, fit_windows, None)
            != fit_base
        ):
            raise ValueError("V6 independent fit baseline mismatch")
        ...
        if recomputed_candidates != candidates:
            raise ValueError("V6 independent candidate mismatch")
        ...
        if (
            ...
            raise ValueError("V6 independent assessment/control mismatch")
        if (
            c.model_parameter_digest(model_value)
            != results["model_parameter_digest_after"]
            or c.model_manifest(model_path) != current_model
        ):
            raise ValueError("V6 independent model custody-after mismatch")
    if (
        input_snapshot(source_root, corpus_root, raw_root, prior_root, model_path)
        != current_input
    ):
        raise ValueError("V6 inputs changed during independent recomputation")
...
def _tokenizer_for_validation(model_path: Path) -> Any:
    ...
    if (
        manifest["manifest_sha256"] != c.EXPECTED_MODEL_MANIFEST_SHA256
        or manifest["cache_manifest_sha256"] != c.EXPECTED_MODEL_CACHE_MANIFEST_SHA256
        or c.runtime_versions() != c.RUNTIME_VERSIONS
    ):
        raise RuntimeError("V6 tokenizer custody/runtime mismatch")

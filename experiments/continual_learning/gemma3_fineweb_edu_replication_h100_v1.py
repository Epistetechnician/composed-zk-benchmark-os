...
SCHEMA = "gemma3-fineweb-edu-replication-h100-v1-result"
...
def validate_runtime_lock(repo_root: Path, launch: dict[str, Any], torch: Any) -> dict[str, Any]:
    ...
    if not isinstance(lock, dict) or set(lock) != expected_keys or lock["schema"] != "gemma3-fineweb-edu-replication-h100-v1-runtime-lock" or lock["state_slice"] != STATE_SLICE:
    ...
    if lock["dtype"] != "bfloat16" or lock["gpu_required"] != "NVIDIA H100" or lock["network_policy"] != "network-none-v1" or lock["package_install_at_runtime"] is not False:
...
def run(model_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, result_root: Path, launch_manifest: Path, repo_root: Path | None = None, tokenizer: Any | None = None) -> dict[str, Any]:
    ...
    from experiments.continual_learning import (
        gemma3_fineweb_edu_replication_h100_v1_preflight as preflight,
    )
    ...
    from experiments.continual_learning import (
        pack_gemma3_fineweb_edu_replication_h100_v1 as packer,
    )
    ...
    result = {
        ...
        "claim_ceiling": "LocalDevelopmentGemma3FineWebEduReplicationH100V1",
        ...
        "runtime": {
            **runtime,
            ...
            "dtype": "bfloat16",
            "network": "offline-process-block-v1",
        },
    ...
    try:
        ...
        with destination.open("x", encoding="utf-8") as handle:
            ...
            handle.write("\n")
        receipt = {"schema": "gemma3-fineweb-edu-replication-h100-v1-result-receipt", "state_slice": STATE_SLICE, "result_sha256": result["results_sha256"], "result_file_sha256": sha256_file(destination)}

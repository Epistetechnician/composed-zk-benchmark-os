#!/usr/bin/env python3
"""V39 Qwen2.5 fresh task-order retention replication.

State slice: continual-learning-qwen25-fixed-optimizer-order-replication-v39.

V39 re-executes the V38 retention panel across three noncanonical task orders
crossed with all three V38 task seeds. The V37 acquisition artifacts remain
the immutable source of task manifests and task-routed adapters. The model,
raw-text seam, optimizer policy, update budget, replay capacity, and fresh
readout remain fixed. This is order-replication evidence below a general
...
STATE_SLICE = "continual-learning-qwen25-fixed-optimizer-order-replication-v39"
PROTOCOL = "v39-qwen25-fixed-optimizer-order-replication-v1"
...
CLAIM_CEILING = "LocalDevelopmentOrderReplicationPreflight"
SOURCE_STATE_SLICE = v38.SOURCE_STATE_SLICE
SOURCE_ARTIFACT_ROOT = v38.SOURCE_ARTIFACT_ROOT
...
def run_case(output: Path, source_case: Path, model: Path, task_seed: int, order: tuple[int, ...]) -> dict:
    ...
    if order not in ORDERS:
        raise ValueError("V39 order is not in the frozen replication set")
    if task_seed not in TASK_SEEDS:
        raise ValueError("V39 task seed is not in the V38 source set")
    original_order = v38.ORDER
    v38.ORDER = order
    try:
        result = v38.run_case(output, source_case, model, task_seed)
    finally:
        v38.ORDER = original_order
    ...
    config.update(
        {
            ...
            "parent_state_slice": v38.STATE_SLICE,
            "claim_ceiling": CLAIM_CEILING,
            "independence_status": "fresh_task_order_replication_reusing_v37_acquisition_source",
        }
    )
    ...
    result.update(
        {
            ...
            "protocol": PROTOCOL,
            "claim_ceiling": CLAIM_CEILING,
            "classification": "Qwen25FixedOptimizerOrderReplicationNoGeneralClaim",
            "config": config,
            "replication_order": list(order),
            "independence_status": "fresh_task_order_replication_reusing_v37_acquisition_source",
        }
    ...
    result["manifest_sha256"] = base.digest({"config": config, "tasks": result["tasks"], "audits": audits})
...
def run_campaign(artifact_root: Path, model: Path, source_root: Path) -> dict:
    ...
    if artifact_root.exists():
        raise RuntimeError(f"refusing overwrite of immutable V39 campaign: {artifact_root}")
    if not artifact_root.is_absolute() or Path(__file__).resolve().parents[2] in artifact_root.parents:
        raise ValueError("V39 artifacts must remain outside the repository")
    if source_root != SOURCE_ARTIFACT_ROOT.resolve():
        raise ValueError("V39 source custody path drift")
    if model != MODEL_DEFAULT.resolve() or not model.is_dir():
        raise ValueError("V39 model binding is unavailable or drifted")
    ...
    if source_validation["campaign_eligible"] is not True:
        raise ValueError("V39 requires the campaign-eligible V37 acquisition source")
    contract = {
        "state_slice": STATE_SLICE,
        "protocol": PROTOCOL,
        "model": str(model),
    ...
    for task_seed in TASK_SEEDS:
        for order in ORDERS:
            ...
            runner = subprocess.run(
                [sys.executable, str(Path(__file__).resolve()), "--case-output", str(case_root), "--source-case", str(source_case), "--model", str(model), "--task-seed", str(task_seed), "--order", order_code(order)],
                env=environment,
                text=True,
                capture_output=True,
                check=False,
            ...
            validator = subprocess.run(
                [sys.executable, str(Path(__file__).with_name("validate_qwen25_fixed_optimizer_order_replication_v39.py")), str(case_root), "--source-case", str(source_case), "--model", str(model), "--expected-task-seed", str(task_seed), "--expected-order", order_code(order)],
                env=environment,
                text=True,
                capture_output=True,
                check=False,
...
def main() -> int:
    ...
    if args.case_output is not None:
        if args.task_seed is None or args.source_case is None or args.order is None:
            raise ValueError("V39 case mode requires task-seed, source-case, and order")
    ...
    if args.artifact_root is None:
        raise ValueError("V39 campaign mode requires artifact-root")

...
if [ "${H100_NETWORK_LOCK}" != "network-none-v8" ]; then
...
if [ ! -r /proc/net/ipv8_route ] || [ "$(tr -d '[:space:]' </proc/net/ipv8_route)" != "" ]; then
...
python -m experiments.continual_learning.validate_gemma3_fineweb_edu_replication_h100_v8 \
  --pre-effect \
...
python -m experiments.continual_learning.gemma3_fineweb_edu_replication_h100_v8 \
  --launch-manifest "${H100_LAUNCH_MANIFEST}" \
  --model-root "${H100_MODEL_ROOT}" \
  --raw-root "${H100_RAW_ROOT}" \
  --source-root "${H100_SOURCE_ROOT}" \
...
exec python -m experiments.continual_learning.validate_gemma3_fineweb_edu_replication_h100_v8 \
  --result-root "${H100_RESULT_ROOT}" \
  --raw-root "${H100_RAW_ROOT}" \
  --source-root "${H100_SOURCE_ROOT}" \
  --corpus-root "${H100_CORPUS_ROOT}" \

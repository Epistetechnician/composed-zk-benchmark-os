#!/usr/bin/env python3
"""V23 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v23"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV23"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v23-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v23-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v23-result"
)
...
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/219-gemma3-fineweb-edu-replication-v23-independent-review-2026-08-31.json"
)
PROTOCOL_SHA256 = "71a71706fe0be7c7f099b2b5fc84ed8c5269164cee3306dadd234ff1e86caaff"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v23-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v23-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v23-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v23-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v23_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v23.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v23.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v23.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v23.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V23 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V23 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V23 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V23 implementation file")
        item = regular(path, "V23 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V23 review receipt")
    value = json.loads(
        stable_read_bytes(path, "V23 review receipt").decode("utf-8")
    )
    if not isinstance(value, dict):
        raise ValueError("V23 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V23 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V23 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V23 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V23 review receipt binding mismatch")
    expected_keys = {
        "schema",
        "state_slice",
        "reviewer",
    ...
    if set(value) != expected_keys:
        raise ValueError("V23 review receipt schema is not closed")
    if type(value.get("effects_run")) is not bool or value["effects_run"] is not False:
        raise ValueError("V23 review effects_run must be a boolean false")
    ...
    if (
        ...
        raise ValueError("V23 receipt requires seven explicit true findings")
    if value.get("material_findings") != []:
        raise ValueError("V23 ACCEPT receipt must have an empty material_findings list")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": stable_read_bytes(PROTOCOL_PATH, "V23 protocol"),
        "packet": stable_read_bytes(PACKET_PATH, "V23 packet"),
        "receipt": stable_read_bytes(RECEIPT_PATH, "V23 receipt"),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V23 reviewed bytes or history changed")

#!/usr/bin/env python3
"""V20 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v20"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV20"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v20-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v20-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v20-result"
)
...
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/207-gemma3-fineweb-edu-replication-v20-independent-review-2026-08-30.json"
)
PROTOCOL_SHA256 = "f42080f8ac660c175941f92cc18ecfd9d736e0afc77f7cc9b40cd4926538a0f2"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v20-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v20-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v20-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v20-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v20_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v20.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v20.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v20.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v20.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V20 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V20 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V20 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V20 implementation file")
        item = regular(path, "V20 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V20 review receipt")
    value = json.loads(regular(path, "V20 review receipt").read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("V20 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V20 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V20 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V20 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V20 review receipt binding mismatch")
    if value.get("findings") != {name: True for name in REVIEW_FINDINGS}:
        raise ValueError("V20 receipt requires seven true findings")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": regular(PROTOCOL_PATH, "V20 protocol").read_bytes(),
        "packet": regular(PACKET_PATH, "V20 packet").read_bytes(),
        "receipt": regular(RECEIPT_PATH, "V20 receipt").read_bytes(),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V20 reviewed bytes or history changed")

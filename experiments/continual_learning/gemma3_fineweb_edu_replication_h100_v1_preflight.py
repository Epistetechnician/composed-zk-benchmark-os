...
PROTOCOL_PATH = Path(
    "docs/research/"
    "continual-learning/252-gemma3-fineweb-edu-replication-h100-v1-protocol.md"
)
PACKET_PATH = Path(
    "docs/research/"
    "continual-learning/253-gemma3-fineweb-edu-replication-h100-v1-review-packet.md"
)
SCHEMA = "gemma3-fineweb-edu-replication-h100-v1-launch-manifest"
EXPECTED_KEYS = {
    "schema",
    "state_slice",
    "provider",
...
def validate_implementation_manifest(
    ...
    if (
        manifest["schema"]
        != "gemma3-fineweb-edu-replication-h100-v1-implementation"
...
def validate_review_receipt(
    ...
    if receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v1-independent-review" or receipt["state_slice"] != STATE_SLICE or receipt["review_decision"] != "ACCEPT" or receipt["effects_run"] is not False:
...
def validate_launch_manifest(
    ...
    if manifest["runner_entrypoint"] != "run_h100_v1.sh":
        raise ValueError("runner entrypoint is not the reviewed provider entrypoint")
    if manifest["launch_command"] != "./run_h100_v1.sh":
    ...
    if manifest["network_lock"] != "network-none-v1":

#!/usr/bin/env python3
"""Stage and run the bounded FineWeb-Edu Gemma3 recirculation pilot.

State slice: continual-learning-gemma3-fineweb-edu-bounded-v1.

The runner consumes only the independently validated external FineWeb-Edu
source bundle, stages document-disjoint 1024-token windows, and evaluates the
cached Gemma3 1B BF16 MLX checkpoint offline with frozen weights. It is a
bounded local pilot, not exact C4 or full-paper replication.
"""
...
from experiments.continual_learning.validate_gemma3_fineweb_edu_bounded_v1 import (
    CLAIM_CEILING,
    CORPUS_SCHEMA,
    PARITY_TOLERANCE,
    PILOT_PAIRS,
...
DEFAULT_CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/"
    "gemma3-fineweb-edu-bounded-corpus-v1"
)
DEFAULT_OUTPUT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/"
    "gemma3-fineweb-edu-bounded-recirculation-v1"
)
VALIDATOR = Path(__file__).with_name("validate_gemma3_fineweb_edu_bounded_v1.py")
...
def stage_corpus(source_root: Path, corpus_root: Path, model_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for split, key, target in (
            ("fit", "fit/fineweb_edu", FIT_WINDOW_COUNT),
            ("assessment", "assessment/fineweb_edu", ASSESSMENT_WINDOW_COUNT),
        ):
            metadata = source_manifest["datasets"][key]
            rows = _read_jsonl(source_root / metadata["normalized_path"], f"normalized {key}")
        ...
        (staging / "staging-receipt.json").write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def _run_validator(result_root: Path, corpus_manifest_sha256: str, model_path: Path) -> dict[str, Any]:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            "independent FineWeb-Edu result validation failed: "
...
def run_campaign(source_root: Path, corpus_root: Path, output_root: Path, model_path: Path) -> dict[str, Any]:
    ...
    config = {
        ...
        "protocol": "fineweb-edu-bounded-1024-token-mechanics-v1",
    ...
    results = {
        ...
        "performance_result_is_local_fineweb_edu_bounded_pilot_only": True,
    }
    ...
    try:
        for name, value in (
            ...
            ("model-manifest.json", model_files_after),
        ):
...
def main() -> int:
    ...
    if args.pack_only:
        value = stage_corpus(args.source_root, args.corpus_root, args.model)
    else:
        value = run_campaign(args.source_root, args.corpus_root, args.output_root, args.model)

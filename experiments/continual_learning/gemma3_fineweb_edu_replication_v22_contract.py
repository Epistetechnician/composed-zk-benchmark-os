#!/usr/bin/env python3
"""V22 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v22"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV22"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v22-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v22-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v22-result"
)
...
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/215-gemma3-fineweb-edu-replication-v22-independent-review-2026-08-30.json"
)
PROTOCOL_SHA256 = "d52a99e412a95b2e8c88fd118fab75ff4262db1db1cef274ccb8c1402e52c0e3"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v22-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v22-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v22-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v22-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v22_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v22.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v22.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v22.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v22.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V22 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V22 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V22 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V22 implementation file")
        item = regular(path, "V22 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V22 review receipt")
    value = json.loads(regular(path, "V22 review receipt").read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("V22 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V22 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V22 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V22 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V22 review receipt binding mismatch")
    expected_keys = {
        "schema",
        "state_slice",
        "reviewer",
    ...
    if set(value) != expected_keys:
        raise ValueError("V22 review receipt schema is not closed")
    if type(value.get("effects_run")) is not bool or value["effects_run"] is not False:
        raise ValueError("V22 review effects_run must be a boolean false")
    ...
    if (
        ...
        raise ValueError("V22 receipt requires seven explicit true findings")
    if value.get("material_findings") != []:
        raise ValueError("V22 ACCEPT receipt must have an empty material_findings list")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": regular(PROTOCOL_PATH, "V22 protocol").read_bytes(),
        "packet": regular(PACKET_PATH, "V22 packet").read_bytes(),
        "receipt": regular(RECEIPT_PATH, "V22 receipt").read_bytes(),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V22 reviewed bytes or history changed")
    ...
    try:
        ...
        if final.exists():
            for name in reversed(moved):
                child = final / name
                if child.exists():
                    os.rename(child, staging / name)
            if not any(final.iterdir()):
                final.rmdir()
        raise
    staging.rmdir()

#!/usr/bin/env python3
"""Review-gated V28 corpus stage and offline Gemma3 runner.

State slice: continual-learning-gemma3-fineweb-edu-replication-v28.
No model or scientific effect is reachable until the V28 receipt validates.
"""
...
from experiments.continual_learning import (
    validate_gemma3_fineweb_edu_replication_v28 as v,
)

VALIDATOR = "experiments.continual_learning.validate_gemma3_fineweb_edu_replication_v28"
...
def _sandbox() -> None:
    if sys.platform != "darwin" or c.native_network_denied():
        return
    if os.environ.get("V28_RUN_REEXEC") == "1":
        raise RuntimeError("V28 native network denial could not be established")
    executable = shutil.which("sandbox-exec")
    if executable is None:
        raise RuntimeError("sandbox-exec is unavailable")
    env = os.environ.copy()
    env["V28_RUN_REEXEC"] = "1"
    os.execvpe(
        executable,
        [
            executable,
...
def parity(model: Any, windows: Sequence[v.Window]) -> dict[str, Any]:
    ...
    if len(checks) != 128 or not all(item["passed"] is True for item in checks):
        raise RuntimeError("V28 zero-alpha parity failed")
    return {
...
def _run_effects(
    ...
    if (
        ...
        raise RuntimeError("V28 input changed before effects")
    with c.network_block():
        model, tokenizer, runtime = load_runtime(model_path, receipt)
        if (
            getattr(model.args, "model_type", None) != "gemma3_text"
            or len(model.model.layers) != 26
        ):
            raise ValueError("V28 architecture mismatch")
        ...
        if (
            ...
            raise RuntimeError("V28 input changed during effects")
        ...
        if before_parameters != after_parameters or before_model != after_model:
            raise RuntimeError("V28 model custody changed")
    ...
    if not any(item["reached"] for item in reach):
        raise RuntimeError("V28 nonzero intervention reach failed")
    common = {
...
def stage_corpus(
    ...
    source, final, model = (
        c.exact_path(source_root, c.SOURCE_ROOT, "V28 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V28 corpus"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V28 corpus exists: {final}")
    ...
    if _reviewed_input(source, None, raw_root, prior_root, model) != input_snapshot:
        raise RuntimeError("V28 inputs changed before tokenizer")
    staging = Path(tempfile.mkdtemp(prefix=f".{final.name}.staging-", dir=final.parent))
    try:
        with c.network_block():
            tokenizer = load_tokenizer(model, receipt)
            entries = {}
        for split, count in (("fit", 64), ("assessment", 64)):
            selected = []
            for row in v.jsonl(source / f"{split}/fineweb_edu.jsonl", f"V28 {split}"):
                ...
                if (
                    ...
                    raise ValueError("V28 tokenizer round trip failed")
            ...
            if len(selected) != count:
                raise ValueError(f"V28 {split} has {len(selected)} eligible windows")
        ...
        _assert_review(review_snapshot)
        c.exact_file_set(staging, _expected_corpus_files(), "V28 corpus staging")
        with c.network_block():
            v.audit_corpus(
                staging,
                source,
        ...
        output = c.snapshot_files(
            staging, _expected_corpus_files(), "V28 corpus staging"
        )
        c.publish_no_replace(
            ...
            lambda: (
                ...
                if _reviewed_input(source, None, raw_root, prior_root, model) == input_snapshot
                    ...
                    RuntimeError("V28 corpus inputs changed after publication")
                ),
            ),
        )
        return {
            "manifest": _json(final / "manifest.json", "V28 corpus manifest"),
            "snapshot": output,
        }
    except Exception:
        if staging.exists():
...
def run_campaign(
    ...
    final, source, corpus, raw, prior, model = (
        c.exact_path(result_root, c.RESULT_ROOT, "V28 result"),
        c.exact_path(source_root, c.SOURCE_ROOT, "V28 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V28 corpus"),
        c.exact_path(raw_root, c.RAW_ROOT, "V28 raw"),
        c.exact_path(prior_root, c.PRIOR_ROOT, "V28 prior"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V28 result exists: {final}")
    review_snapshot = _review(receipt)
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V28 inputs changed during tokenizer load")
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V28 inputs changed before effects")
    staging = Path(tempfile.mkdtemp(prefix=f".{final.name}.staging-", dir=final.parent))
    try:
        ...
        c.exact_file_set(staging, expected, "V28 result staging")
        _assert_review(review_snapshot)
        current = _reviewed_input(source, corpus, raw, prior, model)
        if current != input_snapshot:
            raise RuntimeError("V28 inputs changed immediately before validator")
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"V28 independent validator failed:\n{completed.stdout}\n{completed.stderr}"
            )
        ...
        if (
            ...
            raise RuntimeError("V28 post-validator custody check failed")
        _validator_receipt(staging, validation)
        v.validate_validator_receipt(staging / "validator-receipt.json", validation)
        final_command = [
            argument for argument in command if argument != "--allow-missing-validator-receipt"
        ]
        ...
        if final_completed.returncode != 0:
            raise RuntimeError(
                f"V28 final independent validator failed:\n{final_completed.stdout}\n{final_completed.stderr}"
            )
        final_validation = json.loads(final_completed.stdout)
        if c.validation_binding(final_validation) != c.validation_binding(validation):
            raise RuntimeError("V28 final validator result mismatch")
        expected_with_validator = set(expected) | {"validator-receipt.json"}
        c.exact_file_set(staging, expected_with_validator, "V28 result staging")
        result_snapshot = final_validation.get("result_snapshot")
        if (
            not isinstance(result_snapshot, list)
            or c.snapshot_files(staging, expected_with_validator, "V28 result staging")
            != result_snapshot
        ):
            raise RuntimeError("V28 final validator snapshot mismatch")
        c.publish_no_replace(
            staging,
            final,
            ...
            lambda: (
                ...
                if _reviewed_input(source, corpus, raw, prior, model) == input_snapshot
                    ...
                    RuntimeError("V28 result inputs changed after publication")
                ),
            ),
            expected_snapshot=result_snapshot,
        )
        return {**payload, "validation": final_validation}
    except Exception:
        if staging.exists():
            shutil.rmtree(staging)
        raise

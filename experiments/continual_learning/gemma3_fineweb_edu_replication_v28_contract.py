#!/usr/bin/env python3
"""V28 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v28"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV28"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v28-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v28-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v28-result"
)
MODEL_PATH = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
MODEL_SNAPSHOT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v28-model-snapshot"
)
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/237-gemma3-fineweb-edu-replication-v28-protocol.md"
)
PACKET_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/238-gemma3-fineweb-edu-replication-v28-review-packet.md"
)
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/239-gemma3-fineweb-edu-replication-v28-independent-review-2026-08-31.json"
)
PROTOCOL_SHA256 = "d9c77afb6f10eed7a6a6bbd51a46d7412e3b221cf8472940ab00c531de2b9a60"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v28-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v28-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v28-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v28-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v28_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v28.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v28.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v28.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v28.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V28 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def materialize_model_snapshot(model_path: Path) -> Path:
    ...
    if snapshot.exists() or snapshot.is_symlink():
        if (
            ...
            raise ValueError("V28 model snapshot already exists with a different identity")
        _require_read_only_snapshot(snapshot, "V28 model snapshot")
        return snapshot
    reject_symlink_components(snapshot, "V28 model snapshot")
    staging = snapshot.parent / f".{snapshot.name}.staging-{os.getpid()}"
    if staging.exists() or staging.is_symlink():
        raise FileExistsError(f"V28 model snapshot staging exists: {staging}")
    staging.mkdir()
    published = False
    try:
        ...
        for relative in expected:
            ...
            with stable_binary_file(source, "V28 model source") as source_handle:
        ...
        staged_manifest = model_manifest_at(
            staging, "V28 model snapshot", MODEL_PATH.name
        )
        if staged_manifest != source_manifest:
            raise RuntimeError("V28 model snapshot digest mismatch")
        ...
        if (
            ...
            raise RuntimeError("V28 published model snapshot mismatch")
        return snapshot
    except Exception:
        if published and snapshot.exists():
            _remove_tree(snapshot, "V28 published model snapshot")
        if staging.exists():
            _remove_tree(staging, "V28 model snapshot staging")
        raise
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V28 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V28 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V28 implementation file")
        item = regular(path, "V28 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V28 review receipt")
    value = json.loads(
        stable_read_bytes(path, "V28 review receipt").decode("utf-8")
    )
    if not isinstance(value, dict):
        raise ValueError("V28 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V28 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V28 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V28 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V28 review receipt binding mismatch")
    expected_keys = {
        "schema",
        "state_slice",
        "reviewer",
    ...
    if set(value) != expected_keys:
        raise ValueError("V28 review receipt schema is not closed")
    if type(value.get("effects_run")) is not bool or value["effects_run"] is not False:
        raise ValueError("V28 review effects_run must be a boolean false")
    ...
    if (
        ...
        raise ValueError("V28 receipt requires seven explicit true findings")
    if value.get("material_findings") != []:
        raise ValueError("V28 ACCEPT receipt must have an empty material_findings list")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": stable_read_bytes(PROTOCOL_PATH, "V28 protocol"),
        "packet": stable_read_bytes(PACKET_PATH, "V28 packet"),
        "receipt": stable_read_bytes(RECEIPT_PATH, "V28 receipt"),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V28 reviewed bytes or history changed")

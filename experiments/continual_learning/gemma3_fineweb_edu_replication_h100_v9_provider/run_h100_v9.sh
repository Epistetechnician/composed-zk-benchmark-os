...
if [ "${H100_NETWORK_LOCK}" != "network-none-v9" ]; then
...
if [ ! -r /proc/net/dev ] || [ "$(awk -F: 'NF > 1 {gsub(/ /, "", $1); print $1}' /proc/net/dev | sort | tr '\n' ' ')" != "lo " ]; then
  echo "network namespace contains a non-loopback device or device proof unavailable" >&2
  exit 64
fi
if [ ! -r /proc/net/route ] || [ "$(awk 'NR > 1 && NF >= 4 {print}' /proc/net/route)" != "" ]; then
  echo "network namespace has an IPv4 route or IPv4 route proof unavailable" >&2
  exit 64
fi
if [ ! -r /proc/net/ipv6_route ] || [ "$(tr -d '[:space:]' </proc/net/ipv6_route)" != "" ]; then
  echo "network namespace has an IPv6 route or IPv6 route proof unavailable" >&2
  exit 64
fi
...
python -m experiments.continual_learning.validate_gemma3_fineweb_edu_replication_h100_v9 \
  --pre-effect \
...
python -m experiments.continual_learning.gemma3_fineweb_edu_replication_h100_v9 \
  --launch-manifest "${H100_LAUNCH_MANIFEST}" \
  --model-root "${H100_MODEL_ROOT}" \
  --raw-root "${H100_RAW_ROOT}" \
  --source-root "${H100_SOURCE_ROOT}" \
...
exec python -m experiments.continual_learning.validate_gemma3_fineweb_edu_replication_h100_v9 \
  --result-root "${H100_RESULT_ROOT}" \
  --raw-root "${H100_RAW_ROOT}" \
  --source-root "${H100_SOURCE_ROOT}" \
  --corpus-root "${H100_CORPUS_ROOT}" \

#!/usr/bin/env python3
"""Independent fail-closed V30 validator.
...
from experiments.continual_learning import (
    gemma3_fineweb_edu_replication_v30_contract as c,
)
...
def pinned_parquet(
    ...
    with c.stable_binary_file(path, label) as handle:
        ...
        if byte_len != item["byte_len"] or hasher.hexdigest() != item["sha256"]:
            raise ValueError("V30 Parquet pin mismatch on parse descriptor")
        handle.seek(0)
        yield handle
...
def parquet_rows(
    ...
    with pinned_parquet(path, item, "V30 Parquet") as handle:
...
def audit_raw(root: Path) -> list[dict[str, Any]]:
    root = c.exact_path(root, c.RAW_ROOT, "V30 raw")
    c.exact_file_set(
        root, {f"dataset/{item['path']}" for item in c.DATASET_FILES}, "V30 raw"
    )
    artifacts = []
    import pyarrow.parquet as pq

    for item in c.DATASET_FILES:
        path = c.regular(root / "dataset" / item["path"], "V30 Parquet")
        with pinned_parquet(path, item, "V30 Parquet") as handle:
            count = pq.ParquetFile(handle).metadata.num_rows
        if count < c.FRESH_ROW_END:
            raise ValueError("V30 raw shard is too short")
        artifacts.append(
            {
...
def audit_prior(root: Path, raw_root: Path) -> set[str]:
    root = c.exact_path(root, c.PRIOR_ROOT, "V30 prior")
    c.exact_file_set(
        root,
        {
    ...
    raw = c.exact_path(raw_root, c.RAW_ROOT, "V30 raw")
    for split, item in (
        ...
        for ordinal, (observed, recomputed) in enumerate(
            ...
            if (
                ...
                or strict_int(observed.get("source_row_index"), "V30 prior source row")
                != ordinal
            ):
    ...
    for item in c.DATASET_FILES:
        for row in parquet_rows(
            ...
            if not isinstance(document_id, str) or document_id in discarded_ids:
                raise ValueError("V30 discarded prior document identity mismatch")
...
def audit_source(
    ...
    root = c.exact_or_staging(root, c.SOURCE_ROOT, "V30 source")
    c.exact_file_set(
        root,
        {
            "acquisition-manifest.json",
    ...
    manifest = obj(root / "acquisition-manifest.json", "V30 source manifest")
    exact_keys(manifest, SOURCE_MANIFEST_KEYS, "V30 source manifest")
    required = {
        ...
        "selection_policy": "rows-18432-through-34815-two-pinned-crawls-v30",
    ...
    if any(manifest.get(key) != value for key, value in required.items()) or any(
        ...
        raise ValueError("V30 source contract mismatch")
    self_digest(manifest, "manifest_sha256", "V30 source manifest")
    raw = c.exact_path(raw_root, c.RAW_ROOT, "V30 raw")
    ...
    if (
        ...
        raise ValueError("V30 source custody mismatch")
    ids, split_rows = set(), {}
    datasets = manifest.get("datasets")
    if not isinstance(datasets, dict):
        raise ValueError("V30 source datasets missing")
    exact_keys(datasets, DATASET_KEYS, "V30 source datasets")
    for split, item in (
        ("fit", c.DATASET_FILES[0]),
        ("assessment", c.DATASET_FILES[1]),
    ):
        metadata = datasets.get(f"{split}/fineweb_edu")
        exact_keys(metadata, DATASET_METADATA_KEYS, "V30 source dataset metadata")
        expected = {
            "source": c.DATASET_SOURCE,
            "revision": c.DATASET_REVISION,
            "config": c.DATASET_CONFIG,
        ...
        if not isinstance(metadata, dict) or any(
            ...
            raise ValueError("V30 source dataset metadata mismatch")
        path = c.safe_relative(root, expected["normalized_path"], "V30 source JSONL")
        if c.sha256_file(path) != metadata.get(
            "normalized_sha256"
        ) or path.stat().st_size != metadata.get("normalized_byte_len"):
            raise ValueError("V30 source JSONL digest mismatch")
        rows = jsonl(path, "V30 source JSONL")
        if len(rows) != c.FRESH_ROW_COUNT:
            raise ValueError("V30 source row count mismatch")
        ...
        for ordinal, (row, recomputed) in enumerate(
            ...
            if (
                ...
                or strict_int(row.get("source_row_index"), "V30 fresh source row")
                != c.FRESH_ROW_START + ordinal
                or row.get("source_crawl") != item["crawl"]
                or row.get("source_path") != item["path"]
            ):
                raise ValueError("V30 source identity mismatch")
...
def parse_window(
    ...
    if (
        ...
        or strict_int(entry.get("window_ordinal"), "V30 window ordinal") != 0
        or strict_int(entry.get("token_count"), "V30 window token count")
        != c.WINDOW_TOKENS
    ):
        raise ValueError("V30 window metadata mismatch")
    path = c.safe_relative(root, entry.get("path"), "V30 window")
    raw = c.stable_read_bytes(path, "V30 window")
    text = raw.decode("utf-8")
    if len(raw) != strict_int(
        entry.get("byte_len"), "V30 window byte length"
    ) or hashlib.sha256(raw).hexdigest() != entry.get("text_sha256"):
        raise ValueError("V30 window bytes mismatch")
    token_ids = tuple(
    ...
    if len(token_ids) != c.WINDOW_TOKENS or tokenizer.decode(list(token_ids)) != text:
        raise ValueError("V30 tokenizer window mismatch")
    if expected_source_row is not None:
        source_text = expected_source_row.get("text")
        if not isinstance(source_text, str):
            raise ValueError("V30 source row text missing")
        ...
        if (
            ...
            raise ValueError("V30 window is not the source-row prefix")
    return Window(
        "fineweb_edu",
        entry["document_id"],
        entry["path"],
...
def audit_corpus(
    ...
    root = c.exact_or_staging(root, c.CORPUS_ROOT, "V30 corpus")
    expected = {
        "manifest.json",
        *{
    ...
    c.exact_file_set(root, expected, "V30 corpus")
    source = audit_source(source_root, raw_root, prior_root, review_sha)
    manifest = obj(root / "manifest.json", "V30 corpus manifest")
    exact_keys(manifest, CORPUS_MANIFEST_KEYS, "V30 corpus manifest")
    required = {
    ...
    if any(manifest.get(key) != value for key, value in required.items()) or any(
        ...
        raise ValueError("V30 corpus contract mismatch")
    self_digest(manifest, "manifest_sha256", "V30 corpus manifest")
    ...
    if (
        ...
        raise ValueError("V30 model pin mismatch")
    by_id = {
    ...
    for split in ("fit", "assessment"):
        ...
        if [entry.get("document_id") for entry in entries] != [
            ...
            strict_int(entry.get("source_row_index"), "V30 corpus source row")
            for entry in entries
        ] != [
            strict_int(row.get("source_row_index"), "V30 source row")
            for row in expected_rows
        ]:
            raise ValueError("V30 corpus is not the first eligible source sequence")
        parsed = []
        for entry in entries:
            exact_keys(entry, CORPUS_WINDOW_KEYS, "V30 corpus window")
            source_row = by_id.get(entry.get("document_id"))
            if source_row is None:
                raise ValueError("V30 corpus source document is unknown")
            window = parse_window(
                root, entry, tokenizer, split, expected_source_row=source_row
            )
            if (
                strict_int(entry.get("source_row_index"), "V30 corpus source row")
                != strict_int(source_row["source_row_index"], "V30 source row")
            ):
                raise ValueError("V30 corpus source binding mismatch")
    ...
    if set(w.document_id for w in windows["fit"]) & set(
        ...
        raise ValueError("V30 fit/assessment overlap")
    return {
        "manifest": manifest,
        "manifest_sha256": manifest["manifest_sha256"],
        "windows": windows,
...
def parity(value: Any, windows: list[Window]) -> dict[str, Any]:
    exact_keys(value, PARITY_KEYS, "V30 parity")
    strict_bool(value.get("all_passed"), "V30 parity all_passed")
    if (
        ...
        raise ValueError("V30 parity summary mismatch")
    ...
    if (
        ...
        raise ValueError("V30 parity checks mismatch")
    if [check.get("document_id") for check in checks] != [
        window.document_id for window in windows
    ]:
        raise ValueError("V30 parity check order mismatch")
    by_id = {window.document_id: window for window in windows}
    for check in checks:
        exact_keys(check, PARITY_CHECK_KEYS, "V30 parity check")
        if (
            ...
            or strict_int(check.get("token_count"), "V30 parity tokens")
            != c.WINDOW_TOKENS
            or check.get("tolerance") != c.PARITY_TOLERANCE
        ):
            raise ValueError("V30 parity identity mismatch")
        finite(check.get("max_abs_logit_delta"), "V30 parity delta")
        seen.add(check["document_id"])
    if seen != set(by_id) or finite(
        value.get("max_abs_logit_delta"), "V30 parity maximum"
    ) != max(check["max_abs_logit_delta"] for check in checks):
        raise ValueError("V30 parity aggregate mismatch")
    return value
...
def input_snapshot(
    ...
    result = {
        "source": c.snapshot_files(
            c.exact_path(source, c.SOURCE_ROOT, "V30 source"),
            {
                "acquisition-manifest.json",
                "fit/fineweb_edu.jsonl",
                "assessment/fineweb_edu.jsonl",
        ...
        "raw": c.snapshot_files(
            c.exact_path(raw, c.RAW_ROOT, "V30 raw"),
            {f"dataset/{item['path']}" for item in c.DATASET_FILES},
            "V30 raw",
        ),
        "prior": c.snapshot_files(
            c.exact_path(prior, c.PRIOR_ROOT, "V30 prior"),
            {
                "acquisition-manifest.json",
                "fit/fineweb_edu.jsonl",
                "assessment/fineweb_edu.jsonl",
    ...
    if corpus is not None:
        result["corpus"] = c.snapshot_files(
            c.exact_path(corpus, c.CORPUS_ROOT, "V30 corpus"),
            {
                "manifest.json",
                *{
                    f"{split}/window-{i:06d}.txt"
...
def validate_source(
    ...
    return audit_source(
        ...
        raw_root,
        prior_root,
        c.sha256_file(c.exact_path(receipt, c.RECEIPT_PATH, "V30 receipt")),
    ) | {
        "review_receipt_sha256": c.sha256_file(
            c.exact_path(receipt, c.RECEIPT_PATH, "V30 receipt")
        ),
...
def independent_load_runtime(
    ...
    if (
        ...
        raise RuntimeError("V30 independent model custody/runtime mismatch")
    ...
    if c.model_manifest_at(
        ...
        raise RuntimeError("V30 independent model snapshot mismatch")
...
class IndependentRecirculationConfig:
    ...
    def validate(self, layer_count: int) -> None:
        if (
            ...
            raise ValueError("V30 layer pair invalid")
        if (
            any(
                isinstance(value, bool)
                or not isinstance(value, (int, float))
            ...
            raise ValueError("V30 recirculation parameters invalid")
...
def load_tokenizer(model_path: Path, receipt: Path) -> Any:
    ...
    if (
        ...
        raise RuntimeError("V30 tokenizer custody/runtime mismatch")
    load_path = c.materialize_model_snapshot(model_path)
    if c.model_manifest_at(
        load_path,
        "V30 model snapshot",
        ...
        raise RuntimeError("V30 tokenizer model snapshot mismatch")
    c.require_native_network_denial()
...
def load_runtime(
    ...
    if (
        ...
        raise RuntimeError("V30 model custody/runtime mismatch")
    load_path = c.materialize_model_snapshot(model_path)
    if c.model_manifest_at(
        load_path,
        "V30 model snapshot",
        ...
        raise RuntimeError("V30 model snapshot mismatch")
    c.require_native_network_denial()
    from mlx_lm import load
        ...
        if config is None:
            ...
            for index, (layer, layer_cache) in enumerate(
                ...
                if index == config.source_layer:
                    current = hidden
            if current is None:
                raise RuntimeError("V30 source activation not captured")
...
def independent_evaluate_windows(
    ...
    if (
        ...
        raise ValueError("V30 temperature invalid")
    rows = []
    for window in windows:
        ...
        if tuple(ids) != window.token_ids or len(ids) != c.WINDOW_TOKENS:
            raise ValueError("V30 tokenizer changed")
        ...
        if not math.isfinite(total):
            raise ValueError("V30 NLL invalid")
        rows.append(
            {
...
def independent_parity(model: Any, windows: Sequence[Window]) -> dict[str, Any]:
    ...
    if len(checks) != 128 or not all(item["passed"] is True for item in checks):
        raise RuntimeError("V30 zero-alpha independent_parity failed")
    return {
...
def validate_validator_receipt(
    ...
    value = obj(path, "V30 validator receipt")
    exact_keys(value, VALIDATOR_RECEIPT_KEYS, "V30 validator receipt")
    self_digest(value, "receipt_sha256", "V30 validator receipt")
    for field in ("independent_recomputation", "validator_valid", "custody_recomputed"):
        strict_bool(value.get(field), f"V30 validator receipt {field}")
    expected = {
        "schema": "gemma3-fineweb-edu-replication-v30-validator-receipt",
        "state_slice": c.STATE_SLICE,
    ...
    if any(value.get(key) != expected_value for key, expected_value in expected.items()):
        raise ValueError("V30 validator receipt binding mismatch")
    return value
...
def validate_result(
    ...
    root = c.exact_or_staging(result_root, c.RESULT_ROOT, "V30 result")
    expected_files = {
        "config.json",
        "results.json",
        "receipt.json",
    ...
    initial_result_snapshot = c.snapshot_files(root, expected_files, "V30 result")
    c.exact_file_set(root, expected_files, "V30 result")
    review = c.validate_review_receipt(receipt)
    review_path = c.exact_path(receipt, c.RECEIPT_PATH, "V30 receipt")
    ...
    if c.digest(current_input) != invocation_snapshot_sha:
        raise ValueError("V30 invocation input snapshot mismatch")
    staged = c.regular(root / "review-receipt.json", "V30 staged receipt")
    staged_bytes = c.stable_read_bytes(staged, "V30 staged receipt")
    review_bytes = c.stable_read_bytes(review_path, "V30 review receipt")
    if staged_bytes != review_bytes:
        raise ValueError("V30 staged review receipt mismatch")
    source = audit_source(source_root, raw_root, prior_root, review_sha)
    corpus = audit_corpus(
        corpus_root,
        source_root,
    ...
    if (
        ...
        or c.stable_read_bytes(
            root / "corpus-manifest.json", "V30 result corpus manifest"
        )
        != c.stable_read_bytes(
            corpus_root / "manifest.json", "V30 corpus manifest"
        )
    ):
        raise ValueError("V30 corpus result binding mismatch")
    model = obj(root / "model-manifest.json", "V30 result model manifest")
    ...
    if (
        ...
        raise ValueError("V30 model result binding mismatch")
    config, results, result_receipt = (
        obj(root / name, f"V30 {name}")
        for name in ("config.json", "results.json", "receipt.json")
    )
    validator_receipt = (
        obj(root / "validator-receipt.json", "V30 validator receipt")
        if require_validator_receipt
        else None
    )
    exact_keys(config, CONFIG_RESULT_KEYS, "V30 config")
    exact_keys(results, RESULT_KEYS, "V30 results")
    exact_keys(result_receipt, RECEIPT_KEYS, "V30 result receipt")
    if require_validator_receipt:
        exact_keys(validator_receipt, VALIDATOR_RECEIPT_KEYS, "V30 validator receipt")
    for value, field, label in (
        (config, "config_sha256", "V30 config"),
        (results, "results_sha256", "V30 results"),
        (result_receipt, "receipt_sha256", "V30 result receipt"),
    ):
        self_digest(value, field, label)
    for value, label in (
        (config, "V30 config"),
        (results, "V30 results"),
        (result_receipt, "V30 result receipt"),
    ):
    ...
    if require_validator_receipt and (
        ...
        != "gemma3-fineweb-edu-replication-v30-validator-receipt"
        ...
        raise ValueError("V30 validator receipt mismatch")
    if (
        ...
        raise ValueError("V30 result receipt cross-binding mismatch")
    if any(
        ...
        raise ValueError("V30 result flags mismatch")
    locked_constants = {
    ...
    if (
        ...
        raise ValueError("V30 locked configuration mismatch")
    expected_config_values = {
        ...
        "selection_policy": "first-64-eligible-1024-token-windows-per-disjoint-v30-source-split",
    }
    if any(config.get(key) != value for key, value in expected_config_values.items()):
        raise ValueError("V30 declared configuration value mismatch")
    windows = corpus["windows"]
    fit_base = metrics(
        results.get("fit_baseline"), windows["fit"], "V30 fit baseline", 1.0, None
    )
    candidates = results.get("fit_candidates")
    if not isinstance(candidates, list) or len(candidates) != len(c.CANDIDATE_PAIRS):
        raise ValueError("V30 candidate set mismatch")
    candidate_means = []
    for candidate, pair in zip(candidates, c.CANDIDATE_PAIRS, strict=True):
        ...
        exact_keys(candidate, CANDIDATE_KEYS, "V30 candidate")
        exact_keys(candidate.get("config"), RECIRCULATION_CONFIG_KEYS, "V30 candidate config")
        if candidate.get("config") != cfg:
            raise ValueError("V30 candidate config mismatch")
        candidate_means.append(
            (
                metrics(
                    candidate.get("metrics"), windows["fit"], "V30 candidate", 1.0, cfg
                )["mean_nll"],
                pair[0],
                pair[1],
            )
    ...
    if (
        ...
        raise ValueError("V30 selection mismatch")
    assessment_base = metrics(
        results.get("assessment_baseline"),
        windows["assessment"],
        "V30 assessment baseline",
        1.0,
        None,
    )
    selected = metrics(
    ...
    if repeat != selected or results.get("deterministic_repeat_passed") is not True:
        raise ValueError("V30 repeat mismatch")
    parity_value = parity(
    ...
    if not isinstance(per_document, list) or len(per_document) != len(
        ...
        raise ValueError("V30 per-document count mismatch")
    if (
        ...
        raise ValueError("V30 per-document order mismatch")
    deltas, seen = [], set()
    ...
    for row in per_document:
        exact_keys(
            ...
            "V30 assessment_per_document row",
        )
        if (
            not isinstance(row, dict)
            ...
            or strict_int(row.get("window_ordinal"), "V30 retained window ordinal") != 0
            or strict_int(row.get("token_count"), "V30 retained token count")
            != c.WINDOW_TOKENS
            or strict_int(row.get("target_count"), "V30 retained targets")
            != c.WINDOW_TOKENS - 1
        ):
            raise ValueError("V30 per-document identity mismatch")
        ...
        if (
            ...
            raise ValueError("V30 per-document metric mismatch")
        finite(row.get("baseline_nll"), "V30 retained baseline NLL")
        finite(row.get("selected_nll"), "V30 retained selected NLL")
        finite(row["delta_selected_minus_baseline"], "V30 delta")
        deltas.append(row["delta_selected_minus_baseline"])
        seen.add(document_id)
    if seen != set(by_id):
        raise ValueError("V30 per-document set mismatch")
    ...
    if (
        ...
        raise ValueError("V30 uncertainty mismatch")
    reach_evidence = []
    ...
    if reached is not True:
        raise ValueError("V30 nonzero intervention reach was not proven")
    qualification = results.get("qualification")
    exact_keys(qualification, QUALIFICATION_KEYS, "V30 qualification")
    strict_bool(
        qualification.get("nonzero_intervention_reach"),
        "V30 qualification nonzero_intervention_reach",
    )
    for evidence in qualification.get("reach_evidence", []):
        exact_keys(evidence, REACH_KEYS, "V30 reach evidence")
        strict_bool(evidence.get("reached"), "V30 reach evidence reached")
    if (
        qualification
        ...
        raise ValueError("V30 reach was not independently derived")
    if results.get("controls") != {
        ...
        raise ValueError("V30 controls mismatch")
    for field in ("model_parameter_digest_before", "model_parameter_digest_after"):
        if (
            ...
            raise ValueError("V30 parameter binding mismatch")
    if (
        results["model_parameter_digest_before"]
        != results["model_parameter_digest_after"]
    ):
        raise ValueError("V30 model parameters changed")
    c.require_native_network_denial()
    if c.model_manifest(model_path) != current_model:
        raise ValueError("V30 model changed before independent recomputation")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V30 independent model shape/runtime mismatch")
        if (
            c.model_parameter_digest(model_value)
            != results["model_parameter_digest_before"]
        ):
            raise ValueError("V30 independent parameter-before mismatch")
        ...
        if (
            ...
            raise ValueError("V30 independent parity mismatch")
        if (
            independent_evaluate_windows(model_value, tokenizer_value, fit_windows, None)
            != fit_base
        ):
            raise ValueError("V30 independent fit baseline mismatch")
        ...
        if recomputed_candidates != candidates:
            raise ValueError("V30 independent candidate mismatch")
        ...
        if (
            ...
            raise ValueError("V30 independent assessment/control mismatch")
        if (
            c.model_parameter_digest(model_value)
            != results["model_parameter_digest_after"]
            or c.model_manifest(model_path) != current_model
        ):
            raise ValueError("V30 independent model custody-after mismatch")
    if (
        input_snapshot(source_root, corpus_root, raw_root, prior_root, model_path)
        != current_input
    ):
        raise ValueError("V30 inputs changed during independent recomputation")
    c.assert_code_snapshot(code_snapshot)
    result_snapshot = c.snapshot_files(root, expected_files, "V30 result")
    if result_snapshot != initial_result_snapshot:
        raise RuntimeError("V30 result bytes changed during independent validation")
    validation = {
        "valid": True,
        "state_slice": c.STATE_SLICE,
        "claim_ceiling": c.CLAIM_CEILING,

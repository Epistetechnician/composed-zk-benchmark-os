#!/usr/bin/env python3
"""Independent fail-closed V15 validator.
...
from experiments.continual_learning import (
    gemma3_fineweb_edu_replication_v15_contract as c,
)
...
def audit_raw(root: Path) -> list[dict[str, Any]]:
    root = c.exact_path(root, c.RAW_ROOT, "V15 raw")
    c.exact_file_set(
        root, {f"dataset/{item['path']}" for item in c.DATASET_FILES}, "V15 raw"
    )
    artifacts = []
    import pyarrow.parquet as pq

    for item in c.DATASET_FILES:
        path = c.regular(root / "dataset" / item["path"], "V15 Parquet")
        if (
            path.stat().st_size != item["byte_len"]
            or c.sha256_file(path) != item["sha256"]
        ):
            raise ValueError("V15 raw pin mismatch")
        count = pq.ParquetFile(path).metadata.num_rows
        if count < c.FRESH_ROW_END:
            raise ValueError("V15 raw shard is too short")
        artifacts.append(
            {
...
def audit_prior(root: Path, raw_root: Path) -> set[str]:
    root = c.exact_path(root, c.PRIOR_ROOT, "V15 prior")
    c.exact_file_set(
        root,
        {
    ...
    raw = c.exact_path(raw_root, c.RAW_ROOT, "V15 raw")
    for split, item in (
        ...
        for ordinal, (observed, recomputed) in enumerate(
            ...
            if (
                ...
                or strict_int(observed.get("source_row_index"), "V15 prior source row")
                != ordinal
            ):
...
def audit_source(
    ...
    root = c.exact_or_staging(root, c.SOURCE_ROOT, "V15 source")
    c.exact_file_set(
        root,
        {
            "acquisition-manifest.json",
    ...
    manifest = obj(root / "acquisition-manifest.json", "V15 source manifest")
    required = {
        ...
        "selection_policy": "rows-18432-through-34815-two-pinned-crawls-v15",
    ...
    if any(manifest.get(key) != value for key, value in required.items()) or any(
        ...
        raise ValueError("V15 source contract mismatch")
    self_digest(manifest, "manifest_sha256", "V15 source manifest")
    raw = c.exact_path(raw_root, c.RAW_ROOT, "V15 raw")
    ...
    if (
        ...
        raise ValueError("V15 source custody mismatch")
    ids, split_rows = set(), {}
    datasets = manifest.get("datasets")
    if not isinstance(datasets, dict):
        raise ValueError("V15 source datasets missing")
    for split, item in (
        ("fit", c.DATASET_FILES[0]),
        ("assessment", c.DATASET_FILES[1]),
    ):
        ...
        if not isinstance(metadata, dict) or any(
            ...
            raise ValueError("V15 source dataset metadata mismatch")
        path = c.safe_relative(root, expected["normalized_path"], "V15 source JSONL")
        if c.sha256_file(path) != metadata.get(
            "normalized_sha256"
        ) or path.stat().st_size != metadata.get("normalized_byte_len"):
            raise ValueError("V15 source JSONL digest mismatch")
        rows = jsonl(path, "V15 source JSONL")
        if len(rows) != c.FRESH_ROW_COUNT:
            raise ValueError("V15 source row count mismatch")
        ...
        for ordinal, (row, recomputed) in enumerate(
            ...
            if (
                ...
                or strict_int(row.get("source_row_index"), "V15 fresh source row")
                != c.FRESH_ROW_START + ordinal
                or row.get("source_crawl") != item["crawl"]
                or row.get("source_path") != item["path"]
            ):
                raise ValueError("V15 source identity mismatch")
...
def parse_window(
    ...
    if (
        ...
        or strict_int(entry.get("window_ordinal"), "V15 window ordinal") != 0
        or strict_int(entry.get("token_count"), "V15 window token count")
        != c.WINDOW_TOKENS
    ):
        raise ValueError("V15 window metadata mismatch")
    path = c.safe_relative(root, entry.get("path"), "V15 window")
    raw = path.read_bytes()
    text = raw.decode("utf-8")
    if len(raw) != strict_int(
        entry.get("byte_len"), "V15 window byte length"
    ) or hashlib.sha256(raw).hexdigest() != entry.get("text_sha256"):
        raise ValueError("V15 window bytes mismatch")
    token_ids = tuple(
    ...
    if len(token_ids) != c.WINDOW_TOKENS or tokenizer.decode(list(token_ids)) != text:
        raise ValueError("V15 tokenizer window mismatch")
    if expected_source_row is not None:
        source_text = expected_source_row.get("text")
        if not isinstance(source_text, str):
            raise ValueError("V15 source row text missing")
        ...
        if (
            ...
            raise ValueError("V15 window is not the source-row prefix")
    return Window(
        "fineweb_edu",
        entry["document_id"],
        entry["path"],
...
def audit_corpus(
    ...
    root = c.exact_or_staging(root, c.CORPUS_ROOT, "V15 corpus")
    expected = {
        "manifest.json",
        *{
    ...
    c.exact_file_set(root, expected, "V15 corpus")
    source = audit_source(source_root, raw_root, prior_root, review_sha)
    manifest = obj(root / "manifest.json", "V15 corpus manifest")
    required = {
    ...
    if any(manifest.get(key) != value for key, value in required.items()) or any(
        ...
        raise ValueError("V15 corpus contract mismatch")
    self_digest(manifest, "manifest_sha256", "V15 corpus manifest")
    ...
    if (
        ...
        raise ValueError("V15 model pin mismatch")
    by_id = {
    ...
    for split in ("fit", "assessment"):
        ...
        if [entry.get("document_id") for entry in entries] != [
            ...
            strict_int(entry.get("source_row_index"), "V15 corpus source row")
            for entry in entries
        ] != [
            strict_int(row.get("source_row_index"), "V15 source row")
            for row in expected_rows
        ]:
            raise ValueError("V15 corpus is not the first eligible source sequence")
        parsed = []
        for entry in entries:
            source_row = by_id.get(entry.get("document_id"))
            if source_row is None:
                raise ValueError("V15 corpus source document is unknown")
            window = parse_window(
                root, entry, tokenizer, split, expected_source_row=source_row
            )
            if (
                strict_int(entry.get("source_row_index"), "V15 corpus source row")
                != strict_int(source_row["source_row_index"], "V15 source row")
            ):
                raise ValueError("V15 corpus source binding mismatch")
    ...
    if set(w.document_id for w in windows["fit"]) & set(
        ...
        raise ValueError("V15 fit/assessment overlap")
    return {
        "manifest": manifest,
        "manifest_sha256": manifest["manifest_sha256"],
        "windows": windows,
...
def parity(value: Any, windows: list[Window]) -> dict[str, Any]:
    if (
        ...
        raise ValueError("V15 parity summary mismatch")
    ...
    if (
        ...
        raise ValueError("V15 parity checks mismatch")
    if [check.get("document_id") for check in checks] != [
        window.document_id for window in windows
    ]:
        raise ValueError("V15 parity check order mismatch")
    by_id = {window.document_id: window for window in windows}
    for check in checks:
        if (
            ...
            or strict_int(check.get("token_count"), "V15 parity tokens")
            != c.WINDOW_TOKENS
            or check.get("tolerance") != c.PARITY_TOLERANCE
        ):
            raise ValueError("V15 parity identity mismatch")
        finite(check.get("max_abs_logit_delta"), "V15 parity delta")
        seen.add(check["document_id"])
    if seen != set(by_id) or finite(
        value.get("max_abs_logit_delta"), "V15 parity maximum"
    ) != max(check["max_abs_logit_delta"] for check in checks):
        raise ValueError("V15 parity aggregate mismatch")
    return value
...
def input_snapshot(
    ...
    result = {
        "source": c.snapshot_files(
            c.exact_path(source, c.SOURCE_ROOT, "V15 source"),
            {
                "acquisition-manifest.json",
                "fit/fineweb_edu.jsonl",
                "assessment/fineweb_edu.jsonl",
        ...
        "raw": c.snapshot_files(
            c.exact_path(raw, c.RAW_ROOT, "V15 raw"),
            {f"dataset/{item['path']}" for item in c.DATASET_FILES},
            "V15 raw",
        ),
        "prior": c.snapshot_files(
            c.exact_path(prior, c.PRIOR_ROOT, "V15 prior"),
            {
                "acquisition-manifest.json",
                "fit/fineweb_edu.jsonl",
                "assessment/fineweb_edu.jsonl",
    ...
    if corpus is not None:
        result["corpus"] = c.snapshot_files(
            c.exact_path(corpus, c.CORPUS_ROOT, "V15 corpus"),
            {
                "manifest.json",
                *{
                    f"{split}/window-{i:06d}.txt"
...
def validate_source(
    ...
    return audit_source(
        ...
        raw_root,
        prior_root,
        c.sha256_file(c.exact_path(receipt, c.RECEIPT_PATH, "V15 receipt")),
    ) | {
        "review_receipt_sha256": c.sha256_file(
            c.exact_path(receipt, c.RECEIPT_PATH, "V15 receipt")
        ),
...
def validate_result(
    ...
    root = c.exact_or_staging(result_root, c.RESULT_ROOT, "V15 result")
    expected_files = {
        "config.json",
        "results.json",
        "receipt.json",
    ...
    c.exact_file_set(root, expected_files, "V15 result")
    review = c.validate_review_receipt(receipt)
    review_path = c.exact_path(receipt, c.RECEIPT_PATH, "V15 receipt")
    ...
    if c.digest(current_input) != invocation_snapshot_sha:
        raise ValueError("V15 invocation input snapshot mismatch")
    staged = c.regular(root / "review-receipt.json", "V15 staged receipt")
    if staged.read_bytes() != review_path.read_bytes():
        raise ValueError("V15 staged review receipt mismatch")
    source = audit_source(source_root, raw_root, prior_root, review_sha)
    corpus = audit_corpus(
        corpus_root,
        source_root,
    ...
    if (
        ...
        raise ValueError("V15 corpus result binding mismatch")
    model = obj(root / "model-manifest.json", "V15 result model manifest")
    ...
    if (
        ...
        raise ValueError("V15 model result binding mismatch")
    config, results, result_receipt, validator_receipt = (
        obj(root / name, f"V15 {name}")
        for name in (
            "config.json",
            "results.json",
            "receipt.json",
    ...
    for value, field, label in (
        (config, "config_sha256", "V15 config"),
        (results, "results_sha256", "V15 results"),
        (result_receipt, "receipt_sha256", "V15 result receipt"),
        (validator_receipt, "receipt_sha256", "V15 validator receipt"),
    ):
        self_digest(value, field, label)
    for value, label in (
        (config, "V15 config"),
        (results, "V15 results"),
        (result_receipt, "V15 result receipt"),
    ):
        common(value, label, review_sha, source["manifest_sha256"], corpus_sha)
    if (
        validator_receipt.get("schema")
        != "gemma3-fineweb-edu-replication-v15-validator-receipt"
        ...
        raise ValueError("V15 validator receipt mismatch")
    if (
        ...
        raise ValueError("V15 result receipt cross-binding mismatch")
    if any(
        ...
        raise ValueError("V15 result flags mismatch")
    locked_constants = {
    ...
    if any(
        ...
        raise ValueError("V15 locked configuration mismatch")
    windows = corpus["windows"]
    fit_base = metrics(
        results.get("fit_baseline"), windows["fit"], "V15 fit baseline", 1.0, None
    )
    candidates = results.get("fit_candidates")
    if not isinstance(candidates, list) or len(candidates) != len(c.CANDIDATE_PAIRS):
        raise ValueError("V15 candidate set mismatch")
    candidate_means = []
    for candidate, pair in zip(candidates, c.CANDIDATE_PAIRS, strict=True):
        ...
        if not isinstance(candidate, dict) or candidate.get("config") != cfg:
            raise ValueError("V15 candidate config mismatch")
        candidate_means.append(
            (
                metrics(
                    candidate.get("metrics"), windows["fit"], "V15 candidate", 1.0, cfg
                )["mean_nll"],
                pair[0],
                pair[1],
            )
    ...
    if (
        ...
        raise ValueError("V15 selection mismatch")
    assessment_base = metrics(
        results.get("assessment_baseline"),
        windows["assessment"],
        "V15 assessment baseline",
        1.0,
        None,
    )
    selected = metrics(
    ...
    if repeat != selected or results.get("deterministic_repeat_passed") is not True:
        raise ValueError("V15 repeat mismatch")
    parity_value = parity(
    ...
    if not isinstance(per_document, list) or len(per_document) != len(
        ...
        raise ValueError("V15 per-document count mismatch")
    if (
        ...
        raise ValueError("V15 per-document order mismatch")
    deltas, seen = [], set()
    ...
    for row in per_document:
        if (
            ...
            or strict_int(row.get("window_ordinal"), "V15 retained window ordinal") != 0
            or strict_int(row.get("token_count"), "V15 retained token count")
            != c.WINDOW_TOKENS
            or strict_int(row.get("target_count"), "V15 retained targets")
            != c.WINDOW_TOKENS - 1
        ):
            raise ValueError("V15 per-document identity mismatch")
        ...
        if (
            ...
            raise ValueError("V15 per-document metric mismatch")
        finite(row.get("baseline_nll"), "V15 retained baseline NLL")
        finite(row.get("selected_nll"), "V15 retained selected NLL")
        finite(row["delta_selected_minus_baseline"], "V15 delta")
        deltas.append(row["delta_selected_minus_baseline"])
        seen.add(document_id)
    if seen != set(by_id):
        raise ValueError("V15 per-document set mismatch")
    ...
    if (
        ...
        raise ValueError("V15 uncertainty mismatch")
    reach_evidence = []
    ...
    if (
        ...
        raise ValueError("V15 reach was not independently derived")
    if results.get("controls") != {
        ...
        raise ValueError("V15 controls mismatch")
    for field in ("model_parameter_digest_before", "model_parameter_digest_after"):
        if (
            ...
            raise ValueError("V15 parameter binding mismatch")
    if (
        results["model_parameter_digest_before"]
        != results["model_parameter_digest_after"]
    ):
        raise ValueError("V15 model parameters changed")
    c.require_native_network_denial()
    from experiments.continual_learning import (
        stage_and_run_gemma3_fineweb_edu_replication_v15 as runner,
    )

    if c.model_manifest(model_path) != current_model:
        raise ValueError("V15 model changed before independent recomputation")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V15 independent model shape/runtime mismatch")
        if (
            c.model_parameter_digest(model_value)
            != results["model_parameter_digest_before"]
        ):
            raise ValueError("V15 independent parameter-before mismatch")
        ...
        if (
            ...
            raise ValueError("V15 independent parity mismatch")
        if (
            runner.evaluate_windows(model_value, tokenizer_value, fit_windows, None)
            != fit_base
        ):
            raise ValueError("V15 independent fit baseline mismatch")
        ...
        if recomputed_candidates != candidates:
            raise ValueError("V15 independent candidate mismatch")
        ...
        if (
            ...
            raise ValueError("V15 independent assessment/control mismatch")
        if (
            c.model_parameter_digest(model_value)
            != results["model_parameter_digest_after"]
            or c.model_manifest(model_path) != current_model
        ):
            raise ValueError("V15 independent model custody-after mismatch")
    if (
        input_snapshot(source_root, corpus_root, raw_root, prior_root, model_path)
        != current_input
    ):
        raise ValueError("V15 inputs changed during independent recomputation")
...
def _tokenizer_for_validation(model_path: Path) -> Any:
    ...
    if (
        ...
        raise RuntimeError("V15 tokenizer custody/runtime mismatch")

#!/usr/bin/env python3
"""Independent validator for one V40 fresh-task acquisition case."""
...
def metric_shape(metric: dict) -> None:
...
def validate(root: Path, model: Path, expected_task_seed: int) -> dict:
    ...
    if expected_task_seed not in TASK_SEEDS:
        raise ValueError("task seed outside fixed V40 set")

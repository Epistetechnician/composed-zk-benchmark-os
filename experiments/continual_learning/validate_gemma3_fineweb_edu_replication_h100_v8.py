...
State slice: continual-learning-gemma3-fineweb-edu-replication-h100-v8.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-h100-v8"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-h100-v8-result"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-h100-v8-corpus"
...
REVIEWED_FILES = (
    "docs/research/continual-learning/320-gemma3-fineweb-edu-replication-h100-v8-protocol.md",
    "docs/research/continual-learning/321-gemma3-fineweb-edu-replication-h100-v8-review-packet.md",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v8_preflight.py",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v8.py",
    "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_h100_v8.py",
    "experiments/continual_learning/pack_gemma3_fineweb_edu_replication_h100_v8.py",
    "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_h100_v8_preflight.py",
    "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_h100_v8.py",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v8_provider/Dockerfile",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v8_provider/requirements.lock",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v8_provider/runtime-lock.json",
    "experiments/continual_learning/gemma3_fineweb_edu_replication_h100_v8_provider/run_h100_v8.sh",
    "AGENTS.md",
    "docs/research/continual-learning/322-gemma3-fineweb-edu-replication-h100-v8-implementation-manifest.json",
)
IMPLEMENTATION_FILES = tuple(
    path for path in REVIEWED_FILES
    if path != "docs/research/continual-learning/322-gemma3-fineweb-edu-replication-h100-v8-implementation-manifest.json"
)
REVIEW_RECEIPT_KEYS = {
...
PROVIDER_RECEIPT_SCHEMA = "gemma3-fineweb-edu-replication-h100-v8-provider-receipt"
...
FRESH_START, FRESH_END = 100_352, 116_736
EXCLUSION_RANGES = (("prior-pilot", 0, 2_048), ("prior-v31", 2_048, 18_432), ("discarded", 18_432, 34_816), ("prior-h100-v1", 34_816, 51_200), ("prior-h100-v3", 51_200, 67_584), ("prior-h100-v4", 67_584, 83_968), ("prior-h100-v5", 83_968, FRESH_START))
...
def validate_provider_receipt(path: Path, launch: dict[str, Any], launch_sha256: str) -> dict[str, Any]:
    ...
    if (stop - start).total_seconds() > finite(launch["max_runtime_minutes"], "launch maximum runtime") * 60:
    ...
    quote = exact_decimal(receipt["quoted_gpu_usd_per_minute"], "provider quote")
    charged = exact_decimal(receipt["charged_usd"], "provider charged USD")
    estimate = exact_decimal(receipt["estimated_max_total_usd"], "provider estimated USD")
    ceiling = exact_decimal(receipt["hard_usd_ceiling"], "provider hard USD ceiling")
    launch_quote = exact_decimal(launch["quoted_gpu_usd_per_minute"], "launch quote")
    launch_estimate = exact_decimal(launch["estimated_max_total_usd"], "launch estimated USD")
    launch_ceiling = exact_decimal(launch["hard_usd_ceiling"], "launch hard USD ceiling")
...
def source_bundle(raw_root: Path, source_root: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if set(manifest) != required or manifest["schema"] != "gemma3-fineweb-edu-replication-h100-v8-source" or manifest["state_slice"] != STATE_SLICE or manifest["dataset"] != {"repo": DATASET_REPO, "source": DATASET_SOURCE, "revision": DATASET_REVISION, "config": DATASET_CONFIG, "split": DATASET_SPLIT} or manifest["raw_root"] != "raw" or manifest["fresh_row_range"] != {"start": FRESH_START, "end_exclusive": FRESH_END, "count_per_shard": FRESH_END - FRESH_START} or manifest["excluded_ranges"] != [{"name": n, "start": s, "end_exclusive": e, "count_per_shard": e - s} for n, s, e in EXCLUSION_RANGES] or any(manifest[name] is not False for name in ("network_access", "training", "scientific_execution", "evidence_ledger_mutation")) or manifest["manifest_sha256"] != digest(manifest, "manifest_sha256"):
...
def validate_tree_manifest(root: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if set(manifest) != {"schema", "model_id", "model_revision", "architecture", "files", "manifest_sha256"} or manifest["schema"] != "gemma3-model-manifest-v8":
...
def validate_launch_binding(path: Path, repo_root: Path, result: dict[str, Any]) -> None:
    ...
    if set(launch) != LAUNCH_KEYS or launch.get("schema") != "gemma3-fineweb-edu-replication-h100-v8-launch-manifest" or launch.get("state_slice") != STATE_SLICE:
    ...
    ceiling = exact_decimal(launch.get("hard_usd_ceiling"), "launch hard USD ceiling")
    estimate = exact_decimal(launch.get("estimated_max_total_usd"), "launch estimated USD")
    rate = exact_decimal(launch.get("quoted_gpu_usd_per_minute"), "launch quote")
    ...
    if launch.get("runner_entrypoint") != "run_h100_v8.sh" or launch.get("launch_command") != "./run_h100_v8.sh" or launch.get("network_lock") != "network-none-v8" or launch.get("launch_command_sha256") != hashlib.sha256(b"./run_h100_v8.sh").hexdigest():
    ...
    if (
        ...
        or exact_decimal(launch["hard_usd_ceiling"], "launch hard USD ceiling")
        != exact_decimal(result["hard_usd_ceiling"], "result hard USD ceiling")
        or exact_decimal(launch["estimated_max_total_usd"], "launch estimated USD")
        != exact_decimal(result["estimated_max_total_usd"], "result estimated USD")
    ):
    ...
    protocol = repo_root / "docs/research/continual-learning/320-gemma3-fineweb-edu-replication-h100-v8-protocol.md"
    packet = repo_root / "docs/research/continual-learning/321-gemma3-fineweb-edu-replication-h100-v8-review-packet.md"
    ...
    if set(review) != REVIEW_RECEIPT_KEYS or review.get("schema") != "gemma3-fineweb-edu-replication-h100-v8-independent-review":
    ...
    if implementation.get("schema") != "gemma3-fineweb-edu-replication-h100-v8-implementation" or implementation.get("state_slice") != STATE_SLICE or [item.get("path") for item in implementation.get("files", [])] != list(IMPLEMENTATION_FILES):
...
def metric(value: Any, windows: dict[str, dict[str, Any]], expected_temperature: float, expected_config: dict[str, Any] | None, label: str) -> None:
    ...
    mean = finite(value["mean_nll"], f"{label} mean nll")
    if mean < 0:
        raise ValueError(f"{label} NLL must be nonnegative")
    expected_mean = round(mean, 9)
...
def validate_assessment_ledger(path: Path, result: dict[str, Any], assessment: dict[str, dict[str, Any]]) -> list[float]:
    ...
    for index, line_bytes in enumerate(payload.splitlines()):
        ...
        baseline = finite(value["baseline_nll"], "ledger baseline NLL")
        selected = finite(value["selected_nll"], "ledger selected NLL")
        delta = finite(value["delta_selected_minus_baseline"], "ledger delta")
    ...
    baseline_mean = round(baseline_total / (WINDOW_COUNT * (WINDOW_TOKENS - 1)), 9)
    selected_mean = round(selected_total / (WINDOW_COUNT * (WINDOW_TOKENS - 1)), 9)
    if baseline_mean != result["assessment_baseline"]["mean_nll"] or selected_mean != result["assessment_selected"]["mean_nll"]:
...
def validate_pre_effect_bundle(raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    from experiments.continual_learning import gemma3_fineweb_edu_replication_h100_v8_preflight as preflight
...
def validate(result_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    for field, value in provider_fields.items():
        if field == "provider_charged_usd":
            if exact_decimal(result[field], field) != exact_decimal(value, field):
    ...
    if set(receipt) != {"schema", "state_slice", "result_sha256", "result_file_sha256", "receipt_sha256"} or receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v8-result-receipt" or receipt["state_slice"] != STATE_SLICE or receipt["result_sha256"] != result["results_sha256"] or receipt["result_file_sha256"] != sha256_file(result_root / "result.json") or receipt["receipt_sha256"] != digest(receipt, "receipt_sha256"):
    ...
    if exact_decimal(result["hard_usd_ceiling"], "hard USD ceiling") != HARD_USD_CEILING or exact_decimal(result["estimated_max_total_usd"], "estimated total USD") <= 0:
    ...
    if any(result[key] != value for key, value in expected_paths.items()):
        raise ValueError("result custody paths are not launch-bound")
    ...
    if (
        ...
        or runtime["network"] != "offline-process-block-v8"

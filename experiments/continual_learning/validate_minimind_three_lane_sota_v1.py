#!/usr/bin/env python3
"""Independent aggregate validator for MiniMind three-lane V1.
...
def _validate_trust_and_receipt(receipt_path: Path, source_sha: str, corpus_sha: str) -> str:
    ...
    _require(bundle["state_slice"] == STATE_SLICE and bundle["schema_version"] == "minimind-three-lane-sota-trust-bundle-v1", "trust bundle identity mismatch")
...
def _validate_source_manifest(source: Mapping[str, Any]) -> None:
    ...
    _require(set(source) == expected and source["schema_version"] == "minimind-three-lane-source-manifest-v1" and source["state_slice"] == STATE_SLICE, "source manifest schema mismatch")
...
def _validate_corpus_manifest(corpus: Mapping[str, Any]) -> None:
    ...
    _require(set(corpus) == expected and corpus["schema_version"] == "minimind-three-lane-corpus-manifest-v1" and corpus["state_slice"] == STATE_SLICE, "corpus manifest schema mismatch")
...
def _validate_campaign(root: Path, kind: str) -> dict[str, Any]:
    ...
    for lane in LANES:
        ...
        for arm in expected_arms[lane]:
            for split in SPLITS:
                expected = {(lane, arm, split, seed, order_seed, direction) for seed, order_seed in zip(REPLICATE_SEEDS, ORDER_SEEDS) for direction in ORDER_DIRECTIONS} if kind == "synthetic" else {(lane, arm, split, 2026090201, 7301, "forward")}
...
def main() -> int:
    if len(sys.argv) != 3 or sys.argv[1] not in {"synthetic", "model"}:
        raise SystemExit("usage: validate_minimind_three_lane_sota_v1.py synthetic|model ARTIFACT_ROOT")
    try:

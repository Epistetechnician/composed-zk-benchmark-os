...
EXCLUSION_RANGES = (
    ...
    ("prior-h100-v5", 83_968, 100_352), ("prior-h100-v8", 100_352, 116_736),
    ("prior-h100-v7", 116_736, FRESH_START),
)
RANGED_EXCLUSION_IDENTITIES = ("v7", "v8")
EXCLUDED_PROTOCOL_IDENTITIES = ("v6", "v7", "v8")
EXCLUDED_PROTOCOL_BOUNDARIES = (
    ("v6", 100_352, 116_736),
    ("v7", 116_736, FRESH_START),
    ("v8", 100_352, 116_736),
)
if any(
    f"prior-h100-{identity}" not in {name for name, _start, _end in EXCLUSION_RANGES}
    for identity in RANGED_EXCLUSION_IDENTITIES
):
    raise RuntimeError("V7/V8 exclusion identities are not bound to ranges")
...
def verify_provider_attestation(receipt: dict[str, Any], launch: dict[str, Any]) -> None:
    ...
    try:
        ...
        if len(root_key_bytes) != 32 or len(public_key) != 32 or len(signature) != 64 or len(certificate) != 64:
            raise ValueError("provider attestation key or signature length is invalid")
...
def validate_provider_receipt(path: Path, launch: dict[str, Any], launch_sha256: str) -> dict[str, Any]:
    ...
    if elapsed_minutes > exact_decimal(launch["max_runtime_minutes"], "launch maximum runtime"):
    ...
    launch_quote = parse_usd_canonical(launch["quoted_gpu_usd_per_minute"], "launch quote")
    launch_estimate = parse_usd_canonical(launch["estimated_max_total_usd"], "launch estimated USD")
    launch_ceiling = parse_usd_canonical(launch["hard_usd_ceiling"], "launch hard USD ceiling")
    if charged < 0 or estimate <= 0:
    ...
    if quote != launch_quote:
        raise ValueError("provider quote does not match launch manifest")
    if ceiling != launch_ceiling or ceiling != HARD_USD_CEILING:
        raise ValueError("provider ceiling does not match launch manifest")
    if estimate != launch_estimate:
        raise ValueError("provider estimate does not match launch manifest")
    if quote * exact_decimal(launch["max_runtime_minutes"], "launch maximum runtime") != estimate or estimate > ceiling or charged > estimate or charged > ceiling:
...
def source_bundle(raw_root: Path, source_root: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    required = {"schema", "state_slice", "dataset", "raw_root", "raw_artifacts", "fresh_row_range", "split_sources", "excluded_ranges", "excluded_protocol_identities", "excluded_protocol_boundaries", "excluded_id_sha256", "network_access", "training", "scientific_execution", "evidence_ledger_mutation", "manifest_sha256"}
    if set(manifest) != required or manifest["schema"] != "gemma3-fineweb-edu-replication-h100-v9-source" or manifest["state_slice"] != STATE_SLICE or manifest["dataset"] != {"repo": DATASET_REPO, "source": DATASET_SOURCE, "revision": DATASET_REVISION, "config": DATASET_CONFIG, "split": DATASET_SPLIT} or manifest["raw_root"] != "raw" or manifest["fresh_row_range"] != {"start": FRESH_START, "end_exclusive": FRESH_END, "count_per_shard": FRESH_END - FRESH_START} or manifest["excluded_ranges"] != [{"name": n, "start": s, "end_exclusive": e, "count_per_shard": e - s} for n, s, e in EXCLUSION_RANGES] or manifest["excluded_protocol_identities"] != [f"prior-h100-{identity}" for identity in EXCLUDED_PROTOCOL_IDENTITIES] or manifest["excluded_protocol_boundaries"] != [{"identity": identity, "start": start, "end_exclusive": end} for identity, start, end in EXCLUDED_PROTOCOL_BOUNDARIES] or any(manifest[name] is not False for name in ("network_access", "training", "scientific_execution", "evidence_ledger_mutation")) or manifest["manifest_sha256"] != digest(manifest, "manifest_sha256"):
...
def validate_tree_manifest(root: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if set(manifest) != {"schema", "model_id", "model_revision", "architecture", "files", "manifest_sha256"} or manifest["schema"] != "gemma3-model-manifest-v9":
        raise ValueError("model manifest schema mismatch")
    if manifest["model_id"] != MODEL_ID or manifest["architecture"] != MODEL_ARCHITECTURE or re.fullmatch(r"[0-9a-f]{40}", manifest["model_revision"]) is None:
        raise ValueError("model manifest model identity mismatch")
    ...
    config = obj(config_path, "model config")
    if config.get("model_type") != MODEL_CONFIG_TYPE or MODEL_ARCHITECTURE not in config.get("architectures", []):
        raise ValueError("model architecture identity mismatch")
    return manifest


def validate_launch_binding(path: Path, repo_root: Path, result: dict[str, Any]) -> None:
    launch = obj(path, "launch manifest")
    if set(launch) != LAUNCH_KEYS or launch.get("schema") != "gemma3-fineweb-edu-replication-h100-v9-launch-manifest" or launch.get("state_slice") != STATE_SLICE:
    ...
    if launch.get("model_id") != MODEL_ID or launch.get("model_architecture") != MODEL_ARCHITECTURE or re.fullmatch(r"[0-9a-f]{40}", launch.get("model_revision", "")) is None:
        raise ValueError("launch model identity is not exact and revision-pinned")
    if not isinstance(launch.get("provider_attestation_key_id"), str) or not launch["provider_attestation_key_id"].strip():
        raise ValueError("launch provider attestation key id is missing")
    if re.fullmatch(r"[A-Za-z0-9._:-]+", launch.get("provider_attestation_key_id", "")) is None or re.fullmatch(r"[A-Za-z0-9._:-]+", launch.get("provider_trust_root_id", "")) is None:
        raise ValueError("launch provider trust identity is invalid")
    try:
        if len(base64.b64decode(launch["provider_trust_root_public_key"], validate=True)) != 32:
            raise ValueError("launch provider trust-root public key length is invalid")
    except (ValueError, TypeError) as error:
        raise ValueError("launch provider trust-root public key is invalid") from error
    registry_path = require_immutable_external_file(
        Path(launch["provider_trust_root_registry_path"]),
        repo_root,
        "launch provider trust-root registry",
    )
    hex_digest(launch["provider_trust_root_registry_sha256"], "provider trust-root registry digest")
    validate_provider_trust_root_registry(registry_path, launch)
    ...
    ceiling = parse_usd_canonical(launch.get("hard_usd_ceiling"), "launch hard USD ceiling")
    estimate = parse_usd_canonical(launch.get("estimated_max_total_usd"), "launch estimated USD")
    rate = parse_usd_canonical(launch.get("quoted_gpu_usd_per_minute"), "launch quote")
    minutes = exact_decimal(launch.get("max_runtime_minutes"), "launch maximum runtime")
    if ceiling != HARD_USD_CEILING or min(ceiling, estimate, rate, minutes) <= 0 or rate * minutes != estimate or estimate > ceiling:
        raise ValueError("launch budget arithmetic invalid")
    if launch.get("runner_entrypoint") != "run_h100_v9.sh" or launch.get("launch_command") != "./run_h100_v9.sh" or launch.get("network_lock") != "network-none-v9" or launch.get("launch_command_sha256") != hashlib.sha256(b"./run_h100_v9.sh").hexdigest():
    ...
    for field in ("protocol_sha256", "packet_sha256", "review_receipt_sha256", "implementation_manifest_sha256", "code_bundle_sha256", "runtime_lock_sha256", "data_manifest_sha256", "source_manifest_sha256"):
    ...
    code_bundle_path = require_immutable_external_file(
        Path(launch["code_bundle_path"]),
        repo_root,
        "launch code bundle",
    )
    if sha256_file(code_bundle_path) != launch["code_bundle_sha256"]:
        raise ValueError("launch code bundle digest does not match code_bundle_path")
    if (
        launch["manifest_sha256"] != result["launch_manifest_sha256"]
        or parse_usd_canonical(launch["hard_usd_ceiling"], "launch hard USD ceiling")
        != parse_usd_canonical(result["hard_usd_ceiling"], "result hard USD ceiling")
        or parse_usd_canonical(launch["estimated_max_total_usd"], "launch estimated USD")
        != parse_usd_canonical(result["estimated_max_total_usd"], "result estimated USD")
    ):
    ...
    protocol = repo_root / "docs/research/continual-learning/320-gemma3-fineweb-edu-replication-h100-v9-protocol.md"
    packet = repo_root / "docs/research/continual-learning/321-gemma3-fineweb-edu-replication-h100-v9-review-packet.md"
    ...
    if set(review) != REVIEW_RECEIPT_KEYS or review.get("schema") != "gemma3-fineweb-edu-replication-h100-v9-independent-review":
    ...
    for field in ("review_thread_id", "reviewer_key_id", "reviewer_public_key", "review_signature"):
        if not isinstance(review.get(field), str) or not review[field].strip():
            raise ValueError(f"launch review {field} is missing")
    if re.fullmatch(r"[A-Za-z0-9._:-]+", review["review_thread_id"]) is None or re.fullmatch(r"[A-Za-z0-9._:-]+", review["reviewer_key_id"]) is None:
        raise ValueError("launch review identity binding is invalid")
    try:
        public_key = base64.b64decode(review["reviewer_public_key"], validate=True)
        signature = base64.b64decode(review["review_signature"], validate=True)
        if len(public_key) != 32 or len(signature) != 64:
            raise ValueError("launch review signature length is invalid")
        from cryptography.exceptions import InvalidSignature
        from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PublicKey
        Ed25519PublicKey.from_public_bytes(public_key).verify(
            signature,
            canonical({key: value for key, value in review.items() if key not in {"review_signature", "receipt_sha256"}}),
        )
    except (ImportError, InvalidSignature, ValueError, TypeError) as error:
        raise ValueError("launch review signature is not independently verified") from error
    ...
    if implementation.get("schema") != "gemma3-fineweb-edu-replication-h100-v9-implementation" or implementation.get("state_slice") != STATE_SLICE or [item.get("path") for item in implementation.get("files", [])] != list(IMPLEMENTATION_FILES):
    ...
    if set(runtime_lock) != {"schema", "state_slice", "python", "accelerate", "pytorch", "transformers", "safetensors", "tokenizers", "pyarrow", "cryptography", "cuda_runtime", "dtype", "gpu_required", "network_policy", "package_install_at_runtime"} or runtime_lock["schema"] != "gemma3-fineweb-edu-replication-h100-v9-runtime-lock" or runtime_lock["state_slice"] != STATE_SLICE or runtime_lock["dtype"] != "bfloat16" or runtime_lock["gpu_required"] != "NVIDIA H100" or runtime_lock["network_policy"] != "network-none-v9" or runtime_lock["package_install_at_runtime"] is not False:
...
def config(value: Any, expected_alpha: Decimal, expected_beta: Decimal, label: str) -> dict[str, Any]:
    ...
    if exact_decimal(value["alpha"], f"{label} alpha") != expected_alpha or exact_decimal(value["beta"], f"{label} beta") != expected_beta or exact_decimal(value["epsilon"], f"{label} epsilon") != Decimal("1e-6"):
...
def metric(value: Any, windows: dict[str, dict[str, Any]], expected_temperature: Decimal, expected_config: dict[str, Any] | None, label: str) -> None:
    ...
    if isinstance(value["document_count"], bool) or not isinstance(value["document_count"], int) or value["document_count"] != WINDOW_COUNT or isinstance(value["target_tokens"], bool) or not isinstance(value["target_tokens"], int) or value["target_tokens"] != WINDOW_COUNT * (WINDOW_TOKENS - 1):
        raise ValueError(f"{label} metric count mismatch")
    if not isinstance(value["document_ids_sha256"], str):
        raise ValueError(f"{label} document identity digest missing")
    hex_digest(value["document_ids_sha256"], f"{label} document identity digest")
    expected_ids = list(windows)
    if value["document_ids_sha256"] != hashlib.sha256(canonical(expected_ids)).hexdigest():
        raise ValueError(f"{label} document identity digest mismatch")
    mean = exact_decimal(value["mean_nll"], f"{label} mean nll")
    if mean < 0:
        raise ValueError(f"{label} NLL must be nonnegative")
    expected_mean = mean.quantize(Decimal("1e-9"), rounding=ROUND_HALF_EVEN)
    expected_perplexity = expected_mean.exp().quantize(Decimal("1e-9"), rounding=ROUND_HALF_EVEN)
    if value["mean_nll"] != expected_mean or exact_decimal(value["perplexity"], f"{label} perplexity") != expected_perplexity:
...
def bootstrap(deltas: Sequence[Decimal]) -> dict[str, Any]:
    ...
    def rank(q: Decimal) -> Decimal:
        idx = (q * BOOTSTRAP_RESAMPLES).to_integral_value(rounding=ROUND_CEILING)
        return samples[max(1, min(BOOTSTRAP_RESAMPLES, int(idx))) - 1]

    return {
        "mean_delta": sum(values) / len(values),
        "lower": rank(Decimal("0.025")),
        "upper": rank(Decimal("0.975")),
...
def validate_assessment_ledger(path: Path, result: dict[str, Any], assessment: dict[str, dict[str, Any]]) -> list[Decimal]:
    if path.is_symlink() or not path.is_file() or path.stat().st_mode & 0o777 != 0o600:
        raise ValueError("temporary assessment ledger must be a regular owner-only file")
    ...
    for index, line_bytes in enumerate(payload.splitlines()):
        if not line_bytes.strip():
            raise ValueError("assessment ledger contains a blank line")
        value = json.loads(line_bytes, parse_float=Decimal)
        if not isinstance(value, dict) or set(value) != LEDGER_KEYS:
            raise ValueError("assessment ledger schema mismatch")
        if index >= len(expected_ids) or value["document_id"] != expected_ids[index]:
            raise ValueError("assessment ledger document order mismatch")
        ...
        if baseline < 0 or selected < 0:
            raise ValueError("assessment ledger NLL must be nonnegative")
        ...
        deltas.append(delta)
    if len(deltas) != WINDOW_COUNT:
        raise ValueError("assessment ledger count mismatch")
    baseline_mean = (baseline_total / (WINDOW_COUNT * (WINDOW_TOKENS - 1))).quantize(Decimal("1e-9"), rounding=ROUND_HALF_EVEN)
    selected_mean = (selected_total / (WINDOW_COUNT * (WINDOW_TOKENS - 1))).quantize(Decimal("1e-9"), rounding=ROUND_HALF_EVEN)
    if baseline_mean != exact_decimal(result["assessment_baseline"]["mean_nll"], "baseline nll") or selected_mean != exact_decimal(result["assessment_selected"]["mean_nll"], "selected nll"):
        raise ValueError("assessment ledger aggregate mismatch")
    return deltas
...
def validate_pre_effect_bundle(raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    validate_launch_custody_paths(
        launch, model_root, raw_root, source_root, corpus_root, repo_root
    )
    ...
    model_manifest = validate_tree_manifest(model_root, launch["model_manifest_sha256"])
    if model_manifest["model_id"] != launch["model_id"] or model_manifest["model_revision"] != launch["model_revision"] or model_manifest["architecture"] != launch["model_architecture"]:
        raise ValueError("pre-effect model identity is not launch-bound")
...
def validate(result_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, model_root: Path, launch_manifest: Path, repo_root: Path) -> dict[str, Any]:
    ...
    candidate_mode = (result_root / "candidate-result.json").is_file()
    expected_files = (
        {"provider-receipt.json", "candidate-result.json"}
        if candidate_mode
        else {"provider-receipt.json", "result.json", "result-receipt.json"}
    )
    if actual != expected_files:
        raise ValueError("result exact file set mismatch")
    require_read_only_tree(result_root, "result root")
    result_path = result_root / ("candidate-result.json" if candidate_mode else "result.json")
    result = obj(result_path, "result")
    receipt = obj(result_root / "result-receipt.json", "result receipt") if not candidate_mode else None
    ...
    provider_fields = {
        ...
        "provider_charged_usd": parse_usd_canonical(provider_receipt["charged_usd"], "provider charged USD"),
        "provider_stop_reason": provider_receipt["stop_reason"],
    }
    for field, value in provider_fields.items():
        if field == "provider_charged_usd":
            if parse_usd_canonical(result[field], field) != value:
                raise ValueError("result provider receipt binding mismatch")
    ...
    if not candidate_mode:
        if set(receipt) != {"schema", "state_slice", "result_sha256", "result_file_sha256", "receipt_sha256"} or receipt["schema"] != "gemma3-fineweb-edu-replication-h100-v9-result-receipt" or receipt["state_slice"] != STATE_SLICE or receipt["result_sha256"] != result["results_sha256"] or receipt["result_file_sha256"] != sha256_file(result_path) or receipt["receipt_sha256"] != digest(receipt, "receipt_sha256"):
            raise ValueError("result receipt mismatch")
    for field in ("launch_manifest_sha256", "protocol_sha256", "packet_sha256", "review_receipt_sha256", "implementation_manifest_sha256", "code_bundle_sha256", "runtime_lock_sha256", "data_manifest_sha256", "source_manifest_sha256", "model_manifest_sha256", "corpus_manifest_sha256", "model_parameter_digest_before", "model_parameter_digest_after", "provider_receipt_sha256", "assessment_ledger_sha256"):
    ...
    if parse_usd_canonical(result["hard_usd_ceiling"], "hard USD ceiling") != HARD_USD_CEILING or parse_usd_canonical(result["estimated_max_total_usd"], "estimated total USD") <= 0:
        raise ValueError("result budget is not positive")
    if result["provider"] != launch["provider"] or result["provider_project"] != launch["provider_project"] or result["node_type"] != launch["node_type"] or result["job_mode"] != launch["job_mode"]:
        raise ValueError("result provider identity mismatch")
    expected_paths = {
    ...
    for key, value in expected_paths.items():
        if result[key] != value:
             raise ValueError(f"result custody path mismatch: {key}")
    runtime = result["runtime"]
    runtime_lock = obj(repo_root / "runtime-lock.json", "runtime lock")
    runtime_keys = {"python", "accelerate", "cryptography", "pytorch", "transformers", "safetensors", "tokenizers", "pyarrow", "cuda_runtime", "cuda_driver_version", "gpu_name", "gpu_count", "dtype", "network"}
    lock_fields = ("python", "accelerate", "cryptography", "pytorch", "transformers", "safetensors", "tokenizers", "pyarrow", "cuda_runtime")
    ...
    if (
        ...
        or runtime["network"] != "offline-process-block-v9"
    ...
    model_manifest = validate_tree_manifest(model_root, result["model_manifest_sha256"])
    if result["model_id"] != model_manifest["model_id"] or result["model_revision"] != model_manifest["model_revision"] or result["model_architecture"] != model_manifest["architecture"] or result["model_id"] != launch["model_id"] or result["model_revision"] != launch["model_revision"] or result["model_architecture"] != launch["model_architecture"]:
        raise ValueError("result model identity is not comprehensively bound")
    if result["candidate_pairs"] != [list(pair) for pair in CANDIDATE_PAIRS] or exact_decimal(result["fit_alpha"], "fit alpha") != FIT_ALPHA or exact_decimal(result["fit_beta"], "fit beta") != FIT_BETA or exact_decimal(result["evaluation_alpha"], "eval alpha") != EVALUATION_ALPHA or exact_decimal(result["evaluation_beta"], "eval beta") != EVALUATION_BETA or exact_decimal(result["temperature_control"], "temp control") != TEMPERATURE_CONTROL or result["normalization"] != "source_l2_norm_to_destination_l2_norm":
    ...
    for candidate, pair in zip(candidates, CANDIDATE_PAIRS, strict=True):
        ...
        metric(candidate["metrics"], fit, Decimal("1.0"), candidate["config"], "fit candidate metric")
    selected_mean, selected_index, _selected_candidate = min(
        (exact_decimal(item["metrics"]["mean_nll"], "mean_nll"), index, item)
    ...
    metric(result["assessment_repeat"], assessment, Decimal("1.0"), result["locked_evaluation_config"], "assessment repeat")
    ...
    for source_layer, destination_layer in CANDIDATE_PAIRS:
        ...
        zero_config = config(entry["metrics"]["evaluation_config"], Decimal("0.0"), Decimal("1.0"), "zero alpha")
        ...
        metric(entry["metrics"], assessment, Decimal("1.0"), zero_config, "zero-alpha metrics")
        if exact_decimal(entry["max_abs_nll_delta"], "zero-alpha delta") > PARITY_TOLERANCE:
    ...
    if not isinstance(reach, dict) or set(reach) != {"nonzero_intervention_reach", "reach_evidence", "zero_alpha_identity_passed", "parity_tolerance"} or reach["nonzero_intervention_reach"] is not True or reach["zero_alpha_identity_passed"] is not True or exact_decimal(reach["parity_tolerance"], "parity tolerance") != PARITY_TOLERANCE:
        raise ValueError("qualification schema mismatch")
    evidence = reach["reach_evidence"]
    if not isinstance(evidence, list) or len(evidence) != len(CANDIDATE_PAIRS) or {(int(item.get("source_layer")), int(item.get("destination_layer"))) for item in evidence} != set(CANDIDATE_PAIRS) or not all(isinstance(item, dict) and set(item) == {"source_layer", "destination_layer", "max_abs_fit_nll_delta", "reached"} and item.get("reached") is True and exact_decimal(item.get("max_abs_fit_nll_delta"), "reach delta") > 0 for item in evidence):
...
def promote_validated_result(result_root: Path) -> None:
    """Publish final receipts only after validate() has returned successfully."""
    candidate = result_root / "candidate-result.json"
    final_result = result_root / "result.json"
    final_receipt = result_root / "result-receipt.json"
    ledger_path = result_root.parent / f".{result_root.name}.assessment-ledger.jsonl"
    if not candidate.is_file() or final_result.exists() or final_receipt.exists():
        raise ValueError("result root is not an unpublished candidate root")
    try:
        candidate_bytes = candidate.read_bytes()
        with final_result.open("x", encoding="utf-8") as handle:
            handle.write(candidate_bytes.decode("utf-8"))
        final_result.chmod(0o444)
        result = obj(final_result, "promoted result")
        receipt = {
            "schema": "gemma3-fineweb-edu-replication-h100-v9-result-receipt",
            "state_slice": STATE_SLICE,
            "result_sha256": result["results_sha256"],
            "result_file_sha256": sha256_file(final_result),
        }
        receipt["receipt_sha256"] = digest(receipt, "receipt_sha256")
        with final_receipt.open("x", encoding="utf-8") as handle:
        ...
        for path in (final_result, final_receipt):
            if path.exists():
                path.unlink()
        raise
...
def main() -> int:
    ...
    try:
        if args.pre_effect:
            ...
            if args.promote:
                promote_validated_result(args.result_root)

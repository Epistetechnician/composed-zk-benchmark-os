#!/usr/bin/env python3
"""Pack the reviewed V29 fresh FineWeb-Edu source.
...
def _sandbox() -> None:
    if sys.platform != "darwin" or c.native_network_denied():
        return
    if os.environ.get("V29_PACK_REEXEC") == "1":
...
def _inputs(raw: Path, prior: Path) -> dict[str, Any]:
    return {
        "raw": c.snapshot_files(
            raw, {f"dataset/{item['path']}" for item in c.DATASET_FILES}, "V29 raw"
        ),
...
def pack_source(
    ...
    if final.exists():
        raise FileExistsError(f"V29 source exists: {final}")
    ...
    if _inputs(raw, prior) != input_snapshot:
        raise RuntimeError("V29 source inputs changed before packing")
    staging = final.parent / f".{final.name}.staging-{os.getpid()}"
    if staging.exists():
        raise FileExistsError(f"V29 staging exists: {staging}")
    staging.mkdir()
    try:
        for split, item in (
            ...
            if len(rows) != c.FRESH_ROW_COUNT or any(
                ...
                raise ValueError("V29 source is not fresh and disjoint")
        ...
        c.assert_code_snapshot(code_snapshot)
        if _inputs(raw, prior) != input_snapshot:
            raise RuntimeError("V29 source inputs changed during packing")
        v.validate_source(staging, raw, prior, receipt)
        c.assert_code_snapshot(code_snapshot)
        if _inputs(raw, prior) != input_snapshot:
            raise RuntimeError("V29 source inputs changed before publication")
        files = {
            "acquisition-manifest.json",
            "fit/fineweb_edu.jsonl",
            "assessment/fineweb_edu.jsonl",
        ...
        c.publish_no_replace(
            ...
            lambda: (
                ...
                if _inputs(raw, prior) == input_snapshot
                    ...
                    RuntimeError("V29 source inputs changed after publication")
                ),
            ),
        )
        return {

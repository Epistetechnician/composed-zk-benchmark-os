#!/usr/bin/env python3
"""Review-gated V22 corpus stage and offline Gemma3 runner.

State slice: continual-learning-gemma3-fineweb-edu-replication-v22.
No model or scientific effect is reachable until the V22 receipt validates.
"""
...
from experiments.continual_learning import (
    validate_gemma3_fineweb_edu_replication_v22 as v,
)

VALIDATOR = "experiments.continual_learning.validate_gemma3_fineweb_edu_replication_v22"
...
def _sandbox() -> None:
    if sys.platform != "darwin" or c.native_network_denied():
        return
    if os.environ.get("V22_RUN_REEXEC") == "1":
        raise RuntimeError("V22 native network denial could not be established")
    executable = shutil.which("sandbox-exec")
    if executable is None:
        raise RuntimeError("sandbox-exec is unavailable")
    env = os.environ.copy()
    env["V22_RUN_REEXEC"] = "1"
    os.execvpe(
        executable,
        [
            executable,
...
def parity(model: Any, windows: Sequence[v.Window]) -> dict[str, Any]:
    ...
    if len(checks) != 128 or not all(item["passed"] is True for item in checks):
        raise RuntimeError("V22 zero-alpha parity failed")
    return {
...
def _run_effects(
    ...
    if (
        ...
        raise RuntimeError("V22 input changed before effects")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V22 architecture mismatch")
        ...
        if (
            ...
            raise RuntimeError("V22 input changed during effects")
        ...
        if before_parameters != after_parameters or before_model != after_model:
            raise RuntimeError("V22 model custody changed")
    ...
    if not any(item["reached"] for item in reach):
        raise RuntimeError("V22 nonzero intervention reach failed")
    common = {
    ...
    config = {
        **common,
        ...
        "selection_policy": "first-64-eligible-1024-token-windows-per-disjoint-v22-source-split",
    }
...
def stage_corpus(
    ...
    source, final, model = (
        c.exact_path(source_root, c.SOURCE_ROOT, "V22 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V22 corpus"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V22 corpus exists: {final}")
    ...
    if _reviewed_input(source, None, raw_root, prior_root, model) != input_snapshot:
        raise RuntimeError("V22 inputs changed before tokenizer")
    ...
    try:
        ...
        for split, count in (("fit", 64), ("assessment", 64)):
            selected = []
            for row in v.jsonl(source / f"{split}/fineweb_edu.jsonl", f"V22 {split}"):
                ...
                if (
                    ...
                    raise ValueError("V22 tokenizer round trip failed")
            ...
            if len(selected) != count:
                raise ValueError(f"V22 {split} has {len(selected)} eligible windows")
        ...
        _assert_review(review_snapshot)
        c.exact_file_set(staging, _expected_corpus_files(), "V22 corpus staging")
        with c.network_block():
            v.audit_corpus(
                staging,
                source,
        ...
        output = c.snapshot_files(
            staging, _expected_corpus_files(), "V22 corpus staging"
        )
        c.publish_no_replace(
            ...
            lambda: (
                ...
                if _reviewed_input(source, None, raw_root, prior_root, model) == input_snapshot
                    ...
                    RuntimeError("V22 corpus inputs changed after publication")
                ),
            ),
        )
        return {
            "manifest": _json(final / "manifest.json", "V22 corpus manifest"),
            "snapshot": output,
        }
    except Exception:
        if staging.exists():
...
def run_campaign(
    ...
    final, source, corpus, raw, prior, model = (
        c.exact_path(result_root, c.RESULT_ROOT, "V22 result"),
        c.exact_path(source_root, c.SOURCE_ROOT, "V22 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V22 corpus"),
        c.exact_path(raw_root, c.RAW_ROOT, "V22 raw"),
        c.exact_path(prior_root, c.PRIOR_ROOT, "V22 prior"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V22 result exists: {final}")
    review_snapshot = _review(receipt)
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V22 inputs changed during tokenizer load")
    ...
    _assert_review(review_snapshot)
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V22 inputs changed before effects")
    staging = Path(tempfile.mkdtemp(prefix=f".{final.name}.staging-", dir=final.parent))
    try:
        ...
        c.exact_file_set(staging, expected, "V22 result staging")
        _assert_review(review_snapshot)
        current = _reviewed_input(source, corpus, raw, prior, model)
        if current != input_snapshot:
            raise RuntimeError("V22 inputs changed immediately before validator")
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"V22 independent validator failed:\n{completed.stdout}\n{completed.stderr}"
            )
        ...
        if (
            ...
            raise RuntimeError("V22 post-validator custody check failed")
        ...
        if final_completed.returncode != 0:
            raise RuntimeError(
                f"V22 final independent validator failed:\n{final_completed.stdout}\n{final_completed.stderr}"
            )
        final_validation = json.loads(final_completed.stdout)
        if final_validation != validation:
            raise RuntimeError("V22 final validator result mismatch")
        expected_with_validator = set(expected) | {"validator-receipt.json"}
        c.exact_file_set(staging, expected_with_validator, "V22 result staging")
        result_snapshot = c.snapshot_files(
            staging, expected_with_validator, "V22 result staging"
        )
        c.publish_no_replace(
            staging,
            final,
            ...
            lambda: (
                ...
                if _reviewed_input(source, corpus, raw, prior, model) == input_snapshot
                    ...
                    RuntimeError("V22 result inputs changed after publication")
                ),
            ),

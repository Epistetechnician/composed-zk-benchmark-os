...
PyTorch module hooks, and emits aggregate/per-document scalar results.  It
does not submit provider jobs or acquire data.
"""
...
SCHEMA = "gemma3-fineweb-edu-replication-h100-v5-result"
...
def validate_tree_manifest(root: Path, label: str) -> dict[str, Any]:
    ...
    if manifest["schema"] != "gemma3-model-manifest-v5" or manifest["model_id"] != MODEL_ID or manifest["architecture"] != MODEL_ARCHITECTURE or re.fullmatch(r"[0-9a-f]{40}", manifest["model_revision"]) is None:
...
def validate_runtime_lock(repo_root: Path, launch: dict[str, Any], torch: Any) -> dict[str, Any]:
    ...
    if not isinstance(lock, dict) or set(lock) != expected_keys or lock["schema"] != "gemma3-fineweb-edu-replication-h100-v5-runtime-lock" or lock["state_slice"] != STATE_SLICE:
    ...
    if lock["dtype"] != "bfloat16" or lock["gpu_required"] != "NVIDIA H100" or lock["network_policy"] != "network-none-v5" or lock["package_install_at_runtime"] is not False:
...
def run(model_root: Path, raw_root: Path, source_root: Path, corpus_root: Path, result_root: Path, launch_manifest: Path, repo_root: Path | None = None, tokenizer: Any | None = None) -> dict[str, Any]:
    ...
    from experiments.continual_learning import (
        gemma3_fineweb_edu_replication_h100_v5_preflight as preflight,
    )
    ...
    from experiments.continual_learning import validate_gemma3_fineweb_edu_replication_h100_v5 as validator
    validator.validate_provider_receipt(provider_receipt_path, launch, launch_file_sha256)
    ...
    from experiments.continual_learning import (
        pack_gemma3_fineweb_edu_replication_h100_v5 as packer,
    )
    ...
    with network_block():
        ...
        for source_layer, destination_layer in CANDIDATE_PAIRS:
            ...
            candidates.append({"config": config.as_dict(), "metrics": evaluate_windows(model, tokenizer, fit, config)})
        selected = min(
        ...
        assessment_selected = evaluate_windows(model, tokenizer, assessment, locked)
        temperature_baseline = evaluate_windows(model, tokenizer, assessment, None, TEMPERATURE_CONTROL)
        temperature_selected = evaluate_windows(model, tokenizer, assessment, locked, TEMPERATURE_CONTROL)
        ...
        for source_layer, destination_layer in CANDIDATE_PAIRS:
            ...
            zero[pair] = {"metrics": metrics, "max_abs_nll_delta": max(abs(metrics["rows"][i]["nll"] - assessment_baseline["rows"][i]["nll"]) for i in range(EXPECTED_WINDOWS))}
    ...
    base = {row["document_id"]: row for row in assessment_baseline["rows"]}
    chosen = {row["document_id"]: row for row in assessment_selected["rows"]}
    per_document = []
    ...
    for pair, candidate in zip(CANDIDATE_PAIRS, candidates, strict=True):
    ...
    parity = max(item["max_abs_nll_delta"] for item in zero.values())
    if parity > PARITY_TOLERANCE or any(item["max_abs_nll_delta"] > PARITY_TOLERANCE for item in zero.values()):
    ...
    bootstrap = bootstrap_mean_ci([row["delta_selected_minus_baseline"] for row in per_document])
    elapsed = time.monotonic() - started
    result = {
        "schema": SCHEMA,
        "state_slice": STATE_SLICE,
        "claim_ceiling": "LocalDevelopmentGemma3FineWebEduReplicationH100V5",
        ...
        "runtime": {
            **runtime,
            ...
            "dtype": "bfloat16",
            "network": "offline-process-block-v5",
        },
    ...
    try:
        ...
        with destination.open("x", encoding="utf-8") as handle:
            ...
            handle.write("\n")
        receipt = {"schema": "gemma3-fineweb-edu-replication-h100-v5-result-receipt", "state_slice": STATE_SLICE, "result_sha256": result["results_sha256"], "result_file_sha256": sha256_file(destination)}

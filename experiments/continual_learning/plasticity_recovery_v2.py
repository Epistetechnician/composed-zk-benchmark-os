#!/usr/bin/env python3
"""Fresh exact-synthetic protected-replay recovery factorial.

State slice: ``continual-learning-plasticity-recovery-v2``.

V2 tests one new mechanism suggested by the read-only V1 diagnosis: reserve a
small immutable replay memory for the early protected shards. V1 source code
is used only as a transition-kernel implementation reference; V2 has a fresh
state namespace, fresh panel seeds, fresh order seeds, and fresh panel values.
No model, provider, Astral, or cryptographic backend is executed.
"""
...
try:
    from experiments.continual_learning import plasticity_recovery_v1 as kernel
...
class ProtocolError(ValueError):
    """Raised when the frozen V2 synthetic contract is violated."""
...
def _validate_config() -> None:
    ...
    _require(PROTECTED_REPLAY_CAPACITY == 4, "protected replay capacity drift")
...
def _target(seed: int, split: str, index: int) -> tuple[float, ...]:
    ...
    return tuple(
        ...
        + amplitude * _signed(STATE_SLICE, "fresh-panel", seed, split, index, unit, "target")
...
def _shard_body(shard: kernel.Shard) -> dict[str, Any]:
    return {
...
def make_panel(seed: int) -> tuple[kernel.Shard, ...]:
    """Generate V2's fresh deterministic fit/tune/assessment panel."""

    _validate_config()
    counts = {"fit": FIT_COUNT, "tune": TUNE_COUNT, "assessment": ASSESSMENT_COUNT}
    shards: list[kernel.Shard] = []
    for split in SPLITS:
        for index in range(counts[split]):
            ...
            draft = kernel.Shard(
                shard_id=f"fit-{index:03d}" if split == "fit" else f"{split}-{index:03d}",
                split=split,
                index=index,
                family=index % 4,
                target=target,
...
def panel_digest(panel: Sequence[kernel.Shard]) -> str:
    return _digest([asdict(shard) for shard in panel])


def split_digest(panel: Sequence[kernel.Shard], split: str) -> str:
    _require(split in SPLITS, f"unknown split: {split}")
    return _digest([asdict(shard) for shard in panel if shard.split == split])


def make_order(panel: Sequence[kernel.Shard], order_seed: int) -> tuple[kernel.Shard, ...]:
...
def _replay_target(state: kernel.State, fit_by_id: Mapping[str, kernel.Shard], current: kernel.Shard, seed: int, step: int) -> kernel.Shard:
...
def _protected_replay_target(protected_buffer: Sequence[str], fit_by_id: Mapping[str, kernel.Shard], current: kernel.Shard, seed: int, step: int) -> kernel.Shard:
    if not protected_buffer:
        return current
    index = int(_unit(STATE_SLICE, "protected-replay", seed, current.shard_id, step) * len(protected_buffer))
    return fit_by_id[protected_buffer[index]]


def _state_signature(state: kernel.State) -> dict[str, Any]:
    return {
...
def _run_case(panel: Sequence[kernel.Shard], arm: str, seed: int, order_seed: int) -> dict[str, Any]:
    """Run fit-only transitions and return a pre-assessment draft."""
    ...
    state = kernel._initial_state()
    shadow_state = state
    protected_buffer: tuple[str, ...] = ()
    records: list[dict[str, Any]] = []
    protected_snapshots: list[tuple[kernel.Shard, tuple[float, ...]]] = []
    ...
    for step, current in enumerate(ordered):
        ...
        if arm in ("selective_reinit", "replay_selective_reinit"):
            work, reinitialized_unit = kernel._reinitialize(work, step)
        targets = [current, current]
        if arm in ("replay", "replay_selective_reinit"):
            targets[1] = _replay_target(work, fit_by_id, current, seed, step)
        if arm == "protected_replay":
            targets[1] = _protected_replay_target(protected_buffer, fit_by_id, current, seed, step)
        ...
        for slot, target in enumerate(targets):
            ...
            shadow, update = kernel._gradient_step(
                shadow,
                target,
                current,
                step,
        ...
        if arm == "no_update":
            ...
            accepted = True
        if accepted and arm == "protected_replay" and len(protected_buffer) < PROTECTED_REPLAY_CAPACITY:
            protected_buffer = protected_buffer + (current.shard_id,)
        ...
        records.append(
            {
                ...
                "target_shard_ids": [update.target_shard_id for update in step_records],
                "reinitialized_unit": reinitialized_unit,
                "accepted": accepted,
                "update_effect_sha256": _digest([update.effect_sha256 for update in step_records]),
            }
        )
        state = state_after
    ...
    tune_prediction = sum(_loss(base_weights, shard.target) - _loss(state.weights, shard.target) for shard in tune) / len(tune)
    forgetting_values = [
        max(0.0, _loss(state.weights, shard.target) - _loss(snapshot_weights, shard.target))
    ...
    return {
        ...
        "shadow_gradient_evaluations": state.shadow_gradient_evaluations,
        "reinitializations": state.reinitializations,
        "protected_replay_shard_ids": list(protected_buffer),
...
def _assess_case(case: Mapping[str, Any], panel: Sequence[kernel.Shard]) -> dict[str, Any]:
    ...
    return {
        **case,
        ...
        "adaptation_gain": round(base_assessment_loss - final_assessment_loss, 12),
    }
...
def run_factorial() -> dict[str, Any]:
    """Run the fixed V2 6-arm x 4-seed x 3-order factorial."""
    ...
    body = {
        ...
        "config": {
            ...
            "protected_replay_capacity": PROTECTED_REPLAY_CAPACITY,
        ...
        "claims": [
            ...
            "fresh_v2_formula_derived_panels",
            "base_weights_untouched",
            "reversible_adapter_analogue_only",
            "protected_replay_is_single_new_mechanism",
...
def validate_result(result: Mapping[str, Any]) -> None:
    """Validate the sealed V2 result before independent readback."""
    ...
    _require(config.get("protected_replay_capacity") == PROTECTED_REPLAY_CAPACITY, "protected replay drift")
...
def markdown_report(result: Mapping[str, Any]) -> str:
    lines = [
        "# Plasticity Recovery V2",
        "",
        "Fresh exact synthetic learner. Protected replay is the single new mechanism; base weights are untouched; Astral and ZK/PQC are not run.",
        "",

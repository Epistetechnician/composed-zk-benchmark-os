#!/usr/bin/env python3
"""V24 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v24"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV24"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v24-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v24-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v24-result"
)
MODEL_PATH = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/221-gemma3-fineweb-edu-replication-v24-protocol.md"
)
PACKET_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/222-gemma3-fineweb-edu-replication-v24-review-packet.md"
)
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/223-gemma3-fineweb-edu-replication-v24-independent-review-2026-08-31.json"
)
PROTOCOL_SHA256 = "02d65b554c62650ed3bc2ab07e33c07ec994451ce67f768bb0684281db188178"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v24-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v24-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v24-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v24-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v24_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v24.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v24.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v24.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v24.py",
)
...
def sha256_file(path: Path) -> str:
    ...
    reject_symlink_components(checked, "hashed file")
    if checked.is_symlink() or not checked.is_file():
        raise ValueError(f"hashed file must be a regular file: {checked}")
    before = checked.stat()
    ...
    if (
        ...
        raise RuntimeError(f"hashed file changed during read: {checked}")
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V24 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V24 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V24 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V24 implementation file")
        item = regular(path, "V24 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V24 review receipt")
    value = json.loads(
        stable_read_bytes(path, "V24 review receipt").decode("utf-8")
    )
    if not isinstance(value, dict):
        raise ValueError("V24 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V24 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V24 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V24 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V24 review receipt binding mismatch")
    expected_keys = {
        "schema",
        "state_slice",
        "reviewer",
    ...
    if set(value) != expected_keys:
        raise ValueError("V24 review receipt schema is not closed")
    if type(value.get("effects_run")) is not bool or value["effects_run"] is not False:
        raise ValueError("V24 review effects_run must be a boolean false")
    ...
    if (
        ...
        raise ValueError("V24 receipt requires seven explicit true findings")
    if value.get("material_findings") != []:
        raise ValueError("V24 ACCEPT receipt must have an empty material_findings list")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": stable_read_bytes(PROTOCOL_PATH, "V24 protocol"),
        "packet": stable_read_bytes(PACKET_PATH, "V24 packet"),
        "receipt": stable_read_bytes(RECEIPT_PATH, "V24 receipt"),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V24 reviewed bytes or history changed")

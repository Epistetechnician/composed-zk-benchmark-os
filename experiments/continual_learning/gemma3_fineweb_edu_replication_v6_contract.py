#!/usr/bin/env python3
"""V6 custody, sandbox, bootstrap, and review primitives.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v6"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV6"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v6-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v6-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v6-result"
)
MODEL_PATH = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/157-gemma3-fineweb-edu-replication-v6-protocol.md"
)
PACKET_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/158-gemma3-fineweb-edu-replication-v6-review-packet.md"
)
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/159-gemma3-fineweb-edu-replication-v6-independent-review-2026-08-30.json"
)
PROTOCOL_SHA256 = "90fcbd2602ca215f392faa924316a9d5c34ae1ccb504716b00e1f58ed274c507"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v6-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v6-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v6-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v6-independent-review"
...
EXPECTED_MODEL_CACHE_MANIFEST_SHA256 = (
...
FRESH_ROW_START, FRESH_ROW_COUNT = 2_048, 16_384
FRESH_ROW_END = FRESH_ROW_START + FRESH_ROW_COUNT
WINDOW_TOKENS, FIT_WINDOW_COUNT, ASSESSMENT_WINDOW_COUNT = 1_024, 64, 64
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v6_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v6.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v6.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v6.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v6.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(
            f"{label} must equal exact root or V6 staging sibling: {supplied}"
        )
    reject_symlink_components(supplied, label)
    return supplied
...
def external(path: Path, label: str) -> Path:
    ...
    if supplied == repo or repo in supplied.parents:
        raise ValueError(f"{label} must be outside repository: {supplied}")
    return supplied
...
def safe_relative(root: Path, relative: Any, label: str) -> Path:
    if (
        ...
        raise ValueError(f"{label} must be safe relative path")
...
def model_manifest(model_path: Path) -> dict[str, Any]:
    ...
    def files(names: tuple[str, ...]) -> list[dict[str, Any]]:
        return [
            {
                "path": name,
    ...
    stable = {"model_name": root.name, "files": files(MODEL_STABLE_FILES)}
    cache = {"model_name": root.name, "files": files(MODEL_CACHE_FILES)}
    return {
        "manifest": stable,
...
def validate_prior_history() -> list[dict[str, str]]:
    ...
    for item in PRIOR_HISTORY:
        ...
        reject_symlink_components(path.absolute(), "prior rejection record")
        regular(path, "prior rejection record")
        observed = sha256_file(path)
        if observed != item["sha256"]:
            raise ValueError(f"prior rejection history changed: {item['path']}")
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V6 native outbound-network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V6 offline execution forbids network/process escape")
    ...
    original_spawn = {name: getattr(os, name, None) for name in spawn_names}
    ...
    os.system, os.popen = forbidden, forbidden
    ...
    try:
        ...
        for name, value in original.items():
            ...
            elif name in ("Popen", "run", "check_call", "check_output"):
...
def decide_replication(bootstrap: dict[str, Any]) -> str:
    mean, upper = bootstrap.get("mean_delta"), bootstrap.get("upper")
    if any(
        isinstance(value, bool)
        or not isinstance(value, (int, float))
        or not math.isfinite(float(value))
        for value in (mean, upper)
    ):
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V6 implementation file")
        item = regular(path, "V6 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V6 review receipt")
    value = json.loads(regular(path, "V6 review receipt").read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("V6 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V6 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V6 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V6 review timestamp not canonical")
    if (
        ...
        raise ValueError("V6 review receipt binding mismatch")
    if value.get("findings") != {name: True for name in REVIEW_FINDINGS}:
        raise ValueError("V6 receipt must contain exactly seven true findings")
    return value
...
def snapshot_files(root: Path, expected: set[str], label: str) -> list[dict[str, Any]]:
    ...
    return [
        {
            "path": rel,
            "byte_len": safe_relative(root, rel, f"{label} file").stat().st_size,
            "sha256": sha256_file(safe_relative(root, rel, f"{label} file")),
        }
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": regular(PROTOCOL_PATH, "V6 protocol").read_bytes(),
        "packet": regular(PACKET_PATH, "V6 packet").read_bytes(),
        "receipt": regular(RECEIPT_PATH, "V6 receipt").read_bytes(),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if (
        regular(PROTOCOL_PATH, "V6 protocol").read_bytes() != snapshot["protocol"]
        or regular(PACKET_PATH, "V6 packet").read_bytes() != snapshot["packet"]
        or regular(RECEIPT_PATH, "V6 receipt").read_bytes() != snapshot["receipt"]
        or implementation_manifest() != snapshot["implementation"]
        or validate_prior_history() != snapshot["history"]
    ):
        raise RuntimeError("V6 reviewed bytes or rejection history changed")
...
def publish_no_replace(
    ...
    if final.exists():
        raise FileExistsError(f"{label} final root exists: {final}")

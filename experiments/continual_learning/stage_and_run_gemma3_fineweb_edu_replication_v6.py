#!/usr/bin/env python3
"""Review-gated V6 corpus stage and offline Gemma3 runner.

State slice: continual-learning-gemma3-fineweb-edu-replication-v6.
No model or scientific effect is reachable until the V6 receipt validates.
"""
...
from experiments.continual_learning import (
    validate_gemma3_fineweb_edu_replication_v6 as v,
)

VALIDATOR = "experiments.continual_learning.validate_gemma3_fineweb_edu_replication_v6"
...
def _sandbox() -> None:
    if sys.platform != "darwin" or c.native_network_denied():
        return
    if os.environ.get("V6_RUN_REEXEC") == "1":
        raise RuntimeError("V6 native network denial could not be established")
...
def _assert_review(snapshot: dict[str, Any]) -> None:
    c.assert_code_snapshot(snapshot)
...
class RecirculationConfig:
    ...
    def validate(self, layer_count: int) -> None:
        if (
            ...
            raise ValueError("V6 layer pair invalid")
        if (
            any(
            ...
            raise ValueError("V6 recirculation parameters invalid")
...
def load_runtime(model_path: Path) -> tuple[Any, Any, dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("V6 model custody/runtime mismatch")
    c.require_native_network_denial()
        ...
        if config is None:
            ...
            for index, (layer, layer_cache) in enumerate(
                ...
                if index == config.source_layer:
                    current = hidden
            if current is None:
                raise RuntimeError("V6 source activation not captured")
...
def evaluate_windows(
    ...
    if (
        ...
        raise ValueError("V6 temperature invalid")
    rows = []
    for window in windows:
        ...
        if tuple(ids) != window.token_ids or len(ids) != c.WINDOW_TOKENS:
            raise ValueError("V6 tokenizer changed")
        ...
        if not math.isfinite(total):
            raise ValueError("V6 NLL invalid")
        rows.append(
            {
...
def parity(model: Any, windows: Sequence[v.Window]) -> dict[str, Any]:
    ...
    if len(checks) != 128 or not all(item["passed"] is True for item in checks):
        raise RuntimeError("V6 zero-alpha parity failed")
    return {
...
def _run_effects(
    ...
    if (
        ...
        raise RuntimeError("V6 input changed before effects")
    with c.network_block():
        ...
        if (
            ...
            raise ValueError("V6 architecture mismatch")
        ...
        if (
            ...
            raise RuntimeError("V6 input changed during effects")
        ...
        if before_parameters != after_parameters or before_model != after_model:
            raise RuntimeError("V6 model custody changed")
    ...
    by_id = {window.document_id: window for window in assessment}
    per_document = []
    for document_id in sorted(base_rows):
        window = by_id[document_id]
    ...
    config = {
        **common,
        ...
        "selection_policy": "first-64-eligible-1024-token-windows-per-disjoint-v6-source-split",
    }
...
def stage_corpus(
    ...
    source, final, model = (
        c.exact_path(source_root, c.SOURCE_ROOT, "V6 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V6 corpus"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V6 corpus exists: {final}")
    ...
    if _reviewed_input(source, None, raw_root, prior_root, model) != input_snapshot:
        raise RuntimeError("V6 inputs changed before tokenizer")
    ...
    try:
        ...
        for split, count in (("fit", 64), ("assessment", 64)):
            selected = []
            for row in v.jsonl(source / f"{split}/fineweb_edu.jsonl", f"V6 {split}"):
                ...
                if (
                    ...
                    raise ValueError("V6 tokenizer round trip failed")
            ...
            if len(selected) != count:
                raise ValueError(f"V6 {split} has {len(selected)} eligible windows")
        ...
        _assert_review(review_snapshot)
        c.exact_file_set(staging, _expected_corpus_files(), "V6 corpus staging")
        with c.network_block():
            v.audit_corpus(
                staging,
                source,
        ...
        output = c.snapshot_files(
            staging, _expected_corpus_files(), "V6 corpus staging"
        )
        c.publish_no_replace(staging, final, _expected_corpus_files(), "V6 corpus")
        return {
            "manifest": _json(final / "manifest.json", "V6 corpus manifest"),
            "snapshot": output,
        }
    except Exception:
        if staging.exists():
...
def _validator_receipt(staging: Path, result_sha: str, review_sha: str) -> None:
    body = {
        "schema": "gemma3-fineweb-edu-replication-v6-validator-receipt",
        "state_slice": c.STATE_SLICE,
    ...
    _write(
        staging / "validator-receipt.json", {**body, "receipt_sha256": c.digest(body)}
    )
...
def run_campaign(
    ...
    final, source, corpus, raw, prior, model = (
        c.exact_path(result_root, c.RESULT_ROOT, "V6 result"),
        c.exact_path(source_root, c.SOURCE_ROOT, "V6 source"),
        c.exact_path(corpus_root, c.CORPUS_ROOT, "V6 corpus"),
        c.exact_path(raw_root, c.RAW_ROOT, "V6 raw"),
        c.exact_path(prior_root, c.PRIOR_ROOT, "V6 prior"),
        c.exact_path(model_path, c.MODEL_PATH, "model path"),
    )
    if final.exists():
        raise FileExistsError(f"V6 result exists: {final}")
    review_snapshot = _review(receipt)
    ...
    if _reviewed_input(source, corpus, raw, prior, model) != input_snapshot:
        raise RuntimeError("V6 inputs changed before effects")
    staging = Path(tempfile.mkdtemp(prefix=f".{final.name}.staging-", dir=final.parent))
    try:
        ...
        _validator_receipt(
            staging,
            payload["results"]["results_sha256"],
            review_snapshot["review_sha256"],
        )
        expected = {
            "config.json",
        ...
        c.exact_file_set(staging, expected, "V6 result staging")
        _assert_review(review_snapshot)
        current = _reviewed_input(source, corpus, raw, prior, model)
        if current != input_snapshot:
            raise RuntimeError("V6 inputs changed immediately before validator")
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"V6 independent validator failed:\n{completed.stdout}\n{completed.stderr}"
            )
        validation = json.loads(completed.stdout)
        _assert_review(review_snapshot)
        if (
            _reviewed_input(source, corpus, raw, prior, model) != input_snapshot
            or validation.get("valid") is not True
        ):
            raise RuntimeError("V6 post-validator custody check failed")
        c.publish_no_replace(staging, final, expected, "V6 result")

#!/usr/bin/env python3
"""V14 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v14"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV14"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v14-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v14-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v14-result"
)
...
PROTOCOL_SHA256 = "80c1e441359eb28e16b5b2208a7976cbe9ad9c23448c90f4c6ee2fbaa6d9d4ec"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v14-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v14-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v14-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v14-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v14.py",
    REPO_ROOT
    ...
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v14.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V14 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V14 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V14 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V14 implementation file")
        item = regular(path, "V14 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V14 review receipt")
    value = json.loads(regular(path, "V14 review receipt").read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("V14 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V14 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V14 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V14 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V14 review receipt binding mismatch")
    if value.get("findings") != {name: True for name in REVIEW_FINDINGS}:
        raise ValueError("V14 receipt requires seven true findings")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": regular(PROTOCOL_PATH, "V14 protocol").read_bytes(),
        "packet": regular(PACKET_PATH, "V14 packet").read_bytes(),
        "receipt": regular(RECEIPT_PATH, "V14 receipt").read_bytes(),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V14 reviewed bytes or history changed")

#!/usr/bin/env python3
"""V27 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v27"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV27"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v27-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v27-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v27-result"
)
MODEL_PATH = Path("/Users/shaanp/.lmstudio/models/mlx-community/gemma-3-1b-pt-bf16")
MODEL_SNAPSHOT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v27-model-snapshot"
)
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/233-gemma3-fineweb-edu-replication-v27-protocol.md"
)
PACKET_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/234-gemma3-fineweb-edu-replication-v27-review-packet.md"
)
RECEIPT_PATH = (
    REPO_ROOT
    / "docs/research/continual-learning/235-gemma3-fineweb-edu-replication-v27-independent-review-2026-08-31.json"
)
PROTOCOL_SHA256 = "966ad4a195f675dfb54e72331e6a5fe3c06ba1b7ad7753000795f94fd7740171"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v27-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v27-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v27-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v27-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v27_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v27.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v27.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v27.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v27.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V27 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def materialize_model_snapshot(model_path: Path) -> Path:
    ...
    if snapshot.exists() or snapshot.is_symlink():
        if (
            ...
            raise ValueError("V27 model snapshot already exists with a different identity")
        _require_read_only_snapshot(snapshot, "V27 model snapshot")
        return snapshot
    reject_symlink_components(snapshot, "V27 model snapshot")
    staging = snapshot.parent / f".{snapshot.name}.staging-{os.getpid()}"
    if staging.exists() or staging.is_symlink():
        raise FileExistsError(f"V27 model snapshot staging exists: {staging}")
    staging.mkdir()
    published = False
    try:
        ...
        for relative in expected:
            ...
            with stable_binary_file(source, "V27 model source") as source_handle:
        ...
        staged_manifest = model_manifest_at(
            staging, "V27 model snapshot", MODEL_PATH.name
        )
        if staged_manifest != source_manifest:
            raise RuntimeError("V27 model snapshot digest mismatch")
        ...
        if (
            ...
            raise RuntimeError("V27 published model snapshot mismatch")
        return snapshot
    except Exception:
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V27 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V27 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V27 implementation file")
        item = regular(path, "V27 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V27 review receipt")
    value = json.loads(
        stable_read_bytes(path, "V27 review receipt").decode("utf-8")
    )
    if not isinstance(value, dict):
        raise ValueError("V27 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V27 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V27 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V27 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V27 review receipt binding mismatch")
    expected_keys = {
        "schema",
        "state_slice",
        "reviewer",
    ...
    if set(value) != expected_keys:
        raise ValueError("V27 review receipt schema is not closed")
    if type(value.get("effects_run")) is not bool or value["effects_run"] is not False:
        raise ValueError("V27 review effects_run must be a boolean false")
    ...
    if (
        ...
        raise ValueError("V27 receipt requires seven explicit true findings")
    if value.get("material_findings") != []:
        raise ValueError("V27 ACCEPT receipt must have an empty material_findings list")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": stable_read_bytes(PROTOCOL_PATH, "V27 protocol"),
        "packet": stable_read_bytes(PACKET_PATH, "V27 packet"),
        "receipt": stable_read_bytes(RECEIPT_PATH, "V27 receipt"),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V27 reviewed bytes or history changed")

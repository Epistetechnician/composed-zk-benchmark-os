...
def test_locked_constants_and_exact_bootstrap() -> None:
    assert c.STATE_SLICE.endswith("v18")
...
def test_review_is_mandatory_and_loaders_guard_before_import() -> None:
    ...
    for function_name, import_text, guard_text in (
        (
            ...
            "V18 tokenizer custody/runtime mismatch",
        ),
        (
            "load_runtime",
            "from mlx_lm import load",
            "V18 model custody/runtime mismatch",
        ),
    ):
...
def test_v18_contract_references_are_defined() -> None:
    for relative in (
...
def test_enriched_review_snapshot_projects_canonical_fields(monkeypatch) -> None:
    from experiments.continual_learning import (
        stage_and_run_gemma3_fineweb_edu_replication_v18 as runner,
    )
...
def test_independent_validator_does_not_import_runner_measurement_functions() -> None:
    ...
    assert "stage_and_run_gemma3_fineweb_edu_replication_v18" not in source
...
def test_v18_reach_and_post_validation_gates_are_hard() -> None:
    runner = Path(
        "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v18.py"
    ).read_text(encoding="utf-8")
    validator = Path(
    ...
    assert 'raise RuntimeError("V18 nonzero intervention reach failed")' in runner
    assert 'raise ValueError("V18 nonzero intervention reach was not proven")' in validator

...
from experiments.continual_learning import (
    gemma3_fineweb_edu_replication_h100_v4 as runner,
)
from experiments.continual_learning import (
    validate_gemma3_fineweb_edu_replication_h100_v4 as validator,
)
...
def test_provider_bundle_has_no_runtime_install_or_provider_submission() -> None:
    root = Path(__file__).parents[1] / "gemma3_fineweb_edu_replication_h100_v4_provider"
    dockerfile = (root / "Dockerfile").read_text(encoding="utf-8")
    entrypoint = (root / "run_h100_v4.sh").read_text(encoding="utf-8")
    ...
    assert "network-none-v4" in entrypoint
    ...
    assert "pack_gemma3_fineweb_edu_replication_h100_v4.py" in dockerfile
...
def test_provider_receipt_validator_requires_exact_binding(tmp_path: Path) -> None:
    ...
    launch["provider_attestation_key_id"] = "gman-test-key-v4"
    launch["provider_attestation_public_key"] = base64.b64encode(

...
def test_locked_constants_and_exact_bootstrap() -> None:
    assert c.STATE_SLICE.endswith("v30")
...
def test_review_is_mandatory_and_loaders_guard_before_import() -> None:
    ...
    for function_name, import_text, guard_text in (
        (
            ...
            "V30 tokenizer custody/runtime mismatch",
        ),
        (
            "load_runtime",
            "from mlx_lm import load",
            "V30 model custody/runtime mismatch",
        ),
    ):
...
def test_v30_contract_references_are_defined() -> None:
    for relative in (
...
def test_enriched_review_snapshot_projects_canonical_fields(monkeypatch) -> None:
    from experiments.continual_learning import (
        stage_and_run_gemma3_fineweb_edu_replication_v30 as runner,
    )
...
def test_result_and_nested_schemas_are_closed_world() -> None:
    ...
    assert 'exact_keys(config, CONFIG_RESULT_KEYS, "V30 config")' in source
    assert 'exact_keys(results, RESULT_KEYS, "V30 results")' in source
    assert 'exact_keys(result_receipt, RECEIPT_KEYS, "V30 result receipt")' in source
    assert 'exact_keys(row, METRIC_ROW_KEYS' in source
    assert 'exact_keys(row,' in source and "V30 assessment_per_document row" in source





def test_v30_all_parsed_artifacts_use_stable_reads() -> None:
    validator = Path(
        "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v30.py"
    ).read_text(encoding="utf-8")
    assert "c.stable_read_bytes(path, label)" in validator
    assert 'c.stable_read_bytes(path, "V30 window")' in validator
    assert 'c.stable_read_bytes(staged, "V30 staged receipt")' in validator
    assert 'c.stable_read_bytes(review_path, "V30 review receipt")' in validator
    assert '"V30 result corpus manifest"' in validator
    assert '"V30 corpus manifest"' in validator
    assert ".read_text(encoding=" not in validator
    assert ".read_bytes()" not in validator
    assert ".open(encoding=" not in validator


def test_v30_descriptor_custody_covers_hashes_parquet_and_model_loads(
    tmp_path: Path,
) -> None:
    ...
    assert 'with pinned_parquet(path, item, "V30 Parquet")' in validator
...
def test_v30_parquet_hash_and_parse_share_one_descriptor() -> None:
    validator = Path(
        "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v30.py"
    ).read_text(encoding="utf-8")
    ...
    assert 'V30 Parquet pin mismatch on parse descriptor' in validator


def test_v30_all_loader_helpers_require_review_receipt() -> None:
    validator = Path(
        "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v30.py"
    ).read_text(encoding="utf-8")
    runner = Path(
...
def test_v30_corpus_validator_checks_review_before_tokenizer_load() -> None:
    validator = Path(
        "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v30.py"
    ).read_text(encoding="utf-8")
...
def test_v30_source_metadata_and_configuration_are_closed_and_value_locked() -> None:
    validator = Path(
        "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v30.py"
    ).read_text(encoding="utf-8")
    assert 'DATASET_KEYS = {"fit/fineweb_edu", "assessment/fineweb_edu"}' in validator
    assert 'exact_keys(datasets, DATASET_KEYS, "V30 source datasets")' in validator
    assert 'exact_keys(metadata, DATASET_METADATA_KEYS, "V30 source dataset metadata")' in validator
    for value in (
    ...
    assert 'raise ValueError("V30 declared configuration value mismatch")' in validator


def test_v30_validation_binding_excludes_only_derived_snapshot() -> None:
...
def test_v30_publication_rejects_post_publish_mutation(tmp_path: Path) -> None:
    staging, final = tmp_path / "staging", tmp_path / "final"
    staging.mkdir()
    (staging / "required.txt").write_text("stable", encoding="utf-8")
...
def test_independent_validator_does_not_import_runner_measurement_functions() -> None:
    ...
    assert "stage_and_run_gemma3_fineweb_edu_replication_v30" not in source
...
def test_v30_reach_and_post_validation_gates_are_hard() -> None:
    runner = Path(
        "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v30.py"
    ).read_text(encoding="utf-8")
    validator = Path(
    ...
    assert 'raise RuntimeError("V30 nonzero intervention reach failed")' in runner
    assert 'raise ValueError("V30 nonzero intervention reach was not proven")' in validator
...
def test_v30_runs_full_validator_after_receipt_binding() -> None:
    source = Path(
        "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v30.py"
    ).read_text(encoding="utf-8")
    assert 'argument for argument in command if argument != "--allow-missing-validator-receipt"' in source
    assert 'raise RuntimeError("V30 final validator result mismatch")' in source

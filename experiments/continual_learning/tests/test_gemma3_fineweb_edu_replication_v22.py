...
def test_locked_constants_and_exact_bootstrap() -> None:
    assert c.STATE_SLICE.endswith("v22")
...
def test_review_is_mandatory_and_loaders_guard_before_import() -> None:
    ...
    for function_name, import_text, guard_text in (
        (
            ...
            "V22 tokenizer custody/runtime mismatch",
        ),
        (
            "load_runtime",
            "from mlx_lm import load",
            "V22 model custody/runtime mismatch",
        ),
    ):
...
def test_v22_contract_references_are_defined() -> None:
    for relative in (
...
def test_enriched_review_snapshot_projects_canonical_fields(monkeypatch) -> None:
    from experiments.continual_learning import (
        stage_and_run_gemma3_fineweb_edu_replication_v22 as runner,
    )
...
def test_result_and_nested_schemas_are_closed_world() -> None:
    ...
    assert 'exact_keys(config, CONFIG_RESULT_KEYS, "V22 config")' in source
    assert 'exact_keys(results, RESULT_KEYS, "V22 results")' in source
    assert 'exact_keys(result_receipt, RECEIPT_KEYS, "V22 result receipt")' in source
    assert 'exact_keys(row, METRIC_ROW_KEYS' in source
    assert 'exact_keys(row,' in source and "V22 assessment_per_document row" in source
...
def test_independent_validator_does_not_import_runner_measurement_functions() -> None:
    ...
    assert "stage_and_run_gemma3_fineweb_edu_replication_v22" not in source
...
def test_v22_reach_and_post_validation_gates_are_hard() -> None:
    runner = Path(
        "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v22.py"
    ).read_text(encoding="utf-8")
    validator = Path(
    ...
    assert 'raise RuntimeError("V22 nonzero intervention reach failed")' in runner
    assert 'raise ValueError("V22 nonzero intervention reach was not proven")' in validator
...
def test_v22_runs_full_validator_after_receipt_binding() -> None:
    source = Path(
        "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v22.py"
    ).read_text(encoding="utf-8")
    assert 'argument for argument in command if argument != "--allow-missing-validator-receipt"' in source
    assert 'raise RuntimeError("V22 final validator result mismatch")' in source

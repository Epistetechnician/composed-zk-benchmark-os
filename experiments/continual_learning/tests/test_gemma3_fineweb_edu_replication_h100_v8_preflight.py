...
def _manifest() -> dict[str, object]:
    values: dict[str, object] = {
        ...
        "provider_attestation_key_id": "gman-test-key-v8",
        "provider_trust_root_id": "gman-test-root-v8",
        ...
        "runner_entrypoint": "run_h100_v8.sh",
        "runtime_lock_path": "runtime-lock.json",
        "runtime_lock_sha256": "2" * 64,
        "network_lock": "network-none-v8",
        ...
        "external_storage_namespace": "bucket/namespace-v8",
        "review_receipt_path": "review.json",
        "review_receipt_sha256": "e" * 64,
        "protocol_sha256": "f" * 64,
        "packet_sha256": "0" * 64,
        "launch_command": "./run_h100_v8.sh",
        "launch_command_sha256": hashlib.sha256(b"./run_h100_v8.sh").hexdigest(),
...
def test_budget_requires_exact_decimal_product(tmp_path: Path) -> None:
    values = _manifest()
    values["estimated_max_total_usd"] = 5.0000000001
    values["manifest_sha256"] = _digest(values, "manifest_sha256")
    path = tmp_path / "manifest.json"
    _write(path, values)
    with pytest.raises(ValueError, match="budget arithmetic"):

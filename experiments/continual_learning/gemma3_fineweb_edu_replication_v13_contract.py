#!/usr/bin/env python3
"""V13 custody and statistical contract.
...
STATE_SLICE = "continual-learning-gemma3-fineweb-edu-replication-v13"
CLAIM_CEILING = "LocalDevelopmentGemma3FineWebEduReplicationV13"
RAW_ROOT = Path(
...
SOURCE_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v13-source"
)
CORPUS_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v13-corpus"
)
RESULT_ROOT = Path(
    "/Volumes/PrimaryED/ResearchArtifacts/composed-zk-benchmark-os/gemma3-fineweb-edu-replication-v13-result"
)
...
PROTOCOL_SHA256 = "a8c98f0bf550d0e3cfddf8cd669643f30c74ef0698246cae3d19624e92d159c4"
SOURCE_SCHEMA = "gemma3-fineweb-edu-replication-v13-source"
CORPUS_SCHEMA = "gemma3-fineweb-edu-replication-v13-corpus"
RESULT_SCHEMA = "gemma3-fineweb-edu-replication-v13-result"
REVIEW_SCHEMA = "gemma3-fineweb-edu-replication-v13-independent-review"
...
IMPLEMENTATION_FILES = (
    ...
    PACKET_PATH,
    REPO_ROOT
    / "experiments/continual_learning/gemma3_fineweb_edu_replication_v13_contract.py",
    REPO_ROOT
    / "experiments/continual_learning/prepare_gemma3_fineweb_edu_replication_v13.py",
    REPO_ROOT
    / "experiments/continual_learning/validate_gemma3_fineweb_edu_replication_v13.py",
    REPO_ROOT
    / "experiments/continual_learning/stage_and_run_gemma3_fineweb_edu_replication_v13.py",
    REPO_ROOT
    / "experiments/continual_learning/tests/test_gemma3_fineweb_edu_replication_v13.py",
)
...
def exact_or_staging(path: Path, expected: Path, label: str) -> Path:
    ...
    if supplied != expected and (
        ...
        raise ValueError(f"{label} is not the V13 root or staging sibling")
    reject_symlink_components(supplied, label)
    return supplied
...
def require_native_network_denial() -> None:
    if not native_network_denied():
        raise RuntimeError("V13 native outbound network denial is not proven")
...
def network_block() -> Iterator[None]:
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError("V13 network/process escape denied")
...
def implementation_manifest() -> dict[str, Any]:
    ...
    for path in IMPLEMENTATION_FILES:
        reject_symlink_components(path.absolute(), "V13 implementation file")
        item = regular(path, "V13 implementation file")
        files.append(
            {
...
def validate_review_receipt(path: Path) -> dict[str, Any]:
    path = exact_path(path, RECEIPT_PATH, "V13 review receipt")
    value = json.loads(regular(path, "V13 review receipt").read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("V13 review receipt must be an object")
    ...
    if (
        ...
        raise ValueError("V13 review receipt self-digest mismatch")
    timestamp = value.get("reviewed_at_utc")
    if not isinstance(timestamp, str) or not timestamp.endswith("Z"):
        raise ValueError("V13 review timestamp missing")
    parsed = datetime.fromisoformat(timestamp[:-1] + "+00:00")
    if parsed.microsecond or timestamp != parsed.strftime("%Y-%m-%dT%H:%M:%SZ"):
        raise ValueError("V13 review timestamp noncanonical")
    if (
        ...
        raise ValueError("V13 review receipt binding mismatch")
    if value.get("findings") != {name: True for name in REVIEW_FINDINGS}:
        raise ValueError("V13 receipt requires seven true findings")
    return value
...
def snapshot_code() -> dict[str, Any]:
    return {
        "protocol": regular(PROTOCOL_PATH, "V13 protocol").read_bytes(),
        "packet": regular(PACKET_PATH, "V13 packet").read_bytes(),
        "receipt": regular(RECEIPT_PATH, "V13 receipt").read_bytes(),
...
def assert_code_snapshot(snapshot: dict[str, Any]) -> None:
    if snapshot_code() != snapshot:
        raise RuntimeError("V13 reviewed bytes or history changed")

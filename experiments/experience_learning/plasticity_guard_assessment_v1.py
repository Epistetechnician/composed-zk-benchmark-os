"""Fresh-cohort plasticity-guard assessment on custodied real streams.

State slice: ``oaklab-experience-learning-benchmark-v2``.

The plan is sealed before assessment: one 256-row tune cohort is processed and
seven later, disjoint 256-row cohorts are paired between fixed SGD and the
unchanged plasticity guard. No threshold, order, split, or endpoint is tuned
...
PLAN = {
    "version": "plasticity-guard-real-fresh-cohort-v1",
    "cohort_size": 256,
    "tune_cohort_indices": [0],
    "assessment_cohort_indices": [1, 2, 3, 4, 5, 6, 7],
...
def paired_normal_power(n: int, standardized_effect: float, alpha: float = 0.05) -> float:
    """Approximate two-sided paired-test power under a normal alternative."""
...
def assess(experiences: Sequence[Experience], dataset: str) -> dict:
    cohort_size = PLAN["cohort_size"]
    required = (max(PLAN["assessment_cohort_indices"]) + 1) * cohort_size
    if len(experiences) < required:
        raise ValueError(f"{dataset} requires at least {required} experiences for the sealed plan")
    if any(item.step != index for index, item in enumerate(experiences[:required])):
        raise ValueError("fresh-cohort assessment requires contiguous source order")
    ...
    planned_power = paired_normal_power(
        len(assessment_ids), PLAN["target_standardized_effect"], PLAN["alpha"]
    )
    ...
    return {
        "schema_version": "oaklab.experience-learning.plasticity-guard-assessment.v1",

"""Hermetic V8 protocol compiler and fail-closed validator tests.
...
from experiments.experience_learning import compile_oaklab_h100_v8_protocol as compiler
from experiments.experience_learning import validate_oaklab_h100_v8_protocol as validator
...
def test_campaign_manifest_artifact_is_current_and_validated():
    compiled = compiler.compile_protocol(ROOT)
    artifact = validator.validate_campaign_manifest_artifact(
        ROOT / validator.CAMPAIGN_MANIFEST_ARTIFACT,
        validator.sha256_file(SOURCE),
        validator.sha256_file(COMPILED),
        validator.sha256_file(ROOT / validator.REVIEW_PACKET),
        ROOT,
    )
    assert artifact["compiled_protocol_file_sha256"] == validator.sha256_file(COMPILED)
    assert artifact["manifest_sha256"]
    assert compiled["campaign_manifest_artifact"]["path"] == str(validator.CAMPAIGN_MANIFEST_ARTIFACT)
...
def test_provider_receipts_require_same_node_currency_ceiling_and_interval(tmp_path: Path):
    ...
    core = {"schema": "oaklab.h100.v8.campaign.v1", "state_slice": validator.STATE_SLICE, "compiled_protocol_sha256": "1" * 64, "protocol_review_sha256": "2" * 64, "review_packet_sha256": "3" * 64, "code_sha256": "4" * 64, "model_sha256": "5" * 64, "data_sha256": "6" * 64, "backend_sha256": "7" * 64, "guard_sha256": "8" * 64, "hard_usd_ceiling": 5.0}
    ...
    alloc = {"schema": "oaklab.h100.v8.provider-allocation.v1", "state_slice": validator.STATE_SLICE, "allocation_id": "a", "node_id": "node-a", "provider": "givemeanode", "gpu_model": "H100", "start_utc": "2026-08-31T20:00:00Z", "hard_usd_ceiling": 5.0, "launch_manifest_sha256": core_sha}
    cost = {"schema": "oaklab.h100.v8.provider-cost.v1", "state_slice": validator.STATE_SLICE, "allocation_id": "a", "node_id": "node-a", "charged_usd": 1.0, "currency": "USD", "hard_usd_ceiling": 5.0, "launch_manifest_sha256": core_sha}
    stop = {"schema": "oaklab.h100.v8.provider-stop.v1", "state_slice": validator.STATE_SLICE, "allocation_id": "a", "node_id": "node-a", "start_utc": "2026-08-31T20:00:00Z", "stop_utc": "2026-08-31T20:01:00Z", "stop_reason": "bounded", "launch_manifest_sha256": core_sha}
...
def test_energy_denominator_must_match_counter_total(tmp_path: Path):
    ...
    body = {"schema": "oaklab.h100.v8.energy.v1", "state_slice": validator.STATE_SLICE, "trace_sha256": validator.sha256_file(trace), "sample_count": 2, "joules": 2.0, "learned_events": 9, "joules_per_learned_event": 2 / 9, "reference_joules_per_learned_event": 0.2, "formula": "sum(0.5*(w_i+w_i+1)*(t_i+1-t_i)/1e9)", "denominator": "successfully_learned_events"}
...
def test_counter_and_family_rows_are_closed_and_digest_bound(tmp_path: Path):
    counter = {"schema": "oaklab.h100.v8.counter-rows.v1", "state_slice": validator.STATE_SLICE, "rows": [], "rows_sha256": validator.digest([])}
    row = {"schema": "oaklab.h100.v8.counter-row.v1", "state_slice": validator.STATE_SLICE, "phase": "fit", "cohort": "fit", "family": "drift", "seed": 1, "local_row": 0, "arm_id": "candidate", "learned_events": 1, "active_operations": 2, "parameter_updates": 1, "storage_bytes": 8, "latency_ns": 3}
...
def test_execution_authorization_rejects_unbound_or_missing_accept(tmp_path: Path):
    plan = {"schema": "oaklab.h100.v8.provider-plan.v1", "state_slice": validator.STATE_SLICE, "provider": "givemeanode", "gpu_model": "H100", "hard_usd_ceiling": 1.0, "job_count": 1, "bounded": True, "packet_sha256": "1" * 64, "source_sha256": validator.sha256_file(SOURCE), "compiled_sha256": validator.sha256_file(COMPILED), "campaign_core_sha256": "2" * 64, "manifest_sha256": "3" * 64}
    review = {"schema": "oaklab.h100.v8.protocol-review.v1", "state_slice": validator.STATE_SLICE, "packet_sha256": plan["packet_sha256"], "source_sha256": plan["source_sha256"], "compiled_sha256": plan["compiled_sha256"], "reviewer": "x", "review_decision": "REJECT", "effects_run": False, "findings": {"all": False}, "reviewed_at_utc": "2026-08-31T20:00:00Z", "receipt_sha256": "0" * 64, "signature_hex": "0" * 128, "public_key_hex": "0" * 64}
    synthetic = {"schema": "oaklab.h100.v8.synthetic-qualification.v1", "state_slice": validator.STATE_SLICE, "candidate": True, "source_digest": validator.sha256_file(SOURCE), "result_digest": "4" * 64}
    preflight = {"schema": "oaklab.h100.v8.no-spend-preflight.v1", "state_slice": validator.STATE_SLICE, "spend_usd": 0, "network_access": False, "model_loaded": False, "bounded": True, "receipt_sha256": "0" * 64}
...
def test_execution_authorization_requires_actual_validated_campaign_manifest(tmp_path: Path):
    plan = {"schema": "oaklab.h100.v8.provider-plan.v1", "state_slice": validator.STATE_SLICE, "provider": "givemeanode", "gpu_model": "H100", "hard_usd_ceiling": 1.0, "job_count": 1, "bounded": True, "packet_sha256": "1" * 64, "source_sha256": validator.sha256_file(SOURCE), "compiled_sha256": validator.sha256_file(COMPILED), "campaign_manifest_path": str(tmp_path / "campaign_manifest.json"), "campaign_manifest_sha256": "2" * 64, "campaign_core_sha256": "3" * 64}
    (tmp_path / "plan.json").write_bytes(validator.canonical(plan))
    with pytest.raises(ValueError, match="campaign manifest|missing|provider plan"):
        validator.validate_execution_authorization(tmp_path / "review.json", tmp_path / "synthetic.json", tmp_path / "preflight.json", tmp_path / "plan.json")

#!/usr/bin/env python3
"""Compile the Oak Lab H100 replication V8 protocol.
...
COMPILER_VERSION = "oaklab.h100.v8.protocol-compiler.v1"
SECTIONS = (
...
SOURCE_KEYS = {
    ...
    "campaign_manifest_artifact",
}
ROOT = Path(__file__).resolve().parents[2]
SOURCE_PATH = Path("experiments/experience_learning/oaklab_h100_v8_protocol.json")
COMPILED_PATH = Path("experiments/experience_learning/oaklab_h100_v8_compiled_protocol.json")
CAMPAIGN_MANIFEST_ARTIFACT_PATH = Path("experiments/experience_learning/oaklab_h100_v8_campaign_manifest.json")
FREEZE_FILES = (
    str(SOURCE_PATH),
...
def validate_campaign_manifest_contract(source: dict[str, Any]) -> None:
    ...
    require(artifact["path"] == str(CAMPAIGN_MANIFEST_ARTIFACT_PATH), "campaign manifest artifact path changed")
...
def validate_source(source: dict[str, Any]) -> None:
    ...
    roster = source["generator_roster"]
    expected = {"sparse_signal_v8", "drifting_relevance_v8", "delayed_reward_v8", "event_sensor_v8", "long_horizon_v8", "pure_noise_v8"}

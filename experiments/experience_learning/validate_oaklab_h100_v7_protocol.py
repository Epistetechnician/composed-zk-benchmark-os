#!/usr/bin/env python3
"""Independent V7 protocol and runtime-receipt validator.
...
SOURCE = Path("experiments/experience_learning/oaklab_h100_v7_protocol.json")
COMPILED = Path("experiments/experience_learning/oaklab_h100_v7_compiled_protocol.json")
REVIEW_PACKET = Path("docs/research/experience-learning/58-oaklab-h100-replication-v7-review-packet.md")
SECTIONS = (
...
def validate_source(source: dict[str, Any]) -> None:
    expected = {"schema_version", "protocol_id", "state_slice", "claim_ceiling", "status", "implementation_before_review", "estimand", *SECTIONS, "strict_contract", "stop_rules", "boundaries"}
    ...
    roster = source["generator_roster"]
    expected_streams = {"sparse_signal_v7", "drifting_relevance_v7", "delayed_reward_v7", "event_sensor_v7", "long_horizon_v7", "pure_noise_v7"}
...
def validate_compiled(repo_root: Path = Path.cwd()) -> dict[str, Any]:
    ...
    required = {"schema_version", "protocol_id", "state_slice", "claim_ceiling", "compiler_version", "source_spec_sha256", "freeze_file_sha256", "sections", "section_digests", "transcript_test_vector", "assessment_materialization_state", "compiled", "strict_contract", "estimand", "stop_rules", "boundaries", "execution_gate", "compiled_protocol_sha256"}
    ...
    require(compiled["sections"] == list(SECTIONS) and compiled["compiled"] == {name: source[name] for name in SECTIONS} and compiled["strict_contract"] == source["strict_contract"], "compiled section mismatch")
    ...
    require(compiled["execution_gate"] == {"review_accept_required": True, "packet_binding_required": True, "full_validation_required": True, "synthetic_candidate_required": True, "zero_spend_preflight_required": True, "provider_allocation_required": True, "one_bounded_job_maximum": True, "assessment_absent": True, "effects_run": False}, "execution gate changed")
...
def validate_campaign_manifest(path: Path, compiled_sha256: str) -> dict[str, Any]:
    ...
    keys = {"schema", "state_slice", "compiled_protocol_sha256", "protocol_review_sha256", "review_packet_sha256", "code_sha256", "model_sha256", "data_sha256", "backend_sha256", "guard_sha256", "fit_lock_sha256", "tune_lock_sha256", "tune_lock_receipt_sha256", "provider_allocation_sha256", "provider_cost_sha256", "provider_stop_sha256", "energy_receipt_sha256", "result_root_sha256", "hard_usd_ceiling", "manifest_core_sha256", "manifest_sha256"}
    require(set(value) == keys and value["schema"] == "oaklab.h100.v7.campaign.v1" and value["state_slice"] == STATE_SLICE, "campaign manifest schema is not closed")
    ...
    core_keys = ("schema", "state_slice", "compiled_protocol_sha256", "protocol_review_sha256", "review_packet_sha256", "code_sha256", "model_sha256", "data_sha256", "backend_sha256", "guard_sha256", "hard_usd_ceiling")
...
def validate_protocol_accept(path: Path, packet_sha256: str, source_sha256: str, compiled_sha256: str) -> dict[str, Any]:
    ...
    require(set(value) == expected and value["schema"] == "oaklab.h100.v7.protocol-review.v1" and value["state_slice"] == STATE_SLICE, "protocol review schema is not closed")
...
def validate_provider_receipts(root: Path, manifest: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    allocation = _signed_provider(root / "provider/allocation.json", "oaklab.h100.v7.provider-allocation.v1", manifest["manifest_core_sha256"], {"schema", "state_slice", "allocation_id", "node_id", "provider", "gpu_model", "start_utc", "hard_usd_ceiling", "launch_manifest_sha256", "public_key_hex", "signature_hex", "receipt_sha256"})
    cost = _signed_provider(root / "provider/cost.json", "oaklab.h100.v7.provider-cost.v1", manifest["manifest_core_sha256"], {"schema", "state_slice", "allocation_id", "node_id", "charged_usd", "currency", "hard_usd_ceiling", "launch_manifest_sha256", "public_key_hex", "signature_hex", "receipt_sha256"})
    stop = _signed_provider(root / "provider/stop.json", "oaklab.h100.v7.provider-stop.v1", manifest["manifest_core_sha256"], {"schema", "state_slice", "allocation_id", "node_id", "start_utc", "stop_utc", "stop_reason", "launch_manifest_sha256", "public_key_hex", "signature_hex", "receipt_sha256"})
...
def validate_fit_lock(path: Path, compiled_sha256: str, review_receipt_sha256: str) -> dict[str, Any]:
    ...
    require(set(value) == expected and value["schema"] == "oaklab.h100.v7.fit-lock.v1" and value["state_slice"] == STATE_SLICE, "fit lock schema is not closed")
...
def validate_tune_lock(path: Path, fit_lock_sha256: str) -> dict[str, Any]:
    ...
    require(set(value) == expected and value["schema"] == "oaklab.h100.v7.tune-lock.v1" and value["state_slice"] == STATE_SLICE, "tune lock schema is not closed")
...
def validate_lock_receipt(path: Path, tune_lock_sha256: str, review_receipt_sha256: str) -> dict[str, Any]:
    ...
    require(set(value) == expected and value["schema"] == "oaklab.h100.v7.lock-receipt.v1" and value["state_slice"] == STATE_SLICE, "lock receipt schema is not closed")
...
def validate_energy_receipt(receipt_path: Path, trace_path: Path, expected_learned_events: int) -> dict[str, Any]:
    ...
    require(set(value) == expected and value["schema"] == "oaklab.h100.v7.energy.v1" and value["state_slice"] == STATE_SLICE, "energy receipt schema is not closed")
...
def validate_counter_rows(path: Path) -> list[dict[str, Any]]:
    ...
    rows = _validate_rows_container(path, "oaklab.h100.v7.counter-rows.v1", keys, "counter_sha256")
    for row in rows:
...
def validate_family_rows(path: Path) -> list[dict[str, Any]]:
    ...
    rows = _validate_rows_container(path, "oaklab.h100.v7.family-rows.v1", keys, "row_sha256")
...
def validate_aggregate(path: Path, family_rows: list[dict[str, Any]], totals: dict[str, dict[str, int]], energy: dict[str, Any], custody_ok: bool) -> dict[str, Any]:
    ...
    require(set(value) == expected and value["schema"] == "oaklab.h100.v7.aggregate.v1" and value["state_slice"] == STATE_SLICE, "aggregate schema is not closed")
...
def validate_independent_result(path: Path, manifest: dict[str, Any], aggregate: dict[str, Any]) -> dict[str, Any]:
    ...
    require(set(value) == expected and value["schema"] == "oaklab.h100.v7.independent-validation.v1" and value["state_slice"] == STATE_SLICE, "independent validation schema is not closed")
...
def validate_execution_authorization(review_path: Path, synthetic_path: Path, preflight_path: Path, provider_plan_path: Path) -> dict[str, Any]:
    validate_compiled(Path.cwd())
    plan = load_canonical_json(provider_plan_path)
    require(set(plan) == {"schema", "state_slice", "provider", "gpu_model", "hard_usd_ceiling", "job_count", "bounded", "packet_sha256", "source_sha256", "compiled_sha256", "campaign_core_sha256", "manifest_sha256"} and plan["schema"] == "oaklab.h100.v7.provider-plan.v1" and plan["state_slice"] == STATE_SLICE, "provider plan schema is not closed")
    for key in ("packet_sha256", "source_sha256", "compiled_sha256", "campaign_core_sha256", "manifest_sha256"):
        require_hex(plan[key], f"provider plan {key}")
    require(plan["packet_sha256"] == sha256_file(REVIEW_PACKET) and plan["source_sha256"] == sha256_file(SOURCE) and plan["compiled_sha256"] == sha256_file(COMPILED) and plan["provider"] == "givemeanode" and plan["gpu_model"] == "H100" and plan["job_count"] == 1 and plan["bounded"] is True and isinstance(plan["hard_usd_ceiling"], (int, float)) and not isinstance(plan["hard_usd_ceiling"], bool) and math.isfinite(float(plan["hard_usd_ceiling"])) and plan["hard_usd_ceiling"] > 0, "provider plan is not bound to current packet/source/compiled artifact")
    ...
    require(set(synthetic) == {"schema", "state_slice", "candidate", "source_digest", "result_digest"} and synthetic["schema"] == "oaklab.h100.v7.synthetic-qualification.v1" and synthetic["state_slice"] == STATE_SLICE and synthetic["source_digest"] == sha256_file(SOURCE) and synthetic["candidate"] is True and require_hex(synthetic["result_digest"], "synthetic result digest"), "synthetic candidate is absent or unbound")
    preflight = load_canonical_json(preflight_path)
    require(set(preflight) == {"schema", "state_slice", "spend_usd", "network_access", "model_loaded", "bounded", "receipt_sha256"} and preflight["schema"] == "oaklab.h100.v7.no-spend-preflight.v1" and preflight["state_slice"] == STATE_SLICE, "preflight schema is not closed")

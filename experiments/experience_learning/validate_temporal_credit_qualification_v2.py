"""Independent aggregate-only validator for temporal-utility V2.
...
from .temporal_credit_qualification_v2 import PLAN, PLAN_DIGEST, SCHEMA_VERSION, STATE_SLICE
...
def validate(path: Path) -> dict:
    ...
    if result.get("schema_version") != SCHEMA_VERSION or result.get("state_slice") != STATE_SLICE:
        raise ValueError("temporal-credit schema or state slice mismatch")
    ...
    if execution.get("real_stream_execution") != "sealed_pending_review":
        raise ValueError("real-stream review gate changed")
    if execution.get("stream_names") != PLAN["stream_names"]:
        raise ValueError("stream family set changed")
    if execution.get("seed_offsets") != PLAN["seed_offsets"]:
        raise ValueError("seed cohort mismatch")
    if set(result.get("streams", {})) != set(PLAN["stream_names"]):
        raise ValueError("qualification stream matrix is incomplete")
    for stream_name, stream in result.get("streams", {}).items():
        ...
        for algorithm, record in algorithms.items():
            ...
            if not all(set(item.get("prediction_lock_snapshots", {})) == {"fit", "tune", "assessment"} for item in per_seed):
                raise ValueError(f"prediction-lock snapshots missing for {stream_name}/{algorithm}")
